...
`
	...
	currBranchPtr, err := getCurrentBranch()
	if err != nil {
		panic(err)
	...
	net, err := docker.NetworkCreate(ctx, fmt.Sprintf("supabase_network_%s", runId), types.NetworkCreate{})
	defer docker.NetworkRemove(ctx, net.ID)
	...
	dbName := fmt.Sprintf("supabase_db_%s", runId)
	...
	pgbouncerName := fmt.Sprintf("supabase_pgbouncer_%s", runId)
	...
	err = os.WriteFile("supabase/.temp/kong.yml", []byte(fmt.Sprintf(kongConfigTemplate, runId)), 0644)
	if err != nil {
		panic(err)
	}
	...
	}, hostConfig, nil, nil, fmt.Sprintf("supabase_kong_%s", runId))
	if err != nil {
		panic(err)
	}
	defer func() {
	...
	gotrueName := fmt.Sprintf("supabase_auth_%s", runId)
	...
	}, &container.HostConfig{}, nil, nil, fmt.Sprintf("supabase_realtime_%s", runId))
	if err != nil {
		panic(err)
	}
	...
	}, &container.HostConfig{}, nil, nil, fmt.Sprintf("supabase_rest_%s", runId))
	if err != nil {
		panic(err)
	}
	defer func() {
