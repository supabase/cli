...
func Run(ctx context.Context, slugs []string, useDocker bool, noVerifyJWT *bool, importMapPath string, maxJobs uint, prune bool, fsys afero.Fs) error {
    ...
    // --- BEGIN: JWT verification config change detection ---
    remoteResp, err := utils.GetSupabase().V1ListAllFunctionsWithResponse(ctx, flags.ProjectRef)
    if err == nil && remoteResp.JSON200 != nil {
    ...
    // --- END: JWT verification config change detection ---
