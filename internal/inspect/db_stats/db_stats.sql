SELECT
  ...
  (SELECT round(SUM(idx_blks_hit)::numeric / nullif(SUM(idx_blks_hit + idx_blks_read),0) * 100,2)::text || '%'
   FROM pg_statio_user_indexes)                                                                                               AS index_hit_rate,
  (SELECT round(SUM(heap_blks_hit)::numeric / nullif(SUM(heap_blks_hit + heap_blks_read),0) * 100,2)::text || '%'
