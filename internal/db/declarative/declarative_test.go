...
// TestSyncFromMigrationsUsesBaselineCatalogRef ensures sync --from-migrations
// uses the same baseline subtraction as generate --local (getBaselineCatalogRef),
// so declarative output does not include Supabase bootstrap cluster objects.
// That in turn allows sync --to-migrations to apply declarative files to the
// Supabase shadow without re-applying bootstrap (e.g. supabase_realtime, alter role postgres).
func TestSyncFromMigrationsUsesBaselineCatalogRef(t *testing.T) {
	fsys := afero.NewMemMapFs()
	require.NoError(t, afero.WriteFile(fsys, utils.ConfigPath, []byte("[db]\n"), 0644))
	original := utils.Config.Experimental.PgDelta
	utils.Config.Experimental.PgDelta = &config.PgDeltaConfig{Enabled: true}
	...
	// Pre-create baseline catalog cache (same path getBaselineCatalogRef returns when cache hit).
	require.NoError(t, fsys.Mkdir(filepath.Join(utils.TempDir, "pgdelta"), 0755))
	baselinePath := filepath.Join(pgDeltaTempPath(), fmt.Sprintf(baselineCatalogName, baselineVersionToken()))
	require.NoError(t, afero.WriteFile(fsys, baselinePath, []byte(`{"version":1,"currentUser":"postgres"}`), 0644))

	// Pre-create migrations catalog cache.
	require.NoError(t, fsys.Mkdir(utils.MigrationsDir, 0755))
	migrationPath := filepath.Join(utils.MigrationsDir, "20240101000000_first.sql")
	require.NoError(t, afero.WriteFile(fsys, migrationPath, []byte("create table a();"), 0644))
	hash, err := hashMigrations(fsys)
	require.NoError(t, err)
	migrationsCachePath := filepath.Join(utils.TempDir, "pgdelta", "catalog-local-migrations-"+hash+"-1000.json")
	require.NoError(t, afero.WriteFile(fsys, migrationsCachePath, []byte(`{"version":1}`), 0644))

	var gotSourceRef, gotTargetRef string
	declarativeExportRef = func(_ context.Context, sourceRef, targetRef string, _ []string, _ string, _ ...func(*pgx.ConnConfig)) (diff.DeclarativeOutput, error) {
		gotSourceRef = sourceRef
		gotTargetRef = targetRef
		return diff.DeclarativeOutput{}, nil
	}

	err = SyncFromMigrations(t.Context(), nil, false, fsys)
	require.NoError(t, err)
	assert.Equal(t, baselinePath, gotSourceRef, "SyncFromMigrations must use baseline catalog as source (same as generate --local)")
	assert.Equal(t, migrationsCachePath, gotTargetRef, "SyncFromMigrations must use migrations catalog as target")
}
...
func TestSyncFromMigrationsNoChangesSkipsWrite(t *testing.T) {
	fsys := afero.NewMemMapFs()
	require.NoError(t, afero.WriteFile(fsys, utils.ConfigPath, []byte("[db]\n"), 0644))
	original := utils.Config.Experimental.PgDelta
	utils.Config.Experimental.PgDelta = &config.PgDeltaConfig{Enabled: true}
	...
	existingPath := filepath.Join(utils.GetDeclarativeDir(), "cluster", "roles.sql")
	require.NoError(t, afero.WriteFile(fsys, existingPath, []byte("create role old;"), 0644))
	require.NoError(t, fsys.MkdirAll(filepath.Join(utils.TempDir, "pgdelta"), 0755))

	baselinePath := filepath.Join(pgDeltaTempPath(), fmt.Sprintf(baselineCatalogName, baselineVersionToken()))
	require.NoError(t, afero.WriteFile(fsys, baselinePath, []byte(`{"version":1}`), 0644))

	require.NoError(t, fsys.MkdirAll(utils.MigrationsDir, 0755))
	migrationPath := filepath.Join(utils.MigrationsDir, "20240101000000_first.sql")
	require.NoError(t, afero.WriteFile(fsys, migrationPath, []byte("create table a();"), 0644))
	hash, err := hashMigrations(fsys)
	require.NoError(t, err)
	...
	declHash, err := hashDeclarativeSchemas(fsys)
	require.NoError(t, err)
	declarativeCachePath := filepath.Join(utils.TempDir, "pgdelta", "catalog-local-declarative-"+declHash+"-1000.json")
	require.NoError(t, afero.WriteFile(fsys, declarativeCachePath, []byte(`{"version":1}`), 0644))

	same := diff.DeclarativeOutput{
		Files: []diff.DeclarativeFile{
			{Path: "cluster/roles.sql", SQL: "create role old;"},
		},
	}
	declarativeExportRef = func(_ context.Context, sourceRef, targetRef string, _ []string, _ string, _ ...func(*pgx.ConnConfig)) (diff.DeclarativeOutput, error) {
		assert.Equal(t, baselinePath, sourceRef)
		switch targetRef {
		case migrationsCachePath, declarativeCachePath:
			return same, nil
		default:
			return diff.DeclarativeOutput{}, fmt.Errorf("unexpected target ref %s", targetRef)
		}
	}
	...
	got, err := afero.ReadFile(fsys, existingPath)
	require.NoError(t, err)
	assert.Equal(t, "create role old;", string(got))
}

func TestSyncFromMigrationsChangesWriteSchemas(t *testing.T) {
	fsys := afero.NewMemMapFs()
	require.NoError(t, afero.WriteFile(fsys, utils.ConfigPath, []byte("[db]\n"), 0644))
	original := utils.Config.Experimental.PgDelta
	utils.Config.Experimental.PgDelta = &config.PgDeltaConfig{Enabled: true}
	...
	extracted := diff.DeclarativeOutput{
		Files: []diff.DeclarativeFile{
			{Path: "cluster/roles.sql", SQL: "create role new;"},
		},
	}
	current := diff.DeclarativeOutput{
		Files: []diff.DeclarativeFile{
			{Path: "cluster/roles.sql", SQL: "create role old;"},
		},
	}
	declarativeExportRef = func(_ context.Context, sourceRef, targetRef string, _ []string, _ string, _ ...func(*pgx.ConnConfig)) (diff.DeclarativeOutput, error) {
		assert.Equal(t, baselinePath, sourceRef)
		switch targetRef {
		case migrationsCachePath:
			return extracted, nil
		case declarativeCachePath:
			return current, nil
		default:
			return diff.DeclarativeOutput{}, fmt.Errorf("unexpected target ref %s", targetRef)
		}
	...
	got, err := afero.ReadFile(fsys, existingPath)
	require.NoError(t, err)
	assert.Equal(t, "create role new;", string(got))
}
