...
func TestLinkCommand(t *testing.T) {
	...
	t.Run("link valid project", func(t *testing.T) {
		...
		content, err := afero.ReadFile(fsys, utils.Paths.ProjectRefPath)
		assert.NoError(t, err)
		assert.Equal(t, []byte(project), content)
		restVersion, err := afero.ReadFile(fsys, utils.Paths.RestVersionPath)
		assert.NoError(t, err)
		assert.Equal(t, []byte("v"+rest.Info.Version), restVersion)
		authVersion, err := afero.ReadFile(fsys, utils.Paths.GotrueVersionPath)
		assert.NoError(t, err)
		assert.Equal(t, []byte(auth.Version), authVersion)
		storageVersion, err := afero.ReadFile(fsys, utils.Paths.StorageVersionPath)
		assert.NoError(t, err)
		assert.Equal(t, []byte("v"+storage), storageVersion)
		postgresVersion, err := afero.ReadFile(fsys, utils.Paths.PostgresVersionPath)
		assert.NoError(t, err)
...
func TestStatusCheck(t *testing.T) {
	...
	t.Run("updates postgres version when healthy", func(t *testing.T) {
		...
		// Check error
		assert.NoError(t, err)
		version, err := afero.ReadFile(fsys, utils.Paths.PostgresVersionPath)
		assert.NoError(t, err)
	...
	t.Run("ignores project not found", func(t *testing.T) {
		...
		// Check error
		assert.NoError(t, err)
		exists, err := afero.Exists(fsys, utils.Paths.PostgresVersionPath)
		assert.NoError(t, err)
		assert.False(t, exists)
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
...
func TestLinkDatabase(t *testing.T) {
	t.Run("syncs storage migration", func(t *testing.T) {
		...
		// Check error
		assert.NoError(t, err)
		storage, err := afero.ReadFile(fsys, utils.Paths.StorageMigrationPath)
		assert.NoError(t, err)
	...
	t.Run("ignores missing server version", func(t *testing.T) {
		...
		// Check error
		assert.NoError(t, err)
		version, err := afero.ReadFile(fsys, utils.Paths.StorageMigrationPath)
		assert.NoError(t, err)
	...
	t.Run("throws error on query failure", func(t *testing.T) {
		...
		exists, err := afero.Exists(fsys, utils.Paths.StorageMigrationPath)
		assert.NoError(t, err)
		assert.False(t, exists)
	})
}
