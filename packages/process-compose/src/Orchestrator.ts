        ...
        const setDesired = (name: string, desired: ServiceDesiredState): Effect.Effect<void> => {
          ...
          return SubscriptionRef.update(svc.state, (state) =>
            state.desired === desired
              ? state
              : new ServiceState({ ...serviceStateFields(state), desired }),
          );
        };
        ...
        const runHooks = (def: ServiceDef, trigger: HookTrigger): Effect.Effect<string | null> =>
          Effect.gen(function* () {
            ...
            for (const hook of hooks) {
              ...
              const result = yield* hook.run(log).pipe(
                Effect.mapError((cause) => new HookExecutionError({ cause })),
                ...
                Effect.gen(function* () {
                  ...
                  const runCleanup = () =>
                    def.cleanup == null
                      ? Effect.void
                      : def.cleanup.pipe(
                          Effect.mapError((cause) => new CleanupExecutionError({ cause })),
