...
`create({ ..., startOwner: true })` registers the stack and then starts its owner immediately; if the owner fails to launch, `create` removes the registration it just saved and fails with the launch error.

`destroy` normally returns `{ runtimeCleanup: "complete" }`. When no owner is running and the stack's container engine is unreachable, `destroy` removes the local registration and data anyway and returns `{ runtimeCleanup: "skipped", engine }`; its containers are not removed and remain until a later start of the same stack sweeps them.
