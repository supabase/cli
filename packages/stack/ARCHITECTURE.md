...
During Starting, acquire the exclusive stack lease, load the instance definitions and saved resources needed for normal restart, construct components and open the control endpoint. The lease is held by an operating-system locking primitive whose ownership ends with the process; an on-disk PID/endpoint file is only discovery metadata and never proves an active owner. A competing launcher connects to the winning owner. For a container stack, remove containers labeled with this stack before accepting work. On Unix, start a detached sentinel after that first sweep; it waits for old container mutations to settle and sweeps again before readiness. This is cleanup of proven-owned workloads, not adoption or replay of interrupted operations. Other runtime or port conflicts remain errors.
...
Where the platform delivers SIGTERM or SIGINT to the host, treat it as the same graceful shutdown request as `host.stop`. Repeated shutdown requests join that shutdown; they do not pre-empt executing transitions. On Unix, native launchers stop their process groups when the host pipe closes. A detached container sentinel observes host death, removes containers bearing that host generation, waits for in-flight container mutations, then checks again. This is a crash-cleanup backstop; it does not turn a forced termination into an acknowledged graceful stop.
...
**Successful whole-stack shutdown.** `stack.stop()` reports success only after admitted work settles, every owned live service and tool workload has stopped (including native processes, descendants and Docker containers), the current container generation is confirmed absent, stack listeners close, the shutdown RPC is acknowledged, and the detached host's exit is confirmed. If workload cleanup cannot be confirmed, stop fails and the live host retains ownership for inspection and retry; the stack is not reported stopped. If cleanup succeeds but host exit cannot be confirmed, stop also fails, though the host may already have exited. A delivered SIGTERM or SIGINT follows this cleanup path. Stop preserves stack definitions, service data, caches and saved port assignments. `destroy` follows the same live-workload cleanup, then removes only proven-owned persistent data.
...
Returning to Serving after cleanup failure does not undo completed cleanup. Unexpected host death does not resume interrupted operations or restore live service state. The Unix sentinel cleans containers from the dead host generation without removing volumes or saved definitions; the next host startup sweeps proven-owned leftovers if that cleanup could not finish. Neither path can guarantee immediate cleanup if both host and sentinel die, a container command hangs indefinitely, or the container daemon completes a request after its client disappears. Windows has no detached container sentinel. A lost control response is reported as uncertain; do not blindly retry a mutation.
...
await baseline.stop();
await baseline.saveSnapshot("baseline");

if (!(await shadow.restoreSnapshot("baseline"))) {
  // A miss leaves the empty instance in place; initialize that same instance.
}
await shadow.start();
await shadow.ready();
```
