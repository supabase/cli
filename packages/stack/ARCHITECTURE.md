...
Functions serve reports an unhealthy runtime separately from an exited one; its current `failed`-means-exited check must change. DB/storage readiness checks require running and healthy. The current operation remains observable in memory, but shadow finalizers no longer poll it: `destroy()` queues behind an executing snapshot and runs after settlement. Do not persist it for crash recovery. Health observations are attached to the current launch; they cannot make a replacement healthy or failed.
...
| Functions serve      | Launch/restart → observe health and exit; permit inspector before application health                     |
