...
/** Connects, signals `opened`, then drains the socket until the peer closes it. */
const holdOpen = (socket: Socket.Socket, opened: Deferred.Deferred<void>) =>
  Effect.scoped(
    Effect.gen(function* () {
...
describe("native runtime", { timeout: 15_000 }, () => {
  ...
  const runProcessTreeScenario = (runtimeCommand: string) =>
    withPlatform(
      Effect.gen(function* () {
        ...
        const targetClosed = yield* holdOpen(targetSocket, targetClosedSignal).pipe(
          Effect.exit,
          Effect.forkChild({ startImmediately: true }),
        );
        yield* Deferred.await(targetClosedSignal);
        const descendantClosedSignal = yield* Deferred.make<void>();
        const descendantClosed = yield* holdOpen(descendantSocket, descendantClosedSignal).pipe(
          Effect.exit,
          Effect.forkChild({ startImmediately: true }),
        );
  ...
  const runOwnerLossScenario = (options: {
    ...
    Effect.gen(function* () {
      ...
      const targetClosed = yield* holdOpen(targetSocket, targetOpened).pipe(
        Effect.exit,
        Effect.forkChild({ startImmediately: true }),
      );
      yield* Deferred.await(targetOpened);
      const descendantOpened = yield* Deferred.make<void>();
      const descendantClosed = yield* holdOpen(descendantSocket, descendantOpened).pipe(
        Effect.exit,
        Effect.forkChild({ startImmediately: true }),
      );
    ...
    withPlatform(
      Effect.gen(function* () {
        ...
        `;
          ...
          const closed = yield* holdOpen(socket, opened).pipe(
            Effect.exit,
            Effect.forkChild({ startImmediately: true }),
          );
