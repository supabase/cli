...
export const runDatabaseBootstrap = Effect.fn("DatabaseBootstrap.run")(function* (
  session: DatabaseSession,
  options: DatabaseBootstrapOptions,
) {
  return yield* session.transaction((transaction) =>
    Effect.gen(function* () {
      yield* transaction
      ...
      yield* transaction.execute("SET LOCAL log_min_duration_sample = -1");
