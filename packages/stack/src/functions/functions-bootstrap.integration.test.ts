...
const setupBootstrapOwner = (prefix: string) =>
  Effect.gen(function* () {
    ...
    return { fs, root, stackId, instanceId, owner };
  });
...
describe("functions bootstrap owner", () => {
  it.live("publishes a private Functions bootstrap file with restrictive modes", () =>
    Effect.gen(function* () {
      ...
      const { fs, root, stackId, instanceId, owner } = yield* setupBootstrapOwner(
        "stack-functions-bootstrap-",
      );
      const target = yield* owner.write({ instanceId, content: "export default 1" });
      expect(target).toContain(
        path.join(root, stackId, "runtime", "instances", "primary", "functions", "index.ts"),
      );
  ...
  it.live("recreates readable Functions bootstrap content after cleanup", () =>
    Effect.gen(function* () {
      const { fs, instanceId, owner } = yield* setupBootstrapOwner(
        "stack-functions-bootstrap-recreate-",
      );
      const first = yield* owner.write({ instanceId, content: "export default 1" });
      ...
      const recreated = yield* owner.write({ instanceId, content: "export default 2" });
  ...
  it.live("returns the canonical published path for a symlinked state root", () =>
    Effect.gen(function* () {
      ...
      const target = yield* owner.write({ instanceId, content: "export default 2" });
