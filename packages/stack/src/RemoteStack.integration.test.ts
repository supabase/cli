...
describe("RemoteStack integration", () => {
  ...
  test("preserves daemon identity for invalid SSE responses", async () => {
    const scenarios = [
      ...
      {
        response: () =>
          new Response('data: {"name":"postgres"}\n\n', {
            status: 200,
            headers: { "content-type": "text/event-stream" },
          }),
        reason: "protocol",
    ...
    for (const scenario of scenarios) {
      ...
      try {
        const exit: Exit.Exit<unknown, unknown> =
          scenario.stream === "status"
            ? await runtime.runPromise(
                Effect.flatMap(Stack, (stack) => Stream.runCollect(stack.allStateChanges())).pipe(
                  Effect.exit,
                ),
              )
            : await runtime.runPromise(
        ...
        if (Exit.isFailure(exit)) {
          ...
          if (Result.isSuccess(defect)) {
            expect(defect.success).toBeInstanceOf(HttpTransportClientError);
            expect(defect.success).toMatchObject({ reason: scenario.reason, path: scenario.path });
          }
        }
      } finally {
        await runtime.dispose();
