 ...
 *
 * A stack that was given an identity is keyed by it; one that was not falls back
 * to its api port, which is what names were always built from. The fallback is
 * what keeps the direct ephemeral API unchanged, and the identity is what stops
 * two stacks that happen to share a port — a crashed one's leftovers and the
 * sibling that reused its port — from colliding on the same container names.
 */
export interface StackIdentity {
  /** What container names are built from. */
  readonly key: string;
...
export const stackIdentity = (
  ...
  key: config.instanceId ?? String(config.apiPort),
