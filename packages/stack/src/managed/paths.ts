...
const requireManagedStateRootEffect = (
  stateRoot: string,
  ...
  return trimmed === undefined
    ...
    : Effect.succeed(resolve(trimmed));
};
...
export const resolveManagedStateRootEffect = (
  ...
  if (options.stateRoot !== undefined) {
    return requireManagedStateRootEffect(options.stateRoot);
  }
...
export const requireExplicitManagedStateRootEffect = (
  ...
  stateRoot === undefined
    ...
    : resolveManagedStateRootEffect({ stateRoot });
...
const validateManagedStackId = (
  ...
  SHA256_STACK_ID_PATTERN.test(stackId)
    ? Effect.succeed(stackId)
    : validateManagedUuid(stackId, "stackId");
...
export const managedStackDocumentPathEffect = (
  ...
  managedStackPathsEffect(stateRoot, stackId).pipe(
    Effect.map(({ root }) => join(root, "stack.json")),
  );
