...
describe("BinaryResolver cache publication", () => {
  ...
  it.live("recovers an abandoned lock before the retry budget is exhausted", () => {
    const root = makeTempRoot();
    const archive = makeArchive(root);
    let requestCount = 0;
    const layer = makeResolverLayer(root, archive, () => {
    ...
    return Effect.gen(function* () {
      const resolver = yield* BinaryResolver;
      const cacheDir = yield* authCachePath(root);
      const abandonedLock = `${cacheDir}.lock`;
      mkdirSync(abandonedLock, { recursive: true });
      writeFileSync(join(abandonedLock, "owner.pid"), "2147483647");
      const abandonedAt = new Date(Date.now() - 6_000);
      utimesSync(abandonedLock, abandonedAt, abandonedAt);
      ...
      expect(results.filter((result) => result.downloaded)).toHaveLength(1);
      expect(results.every((result) => result.path === cacheDir)).toBe(true);
      expect(requestCount).toBe(1);
      expect(existsSync(abandonedLock)).toBe(false);
    }).pipe(Effect.provide(layer));
  });

  it.live("preserves a live legacy numeric lock owner", () => {
    const root = makeTempRoot();
    const archive = makeArchive(root);
    let requestCount = 0;
    const layer = makeResolverLayer(root, archive, () => {
    ...
    return Effect.gen(function* () {
      ...
      writeFileSync(join(lockPath, "owner.pid"), String(process.pid));
      ...
      writeFileSync(join(lockPath, "owner.pid"), "2147483647");
      yield* Fiber.join(resolution);
      expect(requestCount).toBe(1);
    }).pipe(Effect.provide(layer), Effect.scoped);
  });

  it.live("reclaims a lock whose PID belongs to a different process generation", () => {
    const root = makeTempRoot();
    const archive = makeArchive(root);
    let requestCount = 0;
    const layer = makeResolverLayer(root, archive, () => {
    ...
    return Effect.gen(function* () {
      ...
      mkdirSync(lockPath, { recursive: true });
      writeFileSync(
        join(lockPath, "owner.reused"),
        JSON.stringify({
          pid: process.pid,
          bootMinute: Math.round((Date.now() - uptime() * 1_000) / 60_000),
          startIdentity: "another-process-generation",
        }),
      );
  ...
  it.live("quarantines and reclaims an abandoned ownerless lock", () => {
    const root = makeTempRoot();
    const archive = makeArchive(root);
    const layer = makeResolverLayer(root, archive, () => {});
