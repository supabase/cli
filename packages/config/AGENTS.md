...
`ProjectConfig` schema, config file loading/saving, and JSON Schema generation.
...
- `@supabase/config` (`.`) — pure, browser/edge-safe surface. The `ProjectConfigSchema` and
...
- `@supabase/config/effect` — the Effect-native superset. Re-exports everything from `.` plus the
  Effect-returning config-loading/saving programs, `ProjectConfigStore`/`projectConfigStoreLayer`,
  project-environment resolution, and functions-manifest inference.
- `@supabase/config/schema.json` — generated JSON Schema for `ProjectConfig` (a `dist/` build
  output).

## Monorepo import rule

- A file anywhere in this monorepo that needs **any** Effect-native symbol (the `ProjectConfigStore`
  service, `loadProjectConfig`/`saveProjectConfig`, project-environment resolution, functions-manifest
...
`project-config.layer.ts`, or `project-config.service.ts` — that would drag file-IO/Effect-platform
