...
// attribute is explicitly unset is unconfigured, never enabled. An unrecognized value maps
...
const smsProviderSelectionRows: ReadonlyArray<ProjectConfigMappingRow> = SMS_PROVIDERS.map(
  (provider) => ({
    ...
    // Absent/null/empty → omit all five (no provider named); a non-string throws like every other
    // mapped field.
    transform: (value, attributes) => {
