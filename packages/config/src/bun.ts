...
// Re-exports every pure symbol from `.` (types, schema, errors, etc.) so `./io`
// consumers can name them without a second import from `@supabase/config`;
// `index.ts`'s own graph is pure, so this doesn't drag in anything
// platform-specific.
