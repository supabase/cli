...
describe("project discovery and lazy env resolution", () => {
  ...
  test("leaves [api].auto_expose_new_tables unset by default and round-trips an explicit value", async () => {
    ...
    try {
      ...
      // Field is intentionally optional so the implicit default can flip alongside the
      // platform's own default without losing track of users who explicitly opted in
      // either direction.
