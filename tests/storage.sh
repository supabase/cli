...
if [[ $(echo "$output" | jq '.name') != 'test' ]]; then
...
output=$(curl -X DELETE 'http://127.0.0.1:54321/storage/v1/bucket/test' \
...
if [[ $(echo "$output" | jq '.message') != 'Successfully deleted' ]]; then
...
if [[ $(echo "$output" | jq '.error') != 'Unauthorized' ]]; then
