...
- Every develop push runs the e2e suite once more, about fifteen runner-minutes, to keep durations current.
- The unit/integration class is bounded by its single slowest file (a 228s legacy `start` integration test at the time of writing); no sequencer can shorten that, only the test can.
