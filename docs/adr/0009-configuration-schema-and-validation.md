...
`effect` is a peer dependency, not a regular one: `ProjectConfigStore`'s `Context.Tag` identity must be the same module instance a consumer's own Effect program resolves against, which a bundled or duplicated `effect` copy would silently break. `@effect/platform-bun`/`@effect/platform-node` are optional peers — only `./io` needs a concrete platform implementation, a given consumer only ever runs on one of Bun or Node, and requiring both unconditionally would be dead weight for whichever runtime isn't in use.
