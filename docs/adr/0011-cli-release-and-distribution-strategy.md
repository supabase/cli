...
| Artifact signing               | **Superseded by [ADR 0013](0013-macos-code-signing-and-notarization.md).** Originally "ship unsigned"; the contingent follow-up A trigger fired (macOS 26+ AMFI SIGKILLs the linker-signed Bun/Go binaries — CLI-1621), so macOS binaries are now ad-hoc signed in-pipeline      |
...
- **A. macOS notarization** — ⚠️ **TRIGGERED — see [ADR 0013](0013-macos-code-signing-and-notarization.md).** macOS 26+ AMFI SIGKILLs the linker-signed Bun/Go binaries (CLI-1621 / #5556). Resolved in two phases: full ad-hoc signing in-pipeline now (no Apple account needed), Developer ID + notarization deferred behind credential secrets. Original note: Apple Developer ID cert in CI + `codesign --sign --options runtime` + `notarytool submit --wait` (no stapling — bare Mach-O can't be stapled).
