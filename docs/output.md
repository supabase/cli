...
`stream-json` commands may emit `log-entry` events for line-oriented output. A log-entry event
has `type: "log-entry"`, `timestamp`, `service`, `stream`, `line`, and `source`; stack logs use
