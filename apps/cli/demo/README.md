...
`workers new` writing the first `[workers.<name>]` section. It reuses whatever
project this checkout is linked to; override with `SUPABASE_PROJECT_REF`.
...
- The recording deploys to a live project, so `workers list` also shows whatever
  else already exists there. Prune the project first if you want a clean table.
...
- The frame is sized around `workers --help`, whose global-flag table is laid out
  at a fixed ~173 columns and does not reflow.
