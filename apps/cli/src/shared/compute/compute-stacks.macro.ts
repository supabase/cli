 ...
 */
function assertCompleteComputeStacks(
  stacks: Record<string, ComputeStack>,
): asserts stacks is Record<ComputeRuntime, ComputeStack> {
  ...
  if (unexpected.length > 0) {
    throw new Error(
      `stacks/${unexpected.join(", stacks/")} has no matching entry in COMPUTE_RUNTIMES`,
    );
  }
 ...
 */
export function readComputeStacks(): Record<ComputeRuntime, ComputeStack> {
  ...
  try {
    assertCompleteComputeStacks(stacks);
  } catch (cause) {
    console.error(`[compute-stacks] ${String(cause)}`);
    throw cause;
  }
  return stacks;
}
