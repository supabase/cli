...
describe("packageComputeDirectory", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "supabase-compute-package-"));
  });
  ...
  const pack = (root: string) =>
    Effect.runPromise(packageComputeDirectory(root).pipe(Effect.provide(BunServices.layer)));
  ...
  test.each([
    ...
    const exit = await Effect.runPromise(
      packageComputeDirectory(dir).pipe(Effect.provide(BunServices.layer), Effect.exit),
    );
    ...
    expect(JSON.stringify(exit)).toContain("ComputeSourceEscapingLinkError");
  });
...
describe("packageComputeDirectory tar limits", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "supabase-compute-tar-"));
  });
