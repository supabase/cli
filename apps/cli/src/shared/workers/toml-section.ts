...
/**
 * Whether `value` is a whole, non-negative, finite number `[workers.<name>] instances` can
 ...
 * and the schema types `[workers.<name>] instances` as a number, so a quoted count would stop
 * `config.toml` from loading at all. Rendering doesn't validate: {@link isRenderableTomlNumber}
 * is the guard that keeps `1.5`/`-1` from reaching here, since `planWorkerEntry`'s re-parse is
