/**
 * Every worker gets a path on the project's own API host, like an Edge Function:
 * `<ref>.supabase.co/workers/v1/<name>` next to `.../functions/v1/<name>`. Nothing is
 * provisioned per-worker in DNS, so the URL is derived from the name rather than returned by the
 * API.
 */
