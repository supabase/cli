 ...
 *
 * Unlike `CommandPlatformApi`, whose layer resolves an access token when the
 * command runtime is built, `make` defers client construction until a command
 * branch actually reaches a Management API call.
 */
export interface CommandPlatformApiFactoryShape {
  readonly make: Effect.Effect<ApiClient, CommandPlatformApiFactoryError>;
}

export class CommandPlatformApiFactory extends Context.Service<
  CommandPlatformApiFactory,
  CommandPlatformApiFactoryShape
>()("supabase/cli/CommandPlatformApiFactory") {}
