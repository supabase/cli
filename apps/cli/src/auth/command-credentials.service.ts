...
interface CommandCredentialsShape {
   ...
   *      fails `DeleteTokenError`; a missing file is ignored.
   ...
   *        entry is absent → `NotLoggedInError`;
   *      - a real delete error → `DeleteTokenError`;
   *      - success → `void`.
   *
   * Deliberate quirk this preserves: on a no-keyring host the file is
   * still removed, yet the call fails `NotLoggedInError` because the
   ...
   * keyring is unavailable (WSL). Fails with `CredentialDeleteError` only
