...
import { commandCredentialsLayer } from "./command-credentials.layer.ts";
...
function makeLayer(
  ...
  const cliSettingsLayer = commandSettingsLayer.pipe(
    Layer.provide(debugLoggerLayer),
    Layer.provide(Layer.succeed(DebugFlag, opts.debug ?? false)),
    Layer.provide(Layer.succeed(ProfileFlag, "supabase")),
...
beforeEach(() => {
  ...
  tempHome = mkdtempSync(join(tmpdir(), "supabase-creds-"));
});
...
describe("commandCredentialsLayer.getAccessToken", () => {
  ...
  it.effect("fails with InvalidAccessTokenError when token format is invalid", () => {
  ...
  it.effect("rejects an unknown version prefix (sbp_v1_)", () =>
    Effect.gen(function* () {
      ...
      if (Option.isSome(errorOption)) {
        expect(errorOption.value).toBeInstanceOf(InvalidAccessTokenError);
      }
    }).pipe(
...
describe("commandCredentialsLayer.saveAccessToken", () => {
  it.effect("rejects invalid token formats up front", () =>
    Effect.gen(function* () {
      ...
      expect(exit._tag).toBe("Failure");
      if (exit._tag === "Failure") {
        expect(JSON.stringify(exit.cause)).toContain("InvalidAccessTokenError");
      }
    }).pipe(Effect.provide(makeLayer())),
  );
...
describe("commandCredentialsLayer.deleteAccessToken", () => {
  ...
  it.effect(
    "keyring profile entry absent → NotLoggedInError even though the file was removed",
    () => {
      seedTokenFile(tempHome);
      return Effect.gen(function* () {
        const { deleteAccessToken } = yield* CommandCredentials;
  ...
  it.effect("file remove error (non-ENOENT) → DeleteTokenError before touching keyring", () => {
    const home = tempHome;
    const env = { HOME: home };
    ...
    const layer = commandCredentialsLayer.pipe(
      Layer.provide(cliSettingsLayer),
      Layer.provide(debugLoggerLayer),
      Layer.provide(Layer.succeed(DebugFlag, false)),
      Layer.provide(runtimeInfoLayer),
  ...
  it.effect("real profile-keyring delete error → DeleteTokenError", () => {
    passwords.set("Supabase CLI/supabase", VALID_TOKEN);
...
describe("commandCredentialsLayer.deleteAllProjectCredentials", () => {
