...
describe("redactHttpUrl", () => {
  test.each([
    ...
    expect(redactHttpUrl(url)).toBe(expected);
  ...
  test.each([
    ...
    expect(redactHttpUrl(url)).toBe(url);
  });

  test("passes through something that is not a parseable URL", () => {
    expect(redactHttpUrl("not a url at all")).toBe("not a url at all");
  });

  test("keeps the path, which is what makes the log line worth having", () => {
    expect(redactHttpUrl("https://store.example/bucket/deep/ctx.tar.gz?sig=x")).toContain(
