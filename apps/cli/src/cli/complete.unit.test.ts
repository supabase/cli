...
import { StackRoutingError } from "../commands/stack/stack-backend.ts";
import {
  CompletionDirective,
...
describe("respondToComplete", () => {
  ...
  it("offers a global flag declared once at rootCommand from a nested command path", () => {
    // `--debug` is declared exactly once, via GLOBAL_FLAGS ->
    // Command.withGlobalFlags on rootCommand (command-internal/global-flags.ts,
    // cli/root.ts) — it must still resolve from a resolved subcommand path.
  ...
  it("offers a non-root command's own declared global flags from a nested subcommand (Command.withGlobalFlags)", () => {
    ...
    // Command.withGlobalFlags on the `seed` group itself, not at rootCommand
  ...
  describe("unmatched root-level command (CLI-1965 review)", () => {
    ...
    it("does not treat a surviving bare `-` leftover as an unmatched command (pflag's stripFlags drops it)", () => {
      ...
      // entirely, unlike resolveCommandPath's own leftoverArgs (which
    ...
    it("does not apply the unmatched-root check to a genuine `help ...` request", () => {
      ...
      // helpArgumentCandidates's doc comment), so the outer resolution
  ...
  describe("required-flag short-circuit", () => {
    ...
    it("short-circuits on a flag Go marks required even though this port made it optional at parse time", () => {
      ...
      // required" — `COMPLETION_REQUIRED_FLAGS` is the explicit table
      // that fixes this.
      const result = respondToComplete(rootCommand, [
        "__complete",
        "vanity-subdomains",
    ...
    it("does not treat a zero-minimum variadic flag (Flag.atLeast(0)) as required", () => {
      // sso/add/add.command.ts: `domains: ssoAddDomainsFlag` builds on
      // `stringSliceFlag`, which is `Flag.string(...).pipe(..., Flag.atLeast(0))`
      ...
      // `COMPLETION_REQUIRED_FLAGS` table, not structural inference, but
  ...
  describe("flag-value completion", () => {
    it.each([
      ...
      ({ command, flag, extension }) => {
        ...
        // (COMPLETION_FLAG_FILE_EXTENSIONS) rather than derived generically.
  ...
  describe("changed-flag exclusion and the variadic exception", () => {
    ...
    it("keeps offering a variadic flag's own name even after it has already been supplied", () => {
      // sso/add/add.command.ts: `domains: ssoAddDomainsFlag` is built on
...
describe("tryComplete", () => {
  function makeDeps(overrides: Partial<CompleteDeps> = {}) {
    ...
    const deps: CompleteDeps = {
      ...
      // production capture (`captureCompleteTelemetryEffect`,
      // `defaultCompleteDeps`'s own default) is covered separately in
