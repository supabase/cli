 ...
 *
 * This deliberately diverges from the general `promptYesNo`
 * (`command-internal/prompt-yes-no.ts`) on two axes — do NOT "unify" them
 ...
 *    auto-overwriting, down/repair-all auto-cancelling). `promptYesNo` instead
 *    silently takes the default in machine modes.
 *  - real TTY: this reads a raw stdin line with a 10-minute timeout;
 *    `promptYesNo` renders a clack confirm UI.
 *
