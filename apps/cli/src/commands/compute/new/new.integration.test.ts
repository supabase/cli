...
describe("compute new", () => {
  it.live("scaffolds the runtime's starter files and records the choice", () => {
    ...
    return Effect.gen(function* () {
      ...
      const computeDir = join(repo.dir, "supabase", "compute", "api");
      expect(existsSync(join(computeDir, "index.mjs"))).toBe(true);
  ...
  it.live("asks for the name when the command line carries none", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(existsSync(join(repo.dir, "supabase", "compute", "orders", "index.mjs"))).toBe(true);
      expect(repo.config()).toContain("[compute.orders]");
    }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
  });
  ...
  it.live.each([
    ...
    return Effect.gen(function* () {
      ...
      expect(existsSync(join(repo.dir, "supabase", "compute"))).toBe(false);
  ...
  it.live("prompts for runtime, size and exposure when none is given", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(existsSync(join(repo.dir, "supabase", "compute", "api", "index.mjs"))).toBe(true);
    }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
  });
  ...
  it.live("writes a count the config loader can read back", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(repo.config()).toContain("instances = 3");
      expect(repo.config()).toContain("[compute.web]");
    }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
  });
  ...
  it.live.each(['compute.api.runtime = "node"', "[compute.api]"])(
    ...
    (entry) => {
      ...
      return Effect.gen(function* () {
        ...
        expect(existsSync(join(repo.dir, "supabase", "compute", "api"))).toBe(false);
      }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
    },
  );
  ...
  it.live("scaffolds in a directory that has no Supabase project yet", () => {
    ...
    return Effect.gen(function* () {
      ...
      const computeDir = join(created.dir, "supabase", "compute", "api");
      expect(existsSync(join(computeDir, "index.mjs"))).toBe(true);
  ...
  it.live("refuses a destination that already has something in it", () => {
    const repo = project({ "supabase/compute/api/leftover.txt": "old" });
    const { layer } = setupCompute({ workdir: repo.dir });
    ...
    return Effect.gen(function* () {
      ...
      expect(existsSync(join(repo.dir, "supabase", "compute", "api", "leftover.txt"))).toBe(true);
    }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
  });
  ...
  it.live("scaffolds into a directory that exists but is empty", () => {
    const repo = project();
    mkdirSync(join(repo.dir, "supabase", "compute", "api"), { recursive: true });
    const { layer } = setupCompute({ workdir: repo.dir });
  ...
  it.live("rejects a name that could not become a hostname", () => {
    ...
    return Effect.gen(function* () {
      const error = yield* computeNew(flags({ name: Option.some("My_Compute") })).pipe(Effect.flip);
  ...
  it.live("records the compute in --workdir's own project, not an ancestor's", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(existsSync(join(workdir, "supabase", "compute", "api", "index.mjs"))).toBe(true);
    }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
  });
  ...
  it.live("refuses a plain file at the destination", () => {
    const repo = project({ "supabase/compute/api": "not a directory" });
    const { layer } = setupCompute({ workdir: repo.dir });
    ...
    return Effect.gen(function* () {
      ...
      expect(error).toBeInstanceOf(ComputeDirectoryExistsError);
      expect(readFileSync(join(repo.dir, "supabase", "compute", "api"), "utf8")).toBe(
        "not a directory",
      );
    }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
  });
  ...
  it.live(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        const exit = yield* computeNew(flags()).pipe(Effect.exit);

        expect(Exit.isFailure(exit)).toBe(true);
        const rendered = JSON.stringify(exit);
        expect(rendered).toContain("ComputeNewWorkdirError");
