 ...
 *
 * Pure, like `compute.format.ts` beside it: no Effect, no services, so the line
 * shapes and the level derivation are unit-testable directly.
 *
 * Kept apart from `compute.format.ts` because that file renders *resources* - a
 * compute's details as a key/value block - while this renders a stream of events,
 ...
 * the one untrusted string this CLI displays: left alone, a compute could emit
 ...
 * would cost 19 columns to say "the compute's own output". The words match
...
export function computeLogText(entry: ComputeLogEntry): string {
  ...
  // called the compute, and a build `reason` is relayed from the builder, so
