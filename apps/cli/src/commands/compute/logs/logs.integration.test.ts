...
function flags(overrides: Record<string, unknown> = {}) {
  return {
    ...
    tail: 100,
    ...overrides,
  } as Parameters<typeof computeLogs>[0];
}
  ...
  it.live("skips history for --tail 0 but still follows", () => {
    ...
    const { layer, out, http } = setupCompute({
      ...
      routes: {
        ...
        [LOGS_ROUTE]: logsResponse([
          computeLogRow({ id: "new", tsMs: Date.now() + 5_000, message: "brand new" }),
        ]),
      },
    });
  ...
  it.live("does not replay pre-invocation lines for --tail 0 --follow", () => {
    ...
    const { layer, out } = setupCompute({
      ...
      routes: {
        ...
        [LOGS_ROUTE]: [
          logsResponse([
            computeLogRow({ id: "before", tsMs: Date.now() - 30_000, message: "written before" }),
            computeLogRow({ id: "after", tsMs: Date.now() + 5_000, message: "written after" }),
          ]),
        ],
      },
    });
