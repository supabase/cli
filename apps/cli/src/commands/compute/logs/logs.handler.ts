 ...
 * cover the two questions a deployed compute raises, and neither answers the
 * other's.
 *
 * Unlike the rest of the family this does not talk to `/v2/.../workers` — there is
 * no compute-scoped log route — but to the project's unified logs stream. See
 ...
 * reasoning as `awaitComputeBuild`'s read retry.
 */
...
export const computeLogs = Effect.fn("compute.logs")(function* (
  ...
  yield* Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      // query and the deployed-compute check. A poll draining a burst spends up to
