...
> may well serve — but a meaningful assertion needs a deployed compute that has
...
Compute private-alpha allow-list — an unenrolled project answers 404.
...
6 a minute, leaving room for the history query, the deployed-compute check, and a
...
`emitComputeMachineOutput` or at `output.success`, so the no-logs line and
...
escape sequences are stripped before it reaches a terminal, so a compute cannot
