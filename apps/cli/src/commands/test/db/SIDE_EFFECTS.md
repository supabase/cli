...
- `--network supabase_network_<project_id>` (Compose `--local`) with env `PGHOST=db PGPORT=5432`, or `--network host` (db-url / linked, and every stack-backend prove) with the published host/port. Stack `--local` never uses `PGHOST=db`. `<project_id>` is sanitized (`sanitizeProjectId`), so an invalid configured value (e.g. `"my project"`) joins the same network the local stack created
