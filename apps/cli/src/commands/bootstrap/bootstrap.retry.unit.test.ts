...
// Drives the schedule's step function directly (no real sleeping): each call returns the next
// delay, and an artificial `now` advances by that delay so the max-elapsed gate sees virtual
// time pass.
