...
| `1`  | resolved `--workdir`/`SUPABASE_WORKDIR` doesn't exist or isn't a directory (`LegacyGenTypesWorkdirError`) — beats every other guard                                                                         |
| `1`  | an explicit `--workdir`/`SUPABASE_WORKDIR` holds no project config on a schema-selecting path (`LegacyGenTypesMissingProjectConfigError`) — a DEFAULTED workdir keeps the embedded-default fallback instead |
