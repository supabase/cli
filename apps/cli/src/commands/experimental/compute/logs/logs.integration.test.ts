...
const LOGS_ROUTE = `GET ${computeLogsRoute()}`;
const GET_COMPUTE_ROUTE = `GET ${computeRoute("/api")}`;
...
function flags(overrides: Partial<Parameters<typeof computeLogs>[0]> = {}) {
  return {
    name: "api",
...
describe("compute logs", () => {
      ...
      const { layer, out } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: logsResponse([
            computeLogRow({ id: "c", tsMs: T3, message: "app drained" }),
            computeLogRow({ id: "a", tsMs: T1, message: "listening on :8080" }),
            computeLogRow({ id: "b", tsMs: T2, message: "terminate hook" }),
          ]),
        },
      });

      return yield* Effect.gen(function* () {
        yield* computeLogs(flags());
  ...
  it.live("prints a build event with its reason", () =>
    Effect.gen(function* () {
      ...
      const { layer, out } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: logsResponse([
            computeApiLogRow({ tsMs: T1, event: "build_failed", reason: "exit status 1" }),
          ]),
        },
      });
  ...
  it.live("always sends both timestamp bounds, under a 24 hour span", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        expect(
          DateTime.toEpochMillis(DateTime.makeUnsafe(end!)) -
  ...
  it.live("narrows to one stream for --kind, and to all three without it", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ kind: Option.some("requests") }));
  ...
  it.live("renders a stream it has never heard of rather than failing", () =>
    Effect.gen(function* () {
      ...
      const { layer, out } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: logsResponse([
            computeLogRow({ tsMs: T1, stream: "compute_future_logs", message: "from the future" }),
          ]),
        },
      });
  ...
  it.live("strips escape sequences a compute printed", () =>
    Effect.gen(function* () {
      ...
      const { layer, out } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: logsResponse([
            computeLogRow({ tsMs: T1, message: `${ESCAPE}[31mERROR: not really${ESCAPE}[0m` }),
          ]),
        },
      });
  ...
  it.live("keeps a blank guest line as a line", () =>
    Effect.gen(function* () {
      ...
      const { layer, out } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: logsResponse([
            computeLogRow({ id: "a", tsMs: T1, message: "before" }),
            computeLogRow({ id: "b", tsMs: T2, message: "" }),
            computeLogRow({ id: "c", tsMs: T3, message: "after" }),
          ]),
        },
      });
  ...
  it.live("makes no request at all for --tail 0", () =>
    Effect.gen(function* () {
      ...
      const { layer, http } = setupCompute({
        workdir: repo.dir,
        routes: {
          [GET_COMPUTE_ROUTE]: { status: 200, body: { data: computeResource({ name: "api" }) } },
        },
      });

      return yield* Effect.gen(function* () {
        // `limit 0` would be a 400, so no-history has to mean no query.
        yield* computeLogs(flags({ tail: 0 }));

        expect(http.routeKeys).not.toContain(computeLogsRoute());
      }).pipe(Effect.provide(layer));
    }).pipe(Effect.scoped, Effect.provide(BunServices.layer)),
  );
  ...
  it.live("passes --tail through as the row limit", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ tail: 7 }));
  ...
  it.live("fails on a 200 that carries a query error", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        expect(error).toBeInstanceOf(ComputeLogsQueryFailedError);
        const detail = error instanceof ComputeLogsQueryFailedError ? error.detail : "";
  ...
  it.live("maps 402 to a usage error and 429 to a rate limit error", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        const usage = yield* computeLogs(flags()).pipe(Effect.flip);
        const limited = yield* computeLogs(flags()).pipe(Effect.flip);

        expect(usage).toBeInstanceOf(ComputeLogsUsageExceededError);
        expect(limited).toBeInstanceOf(ComputeLogsRateLimitedError);
      }).pipe(Effect.provide(layer));
    }).pipe(Effect.scoped, Effect.provide(BunServices.layer)),
  );
  ...
  it.live("reports a transport failure", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        expect(error).toBeInstanceOf(ComputeApiNetworkError);
      }).pipe(Effect.provide(layer));
    }).pipe(Effect.scoped, Effect.provide(BunServices.layer)),
  );
  ...
  it.live("rejects an impossible compute name before any request", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        const error = yield* computeLogs(flags({ name: "Not A Name" })).pipe(Effect.flip);
  ...
  it.live("emits only the payload on stdout for -o json", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        ...
        expect(payload.compute_name).toBe("api");
        expect(payload.logs[0]?.level).toBe("error");
        // Both timestamp forms, and the raw attributes.
        expect(payload.logs[0]?.timestamp).toBe("2026-08-31T14:45:25.212Z");
        expect(payload.logs[0]?.timestamp_ms).toBe(T1);
        const attributes = payload.logs[0]?.attributes;
  ...
  it.live("flushes telemetry even when the query fails", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        yield* computeLogs(flags()).pipe(Effect.ignore);
  ...
  it.live("flushes telemetry when the project ref cannot be resolved", () =>
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        const exit = yield* computeLogs(flags()).pipe(Effect.exit);
  ...
  it.live("uses the project ref from the flag and echoes it in suggestions", () =>
    Effect.gen(function* () {
      ...
      const { layer } = setupCompute({
        ...
        linked: false,
        routes: {
          [`GET /v1/projects/${COMPUTE_PROJECT_REF}/analytics/endpoints/logs`]: logsResponse([]),
          [GET_COMPUTE_ROUTE]: { status: 404 },
        },
      });

      return yield* Effect.gen(function* () {
        const error = yield* computeLogs(
          flags({ projectRef: Option.some(COMPUTE_PROJECT_REF) }),
        ).pipe(Effect.flip);

        // A copy-pasted suggestion must not silently re-resolve to whatever this
        // checkout happens to be linked to.
        const suggestion = error instanceof ComputeNotDeployedError ? error.suggestion : "";
        expect(suggestion).toContain(`--project-ref ${COMPUTE_PROJECT_REF}`);
      }).pipe(Effect.provide(layer));
    }).pipe(Effect.scoped, Effect.provide(BunServices.layer)),
  );
      ...
      const { layer, out, http } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: [
            ...
            logsResponse([computeLogRow({ id: "c", tsMs: T3, message: "third" })]),
          ],
        },
      });

      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ follow: true }), followFor(2));
        ...
        for (const request of http.requests) {
          ...
          expect(
            DateTime.toEpochMillis(DateTime.makeUnsafe(query.iso_timestamp_end!)) -
              DateTime.toEpochMillis(DateTime.makeUnsafe(query.iso_timestamp_start!)),
          ).toBeLessThan(24 * 60 * 60 * 1000);
        }
      }).pipe(Effect.provide(layer));
    }).pipe(Effect.scoped, Effect.provide(BunServices.layer)),
  ...
  it.live("does not reprint a line an overlapping window returns again", () =>
    Effect.gen(function* () {
      ...
      const { layer, out } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: [
            logsResponse([computeLogRow({ id: "a", tsMs: T1, message: "only once" })]),
            // The cursor lags deliberately, so the same row comes back.
            logsResponse([
              computeLogRow({ id: "a", tsMs: T1, message: "only once" }),
              computeLogRow({ id: "b", tsMs: T2, message: "and this" }),
            ]),
          ],
        },
      });
      ...
      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ follow: true }), followFor(1));
  ...
  it.live("still emits a line that arrived late, inside the grace window", () =>
    Effect.gen(function* () {
      ...
      const { layer, out } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: [
            logsResponse([computeLogRow({ id: "a", tsMs: T3, message: "newest first" })]),
            // Older than the cursor, which is why the cursor lags at all.
            logsResponse([computeLogRow({ id: "late", tsMs: T1, message: "arrived late" })]),
          ],
        },
      });
  ...
  it.effect("skips history for --tail 0 but still follows", () =>
    Effect.gen(function* () {
      const repo = yield* project();
      const { layer, out, http } = setupCompute({
        workdir: repo.dir,
        routes: {
          // A tail-zero follow still asks whether the compute exists: with no history
          ...
          [LOGS_ROUTE]: logsResponse([
            computeLogRow({
              id: "new",
              tsMs: (yield* Clock.currentTimeMillis) + 5_000,
              message: "brand new",
            }),
          ]),
        },
      ...
      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ follow: true, tail: 0 }), followFor(1));
  ...
  it.live("emits a log-entry event per line under stream-json", () =>
    Effect.gen(function* () {
      ...
      const { layer, out } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: [
            logsResponse([computeIngressLogRow({ id: "a", tsMs: T1, status: "500" })]),
            logsResponse([computeLogRow({ id: "b", tsMs: T2, message: "app line" })]),
          ],
        },
      });
    ...
    Effect.gen(function* () {
      ...
      return yield* Effect.gen(function* () {
        for (const setup of [
          ...
          expect(error).toBeInstanceOf(ComputeFollowNotSupportedError);
  ...
  it.effect("does not replay pre-invocation lines for --tail 0 --follow", () =>
    Effect.gen(function* () {
      const repo = yield* project();
      const { layer, out } = setupCompute({
        workdir: repo.dir,
        routes: {
          ...
          [LOGS_ROUTE]: [
            logsResponse([
              computeLogRow({
                id: "before",
                tsMs: (yield* Clock.currentTimeMillis) - 30_000,
                message: "written before",
              }),
              computeLogRow({
                id: "after",
                tsMs: (yield* Clock.currentTimeMillis) + 5_000,
                message: "written after",
              }),
            ]),
          ],
        },
      ...
      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ tail: 0, follow: true }), followFor(0));
  ...
  it.live("still checks the compute exists for --tail 0 --follow", () =>
    Effect.gen(function* () {
      ...
      const { layer, out } = setupCompute({
        workdir: repo.dir,
        routes: { [GET_COMPUTE_ROUTE]: { status: 404 } },
      });

      return yield* Effect.gen(function* () {
        const error = yield* computeLogs(flags({ tail: 0, follow: true }), followFor(0)).pipe(
          Effect.flip,
        );
        ...
        expect(out.progressEvents).toContainEqual(
          expect.objectContaining({ type: "start", message: "Checking compute..." }),
        );
      }).pipe(Effect.provide(layer));
    }).pipe(Effect.scoped, Effect.provide(BunServices.layer)),
  );
  ...
  it.live("fails typed rather than throwing on an out-of-range timestamp", () =>
    Effect.gen(function* () {
      ...
      const { layer } = setupCompute({
        workdir: repo.dir,
        routes: {
          [LOGS_ROUTE]: logsResponse([computeLogRow({ id: "bad", tsMs: 8.7e15 })]),
        },
      });
  ...
  it.live("surfaces a definitive poll failure without retrying it", () =>
    Effect.gen(function* () {
      ...
      const { layer, http } = setupCompute({
        workdir: repo.dir,
        routes: {
          [LOGS_ROUTE]: [logsResponse([computeLogRow({ id: "a", tsMs: T1 })]), { status: 404 }],
        },
      });

      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ follow: true }), {
  ...
  it.live("retries a rate-limited poll", () =>
    Effect.gen(function* () {
      ...
      const { layer, http } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: [
            logsResponse([computeLogRow({ id: "a", tsMs: T1 })]),
            { status: 429 },
            logsResponse([computeLogRow({ id: "b", tsMs: T2, message: "after the limit" })]),
          ],
        },
      });
    ...
    Effect.gen(function* () {
      ...
      const { layer, http } = setupCompute({
        ...
        routes: {
          [LOGS_ROUTE]: [
            logsResponse([computeLogRow({ id: "a", tsMs: T1 })]),
            logsResponse([computeLogRow({ id: "b", tsMs: T2 })]),
          ],
        },
      });

      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ tail: 1, follow: true }), followFor(0));
  ...
  it.live("drains a burst larger than one page before advancing the cursor", () =>
    Effect.gen(function* () {
      ...
      const fullPage = Array.from({ length: 1000 }, (_, index) =>
        computeLogRow({ id: `burst-${index}`, tsMs: T2 + index, message: `burst ${index}` }),
      );
      const { layer, out, http } = setupCompute({
        workdir: repo.dir,
        routes: {
          [LOGS_ROUTE]: [
            ...
            // deployed-compute check that an empty history triggers.
            logsResponse([computeLogRow({ id: "seed", tsMs: T1 - 100_000, message: "seed" })]),
            logsResponse(fullPage),
            logsResponse([computeLogRow({ id: "straggler", tsMs: T1, message: "older line" })]),
          ],
        },
      });

      return yield* Effect.gen(function* () {
        yield* computeLogs(flags({ follow: true }), followFor(0));
        ...
        expect(out.stdoutText.indexOf("older line")).toBeLessThan(
          out.stdoutText.indexOf("burst 0"),
        );
  ...
  it.live("records exit 130 on SIGINT and still runs its finalizers", () =>
    Effect.gen(function* () {
      ...
      const { layer, processControl, telemetry } = setupCompute({
        workdir: repo.dir,
        signal: "SIGINT",
        routes: { [LOGS_ROUTE]: logsResponse([computeLogRow({ id: "a", tsMs: T1 })]) },
      });
