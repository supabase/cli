...
machine-readable invocations. `SUPABASE_YES` participates in the existing confirmation setting;
...
reports the destroyed stack ID. JSON and stream-JSON return
`{ "destroyed": true, "id": "..." }`. Success exits `0`; invalid targets, confirmation refusal,
and destruction failures exit `1`; interruption follows the command runtime's interruption exit.
