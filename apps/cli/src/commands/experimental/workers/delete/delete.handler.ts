      ...
      if (lookup.readable && deployed === undefined && !yes) {
        return yield* Effect.fail(
          workerNotDeployed({
            name,
            projectRef,
            suggestion: `See what is deployed with \`${workersCommandLine(`list${refSuffix}`)}\`.`,
          }),
        );
      }
        ...
        if (!rendersText || !output.interactive || !tty.stdinIsTty) {
          return yield* Effect.fail(
            new WorkerDeleteConfirmationRequiredError({
              ...
              suggestion: `Re-run \`${workersCommandLine(`delete ${name} --yes${refSuffix}`)}\` to confirm without a prompt.`,
            }),
          );
        }
        ...
        if (typed.trim() !== name) {
          return yield* Effect.fail(
            new WorkerDeleteNotConfirmedError({
              ...
              suggestion: `Re-run \`${workersCommandLine(`delete ${name}${refSuffix}`)}\` and type the name exactly, or pass --yes.`,
            }),
          );
        }
      }
        ...
        if (kept.length > 0) {
          ...
          if (keptSource !== undefined) {
            ...
            yield* emitSuccessTrailer(
              `Redeploy it with ${aqua(workersPushCommandLine(name, refSuffix))}.\n`,
            );
          }
        } else {
          yield* output.raw(
