...
export const workersCommandLine = (rest: string) => `${PATH} ${rest}`;
 ...
 */
export const workersPushCommandLine = (name: string, refSuffix = "") =>
  workersCommandLine(`push ${name}${refSuffix}`);

/** The `status` that reports on `name`. */
export const workersStatusCommandLine = (name: string, refSuffix = "") =>
  workersCommandLine(`status ${name}${refSuffix}`);
 ...
 */
export const workerNotDeployed = (options: {
