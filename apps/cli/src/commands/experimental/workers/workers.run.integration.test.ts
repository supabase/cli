...
import { workersDelete } from "./delete/delete.handler.ts";
import { workersList } from "./list/list.handler.ts";
import { workersLogs } from "./logs/logs.handler.ts";
import { workersPush } from "./push/push.handler.ts";
import { workersStatus } from "./status/status.handler.ts";
...
const COMMANDS = [
  ["list", () => workersList(REF)],
  ["status", () => workersStatus({ ...REF, name: "api" })],
  ["delete", () => workersDelete({ ...REF, name: "api" })],
  [
    "push",
    () =>
      workersPush({
  ...
  [
    "logs",
    () => workersLogs({ ...REF, name: "api", kind: Option.none(), follow: false, tail: 100 }),
  ],
] as const;

/**
 * The ordering `workersRun` exists to hold.
 *
