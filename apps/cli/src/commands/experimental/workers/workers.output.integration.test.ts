 ...
 */
describe("emitWorkersPayload", () => {
  const PAYLOAD = { project_ref: "demo", workers: [] };

  function setup(options: Parameters<typeof setupWorkers>[0]) {
    const created = makeWorkersProject({ "supabase/config.toml": `project_id = "demo"\n` });
    const it = setupWorkers({ ...options, workdir: created.dir, routes: {} });
    return {
      ...it,
  ...
  const structured = (out: ReturnType<typeof setupWorkers>["out"]) =>
  ...
  it.live("hands a plain text run back to its caller", () => {
    ...
    return Effect.gen(function* () {
      expect(yield* emitWorkersPayload(PAYLOAD)).toBe(false);
  ...
  it.live("emits one structured result for --output-format json", () => {
    ...
    return Effect.gen(function* () {
      expect(yield* emitWorkersPayload(PAYLOAD)).toBe(true);
...
describe("workersRendersText", () => {
  ...
  for (const [label, options, expected] of CASES) {
    it.live(`is ${expected} for ${label}`, () => {
      const created = makeWorkersProject({ "supabase/config.toml": `project_id = "demo"\n` });
      const { layer } = setupWorkers({ ...options, workdir: created.dir, routes: {} });

      return Effect.gen(function* () {
        expect(yield* workersRendersText()).toBe(expected);
      }).pipe(
        Effect.provide(layer),
        Effect.ensuring(Effect.sync(() => rmSync(created.dir, { recursive: true, force: true }))),
      );
