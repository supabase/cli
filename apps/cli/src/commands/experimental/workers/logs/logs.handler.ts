 ...
 * part of that region inside the next window's grace is picked up again.
 * `followWindow` moves the window's floor, not its ceiling, so lowering the
 * cursor just re-fetches the same newest pages and never walks down to the gap.
 * A burst above {@link FOLLOW_PAGE_SIZE} × this bound in one poll interval
 * therefore loses its middle, and says so on stderr.
 */
...
const drainSince = Effect.fnUntraced(function* (input: {
  readonly api: CommandPlatformApi["Service"];
  ...
  let end = new Date();
  // A short page is the only proof the window is empty below this point. Both
  // other exits — the page budget running out, and a full page too narrow to
  // walk past — leave rows unfetched underneath.
  let drained = false;
...
export const workersLogs = Effect.fn("experimental.workers.logs")(function* (
  ...
  yield* workersRun(flags.projectRef, ({ projectRef, refSuffix }) =>
    Effect.gen(function* () {
      ...
      if (!flags.follow && (yield* emitWorkersPayload(payload))) {
        return;
      }
      ...
      if (entries.length === 0 && !flags.follow) {
        ...
        yield* emitSuccessTrailer(
          `Check it is running with ${aqua(workersStatusCommandLine(name, refSuffix))}.\n`,
        );
        return;
      }
      ...
      // Same reason this lives in the generator rather than the closure: an
      ...
      const pollOnce = Effect.gen(function* () {
        ...
        const { rows, drained } = yield* drainSince({
          api,
          projectRef,
          name,
          streams,
