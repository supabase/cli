...
test("diffs a freshly-initialized config against the project", ({ cliEffect, project, signal }) =>
  Effect.runPromise(
    Effect.gen(function* () {
      ...
      const payload = yield* Schema.decodeEffect(DiffPayload)(result.stdout);
      expect(payload.scope.present).toContain("auth");
      const changes = payload.changes.map(({ path, class: kind }) => ({
        path: path.join("."),
        kind,
      }));
