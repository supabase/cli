...
| `<workdir>/supabase/<api.tls.cert_path>`      | PEM text    | Legacy local TLS CA when configured; stack-backed runs also read the configured CA when the observed Storage endpoint is HTTPS. When neither custom TLS path is configured, the embedded default is used. HTTP stack endpoints skip CA resolution.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `<workdir>/supabase/<api.tls.key_path>`       | PEM text    | Legacy local TLS validation; stack-backed runs also read the key when the observed Storage endpoint is HTTPS and a custom CA pair is configured. HTTP stack endpoints skip CA resolution.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `<workdir>/supabase/.env*`, `<workdir>/.env*` | dotenv      | once per run, unless the caller already resolved the map (`db reset --local`, the legacy `start` command, and `stack start`'s first-create seeding all pass their own `resolvedConfig`/`projectEnvValues` through): resolves `SUPABASE_YES` for the overwrite/prune prompts on either target (CLI-1878) and, on local legacy-backend runs, the `SUPABASE_API_*` overrides for gateway URL/TLS (#6452) and `SUPABASE_AUTH_{JWT_SECRET,SERVICE_ROLE_KEY}` overrides for the service-role key; on stack runs, API TLS settings are read only when configured buckets use an observed HTTPS Storage endpoint                                                                                               |
...
| `SUPABASE_API_PORT`, `SUPABASE_API_EXTERNAL_URL`, `SUPABASE_API_TLS_ENABLED`, `SUPABASE_API_TLS_CERT_PATH`, `SUPABASE_API_TLS_KEY_PATH`, `SUPABASE_API_ENABLED` | local runs: legacy mode uses matching `[api]` fields for gateway URL/TLS; stack mode uses TLS settings to trust an observed HTTPS Storage endpoint. HTTP stack endpoints skip TLS CA resolution (shell env or project dotenv; shell wins)                                                        | no                                   |
...
| `1`  | `api.tls.cert_path` or `api.tls.key_path` points to an unreadable file (legacy local TLS, or stack local HTTPS endpoint with custom TLS paths)                                                                                                                                                                                |
...
- **Stack backend (local).** The Storage endpoint comes from the selected composition's
  ...
  service member. Legacy `[api]`/`[api.tls]`/`SUPABASE_API_*` and auth override validation is
  skipped on this path, including when there is nothing configured to seed. For configured
  buckets, an observed HTTPS Storage endpoint uses the configured API TLS CA, or the embedded
  default when no custom CA is configured; HTTP endpoints do not resolve CA files.
...
- **Local TLS CA.** On the legacy backend, `[api.tls] enabled = true` reads the configured
  certificate and key for validation; `cert_path` supplies the CA PEM. On the stack backend,
  configured buckets resolve a CA only when the observed Storage endpoint is HTTPS: the configured
  certificate is used when present, or the embedded `kong.local.crt` when neither custom TLS path
  is configured. HTTP endpoints skip CA resolution. Paths resolve against `<workdir>/supabase/`.
  The CA is injected into Bun fetch without changing system trust. Stack CA
  trust follows the current config files, so changing or removing a file while the stack is
  running can cause requests to fail until the files are restored or the stack is restarted.
