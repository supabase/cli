...
| Path                                          | Format      | When                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `<workdir>/supabase/config.toml`              | TOML        | always, to read `[storage.buckets]` / `[storage.vector]` config; on `--linked`, the matching `[remotes.<name>]` block (whose `project_id` equals the resolved project ref) is merged over the base config before decode, so remote-specific storage config takes effect. With an explicit `--workdir`/`SUPABASE_WORKDIR` that holds no project config, the standalone `seed buckets` command now fails (`LegacySeedMissingProjectConfigError`) instead of authenticating and seeding nothing while still exiting `0`; a DEFAULTED workdir keeps the embedded-default fallback, and `start`/`db reset`'s reuse of this seeding core (which supplies its own already-resolved config) is unaffected either way |
...
| `<workdir>/supabase/.env*`, `<workdir>/.env*` | dotenv      | once per run, unless the caller already resolved the map (`db reset --local` and `start` pass theirs through): resolves `SUPABASE_YES` for the overwrite/prune prompts on either target (CLI-1878) and, on local runs, the `SUPABASE_API_*` overrides for the gateway URL and TLS material (#6452) plus the `SUPABASE_AUTH_{JWT_SECRET,SERVICE_ROLE_KEY}` overrides for the service-role key                                                                                                                                                                                                                                                                                                                 |
...
| `SUPABASE_AUTH_SERVICE_ROLE_KEY`                                                                                                                                | when set and non-empty: for `--linked`, used as the service-role key (skips Management API key fetch; shell env only); for local runs, used as the service-role key instead of `auth.service_role_key` — shell env OR project dotenv (shell wins), `encrypted:` values decrypted | no        |
| `SUPABASE_AUTH_JWT_SECRET`                                                                                                                                      | local runs only: when set and non-empty, overrides `auth.jwt_secret` for service-role key derivation — shell env OR project dotenv (shell wins), `encrypted:` values decrypted                                                                                                   | no        |
...
| Code | Condition                                                                                                                                                                                                                                                                                                             |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `0`  | success (including the empty-config short-circuit, provided the local config checks pass: `[api]` override decode, non-zero enabled port, TLS cert/key pairing, `[auth]` override decrypt and `jwt_secret` length)                                                                                                    |
| `1`  | resolved `--workdir`/`SUPABASE_WORKDIR` doesn't exist or isn't a directory (`LegacySeedWorkdirError`) — beats the `--project-ref` guard and every network call                                                                                                                                                        |
| `1`  | an explicit `--workdir`/`SUPABASE_WORKDIR` holds no project config (`LegacySeedMissingProjectConfigError`) — precedes the api-keys fetch and any Storage call                                                                                                                                                         |
| `1`  | `supabase/config.toml` parse failure                                                                                                                                                                                                                                                                                  |
| `1`  | `auth.jwt_secret` / `auth.service_role_key` (or their env overrides) set to an undecryptable `encrypted:` value — local only, enforced even when nothing is configured to seed                                                                                                                                        |
| `1`  | `auth.jwt_secret` (or `SUPABASE_AUTH_JWT_SECRET`) set but shorter than 16 characters — local only, enforced even when nothing is configured to seed                                                                                                                                                                   |
...
| `1`  | malformed `SUPABASE_API_PORT` / `SUPABASE_API_ENABLED` / `SUPABASE_API_TLS_ENABLED` override, a resolved `api.port` of `0` with the API enabled, or an unreadable/malformed project dotenv file — enforced on the local target even when nothing is configured to seed, including the `api.tls` cert/key pairing rule |
...
- **Content-Type** for uploaded objects: the first
  ...
  (`command-internal/legacy-detect-content-type.ts`), and only a generic `text/plain`
