...
const setupSeedBuckets = Effect.fnUntraced(function* (
  workdir: string,
  opts: {
...
describe("seed buckets", () => {
  ...
  // every test here (the ambient-override tests set their own inside this pin).
  ...
  const seedScenario = <A, E>(
    body: Effect.Effect<A, E, BunServices.BunServices>,
  ): Effect.Effect<A, E> =>
    OVERRIDE_ENV_KEYS.reduce<Effect.Effect<A, E>>(
      (acc, key) => withEnvVar(key, undefined, acc),
  ...
  it.live("sets the object Content-Type from the file bytes, not the extension", () =>
    Effect.gen(function* () {
      ...
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      yield* fs.makeDirectory(path.join(tmp.current, "supabase", "assets"), { recursive: true });
      // Real PNG magic bytes — written raw (a UTF-8 string would mangle 0x89).
      yield* fs.writeFile(
        path.join(tmp.current, "supabase", "assets", "logo.txt"),
        new Uint8Array([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0x00]),
      );
      yield* fs.writeFileString(
        path.join(tmp.current, "supabase", "assets", "data.json"),
        '{"a":1}',
      );
      const { layer, requests } = yield* setupSeedBuckets(tmp.current, {
        toml: '[storage.buckets.images]\npublic = true\nobjects_path = "./assets"\n',
  ...
  it.live("resolves an absolute objects_path as-is (Go IsAbs guard)", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const absRoot = path.join(tmp.current, "external-assets");
      yield* fs.makeDirectory(absRoot, { recursive: true });
      yield* fs.writeFileString(path.join(absRoot, "a.txt"), "hello");
      const { layer, out, requests } = yield* setupSeedBuckets(tmp.current, {
      ...
      const exit = yield* seedBuckets(DEFAULT_FLAGS).pipe(Effect.provide(layer), Effect.exit);
      expect(Exit.isSuccess(exit)).toBe(true);
      expect(out.stderrText).toContain(`Uploading: ${path.join(absRoot, "a.txt")} => images/a.txt`);
      const uploads = requests.filter((r) => r.url.includes("/storage/v1/object/"));
  ...
  it.live("fails on an invalid bucket file_size_limit before any Storage call", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        expect(Cause.pretty(exit.cause)).toContain("invalid size");
      }
      expect(requests).toHaveLength(0);
    }).pipe(seedScenario),
  );
    ...
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(causeText).toContain(
          "Another process may be listening on the configured API port 7654",
        );
        expect(causeText).toContain("lsof -nP -iTCP:7654 -sTCP:LISTEN");
      }
    }).pipe(seedScenario),
  );
  ...
  it.live("omits the port-conflict hint on a connection-refused local failure", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        expect(Cause.pretty(exit.cause)).not.toContain("Another process may be listening");
      }
    }).pipe(seedScenario),
  );
  ...
  it.live("reports the external_url port (not api.port) in the local hint", () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        const causeText = Cause.pretty(exit.cause);
        expect(causeText).toContain("configured API port 9999");
        expect(causeText).not.toContain("port 7654");
      }
    }).pipe(seedScenario),
  );
  ...
  it.live("fails when a bucket create returns a non-object body (Go ParseJSON)", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        expect(Cause.pretty(exit.cause)).toContain("failed to parse response body");
      }
    }).pipe(seedScenario),
  );
  ...
  it.live("rejects SUPABASE_API_PORT=0 with the canonical missing-field error", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        expect(Cause.pretty(exit.cause)).toContain("Missing required field in config: api.port");
      }
      expect(requests).toHaveLength(0);
    }).pipe(seedScenario),
  );
  ...
  it.live("treats a non-200 2xx gateway response as an error (Go expects exactly 200)", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        expect(Cause.pretty(exit.cause)).toContain("Error status 201");
      }
    }).pipe(seedScenario),
  );
      ...
      withEnvVar(
        ...
        withEnvVar(
          ...
          Effect.gen(function* () {
            ...
            expect(requests.every((r) => r.url.startsWith("http://docker.internal:54321"))).toBe(
              true,
            );
          }),
        ),
  ...
  it.live("skips non-regular files during the object walk", () =>
    Effect.gen(function* () {
      ...
      const mkfifo = yield* spawner.exitCode(
        ChildProcess.make("mkfifo", [path.join(tmp.current, "supabase", "assets", "pipe")]),
      );
      expect(mkfifo).toBe(0);
      const { layer, out, requests } = yield* setupSeedBuckets(tmp.current, {
        toml: '[storage.buckets.images]\npublic = true\nobjects_path = "./assets"\n',
        routes: [
          { method: "GET", match: "/storage/v1/bucket", body: [] },
  ...
  it.live("skips a dangling symlink without failing (Go isUploadableEntry parity)", () =>
    Effect.gen(function* () {
      ...
      yield* fs.symlink(
        "./does-not-exist",
        path.join(tmp.current, "supabase", "assets", "dangling"),
      );
      const { layer, out, requests } = yield* setupSeedBuckets(tmp.current, {
        toml: '[storage.buckets.images]\npublic = true\nobjects_path = "./assets"\n',
        routes: [
  ...
  it.live("skips OS metadata files during the object walk (CLI-1950)", () =>
    Effect.gen(function* () {
      ...
      yield* fs.makeDirectory(path.join(tmp.current, "supabase", "assets"), { recursive: true });
      yield* fs.writeFileString(path.join(tmp.current, "supabase", "assets", "a.txt"), "hello");
      yield* fs.writeFileString(path.join(tmp.current, "supabase", "assets", ".DS_Store"), "junk");
      yield* fs.writeFileString(path.join(tmp.current, "supabase", "assets", "Thumbs.db"), "junk");
      yield* fs.writeFileString(
        path.join(tmp.current, "supabase", "assets", "desktop.ini"),
        "junk",
      );
      const { layer, out, requests } = yield* setupSeedBuckets(tmp.current, {
        toml: '[storage.buckets.images]\npublic = true\nobjects_path = "./assets"\n',
  ...
  it.live(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        yield* fs.makeDirectory(path.join(tmp.current, "supabase", "assets"), {
          recursive: true,
        });
        yield* fs.writeFileString(
          path.join(tmp.current, "supabase", "assets", "logo.png"),
          "fake-png-bytes",
        );
        yield* fs.writeFileString(
          path.join(tmp.current, "supabase", "assets", ".DS_Store"),
          "junk",
        );
        const { layer, out, requests } = yield* setupSeedBuckets(tmp.current, {
          toml: [
  ...
  it.live("skips a .DS_Store file when objects_path points directly at it (CLI-1950)", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(path.join(tmp.current, "supabase", ".DS_Store"), "junk");
      const { layer, out, requests } = yield* setupSeedBuckets(tmp.current, {
  ...
  it.live.skipIf(isRoot)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* fs.makeDirectory(path.join(tmp.current, "supabase", "private"), {
          recursive: true,
        });
        yield* fs.writeFileString(path.join(tmp.current, "supabase", "assets", "a.txt"), "hello");
        const secret = path.join(tmp.current, "supabase", "private", "secret.txt");
        yield* fs.writeFileString(secret, "top secret");
        yield* fs.chmod(secret, 0o000);
        yield* fs.symlink(
          "../private/secret.txt",
          path.join(tmp.current, "supabase", "assets", "link-to-secret"),
        );
        const { layer, out, requests } = yield* setupSeedBuckets(tmp.current, {
          toml: '[storage.buckets.images]\npublic = true\nobjects_path = "./assets"\n',
          routes: [
  ...
  it.live("follows a symlinked objects_path root and uploads its files (Go fs.WalkDir)", () =>
    Effect.gen(function* () {
      ...
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      yield* fs.makeDirectory(path.join(tmp.current, "supabase", "real-assets"), {
        recursive: true,
      });
      yield* fs.writeFileString(
        path.join(tmp.current, "supabase", "real-assets", "a.txt"),
        "hello",
      );
      yield* fs.symlink("./real-assets", path.join(tmp.current, "supabase", "linked-assets"));
      const { layer, out, requests } = yield* setupSeedBuckets(tmp.current, {
  ...
  it.live("rejects --project-ref on the default local target", () =>
    Effect.gen(function* () {
      ...
      const FLAG_REF = "flagflagflagflagflag";
      const { layer, requests, linkedCache } = yield* setupSeedBuckets(tmp.current, {
        toml: "[storage.buckets.test]\npublic = true\n",
      });
      const exit = yield* seedBuckets({
        linked: false,
  ...
  it.live("--linked fails before any Storage call when the api-keys list is empty", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        expect(Cause.pretty(exit.cause)).toContain("Anon key not found.");
      }
  ...
  it.live("--linked surfaces tenant.GetApiKeys auth error on a non-200 api-keys response", () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        const causeText = Cause.pretty(exit.cause);
        expect(causeText).toContain("StorageAuthTokenError");
        expect(causeText).toContain(
          "Authorization failed for the access token and project ref pair",
        );
        expect(causeText).not.toContain("unexpected get api keys status");
      }
      expect(requests.some((r) => r.url.includes("/storage/v1/"))).toBe(false);
    }).pipe(seedScenario),
  );
  ...
  it.live("caches the linked project on --linked but not on local", () =>
    Effect.gen(function* () {
      ...
      const linked = yield* setupSeedBuckets(tmp.current, {
        toml: "[storage.buckets.test]\npublic = true\n",
        projectRef: VALID_REF,
        args: ["seed", "buckets", "--linked"],
        routes: [
      ...
      const local = yield* setupSeedBuckets(tmp.current, {
        toml: "[storage.buckets.test]\npublic = true\n",
        routes: [
          { method: "GET", match: "/storage/v1/bucket", body: [] },
          { method: "POST", match: "/storage/v1/bucket", body: { name: "test" } },
  ...
  it.live("reads cert_path and key_path from disk when both api.tls paths are set", () =>
    Effect.gen(function* () {
      ...
      yield* fs.makeDirectory(path.join(tmp.current, "supabase"), { recursive: true });
      yield* fs.writeFileString(path.join(tmp.current, "supabase", "custom-ca.crt"), certContent);
      yield* fs.writeFileString(path.join(tmp.current, "supabase", "custom-ca.key"), keyContent);
      const { layer, requests } = yield* setupSeedBuckets(tmp.current, {
  ...
  it.live("local run: SUPABASE_AUTH_JWT_SECRET overrides auth.jwt_secret", () =>
    seedScenario(
      ...
      withEnvVar(
        "SUPABASE_AUTH_JWT_SECRET",
        "custom-jwt-secret-at-least-32-chars-long!",
        withEnvVar(
          "SUPABASE_AUTH_SERVICE_ROLE_KEY",
          undefined,
          Effect.gen(function* () {
      ...
      withEnvVar(
        ...
        withEnvVar(
          ...
          Effect.gen(function* () {
            ...
            expect(Exit.isSuccess(exit)).toBe(true);
            expect(
              requests.every((r) => r.headers["apikey"] === "env-local-service-role-key"),
            ).toBe(true);
          }),
        ),
      ),
  ...
  it.live("fails when cert_path is set but key_path is missing", () =>
    Effect.gen(function* () {
      ...
      yield* fs.makeDirectory(path.join(tmp.current, "supabase"), { recursive: true });
      yield* fs.writeFileString(
        path.join(tmp.current, "supabase", "custom-ca.crt"),
        "-----BEGIN CERTIFICATE-----\nZHVtbXk=\n-----END CERTIFICATE-----\n",
      );
      const { layer, requests } = yield* setupSeedBuckets(tmp.current, {
        toml: '[api.tls]\nenabled = true\ncert_path = "custom-ca.crt"\n[storage.buckets.docs]\npublic = false\n',
  ...
  it.live("fails when key_path is set but cert_path is missing", () =>
    Effect.gen(function* () {
      ...
      yield* fs.makeDirectory(path.join(tmp.current, "supabase"), { recursive: true });
      yield* fs.writeFileString(
        path.join(tmp.current, "supabase", "custom-ca.key"),
        "-----BEGIN PRIVATE KEY-----\nZHVtbXk=\n-----END PRIVATE KEY-----\n",
      );
      const { layer, requests } = yield* setupSeedBuckets(tmp.current, {
        toml: '[api.tls]\nenabled = true\nkey_path = "custom-ca.key"\n[storage.buckets.docs]\npublic = false\n',
  ...
  it.live("fails when cert_path points to an unreadable file", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        expect(Cause.pretty(exit.cause)).toContain("failed to read TLS cert:");
      }
      expect(requests).toHaveLength(0);
    }).pipe(seedScenario),
  );
  ...
  it.live("fails when key_path points to an unreadable file", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
        expect(Cause.pretty(exit.cause)).toContain("failed to read TLS key:");
      }
      expect(requests).toHaveLength(0);
    }).pipe(seedScenario),
  );
  ...
  it.live(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const { layer, requests, telemetry } = yield* setupSeedBuckets(sub, {
          explicitWorkdir: true,
        });
        const exit = yield* seedBuckets(DEFAULT_FLAGS).pipe(Effect.provide(layer), Effect.exit);
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isFailure(exit)) {
          expect(Cause.pretty(exit.cause)).toContain("SeedMissingProjectConfigError");
        }
        expect(requests).toHaveLength(0);
  ...
  it.live(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const { layer, requests } = yield* setupSeedBuckets(missing, { explicitWorkdir: true });
        const exit = yield* seedBuckets(DEFAULT_FLAGS).pipe(Effect.provide(layer), Effect.exit);
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isFailure(exit)) {
          const causeText = Cause.pretty(exit.cause);
          expect(causeText).toContain("SeedWorkdirError");
