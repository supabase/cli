...
`SUPABASE_USE_SLIM_IMAGES` for db; `SUPABASE_EDGE_RUNTIME_DENO_VERSION`,
`SUPABASE_INTERNAL_IMAGE_REGISTRY`, `BITBUCKET_CLONE_DIR` for functions) — `pull` does not read
or override any of these itself, it only supplies the resolved `ref`. The historical
`SUPABASE_EXPERIMENTAL_PG_DELTA` opt-in is no longer read by the db step.
