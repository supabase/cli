...
import {
  ...
  pgDeltaNextDiagnosticMessage,
  pgDeltaNextDiagnosticReport,
  pgDeltaNextFeedbackInvitation,
  reportPgDeltaNextDiagnostics,
} from "./pgdelta-next-diagnostics.ts";

const unmodeled = (
  kind: unknown,
  overrides: Partial<PgDeltaNextDiagnostic> = {},
...
describe("pg-delta next diagnostic coverage policy", () => {
  it("summarizes unmodeled kinds and routes nonfatal diagnostic detail to debug", () => {
    ...
    return Effect.gen(function* () {
      yield* reportPgDeltaNextDiagnostics(
        "diff",
        [
  ...
  it("renders coverage diagnostics and then fails in strict mode", () => {
    ...
    const debugMessages: string[] = [];
    return Effect.gen(function* () {
      const exit = yield* reportPgDeltaNextDiagnostics(
        "declarativePlan",
        [unmodeled("text search configuration")],
        true,
      ).pipe(Effect.exit);
  ...
  const skippedStatement = (file: string, statement: string): PgDeltaNextDiagnostic => ({
  ...
  it("classifies all upstream coverage codes and aggregates arbitrary kinds safely", () => {
    const report = pgDeltaNextDiagnosticReport(
      [
  ...
  it("omits an unknown subject and keeps feedback free of diagnostic details", () => {
    ...
    const invitation = pgDeltaNextFeedbackInvitation(["operator class"]);
    ...
    expect(pgDeltaNextFeedbackInvitation(["user's future kind"])).toContain(
      `user'"'"'s future kind`,
    );
  });
});
