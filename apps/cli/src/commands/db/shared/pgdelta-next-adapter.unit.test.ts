...
import {
  formatPgDeltaNextSql,
  pgDeltaNextAdapterLayerFromLibraries,
  pgDeltaNextDefaultFormatOptions,
  pgDeltaNextProfile,
  summarizePgDeltaNextHazards,
  summarizePgDeltaNextRemovals,
  type PgDeltaNextLibraries,
} from "./pgdelta-next-adapter.layer.ts";
import { PgDeltaNextAdapter, PgDeltaNextError } from "./pgdelta-next-adapter.service.ts";
...
function setupLibraries(sourcePool: Pool, desiredPool: Pool) {
  ...
  return {
    state,
    layer: pgDeltaNextAdapterLayerFromLibraries(libraries),
  };
}
...
describe("PgDeltaNextAdapter", () => {
  ...
  it("derives semantic hazards and destructive non-DROP actions from the typed plan", () => {
    ...
    expect(
      summarizePgDeltaNextHazards({ actions: [destructiveAlter] }, [
        {
...
`);
  ...
  it("renders selected-schema state without leaking other user or platform objects", () => {
    ...
    const profile = pgDeltaNextProfile(["public", "auth"]);
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(state.exportInputs[1]).toMatchObject({
          format: pgDeltaNextDefaultFormatOptions,
        });
