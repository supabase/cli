...
export function debugBundleMessage(debugDir: string): string {
  ...
  if (debugDir.length > 0) {
    lines.push(`Debug information saved to ${bold(debugDir)}`, "");
  }
  lines.push(
    ...
    "",
    yellow("WARNING: The debug folder may contain sensitive information about your"),
    yellow("database schema, including table structures, function definitions, and role"),
    yellow("configurations. Review the contents carefully before sharing publicly."),
    yellow("If unsure, prefer opening a support ticket (option 2) instead."),
  );
  return `${lines.join("\n")}\n`;
}
