...
import type { LegacyEdgeRuntimeScript } from "../../../command-internal/legacy-edge-runtime-script.service.ts";
import {
  LegacyDbConnection,
...
} from "../../../command-internal/legacy-glob.ts";
import type { LegacyDockerRun } from "../../../command-internal/legacy-docker-run.service.ts";
import type { LegacyImagePrepullError } from "../../../command-internal/db-bootstrap/image-prepull.ts";
import type { LegacyHealthCheckTimeoutError } from "../../../command-internal/db-bootstrap/health-check.ts";
import { legacyWaitForShadowReady } from "../../../command-internal/db-bootstrap/health-check.ts";
import { legacySeedGlobals } from "../../../command-internal/legacy-migration-apply.ts";
import {
  LEGACY_BAD_PATTERN_MESSAGE,
  legacyPathMatch,
} from "../../../command-internal/legacy-path-match.ts";
import { legacyToPostgresURL } from "../../../command-internal/legacy-postgres-url.ts";
import type { LegacyShadowAcquiredHandle } from "../../../command-internal/db-bootstrap/shadow-cache.ts";
import {
...
import type { LegacyStartSetupLocalDatabaseError } from "../../../command-internal/db-bootstrap/db-setup.ts";
import {
  ...
  return Effect.gen(function* () {
    ...
    for (const rawPattern of patterns) {
      ...
      for (const match of matches) {
        ...
        // local-target diff against the wrong target. `legacyWalkSqlFiles` (`legacy-glob.ts`)
        // also matches Go's byte-sorted, no-follow-symlink walk semantics — see its own doc
        // comment.
        const sqlRelativeResult = yield* legacyWalkSqlFiles(fs, absMatch, "").pipe(Effect.result);
 ...
 * `apps/cli-go/internal/db/diff/diff.go:65-76,86-96`). `legacyWalkSqlFiles` (`legacy-glob.ts`)
 ...
 * matching `legacyWalkSqlFiles`'s existing never-checks-its-own-root behavior). `afero.Walk` is the
 ...
 * `legacyWalkSqlFiles`'s own relative-path construction produced internally, not just the outer
...
function legacyWalkSqlFilesSorted(
  ...
  return Effect.gen(function* () {
    ...
    const sqlRelative = yield* legacyWalkSqlFiles(fs, dirAbs, "").pipe(
      Effect.mapError(
        (cause) =>
