...
import {
  ...
  PGADMIN_DIFF_HEADER,
  parsePgAdminDiffEntries,
  processPgAdminDiffOutput,
  processPgAdminDiffProgress,
  renderPgAdminDiff,
...
describe("processPgAdminDiffOutput", () => {
  describe("filtering rules (container_output.go:154-195)", () => {
    ...
    it("skips an entry whose status is Identical, even with a non-empty diff_ddl", () => {
      const result = processPgAdminDiffOutput(JSON.stringify([entry({ status: "Identical" })]));
  ...
  describe("empty / DESKTOP-mode-prefix handling (container_output.go:141-147)", () => {
    ...
    it("trims the DESKTOP-mode NOTE prefix from the front of the buffer before parsing", () => {
      const payload = PGADMIN_DESKTOP_NOTE_PREFIX + JSON.stringify([entry()]);
      expect(processPgAdminDiffOutput(payload)).toEqual(
  ...
  describe("Go encoding/json acceptance rules", () => {
    ...
    it("rejects an array whose element is neither an object nor null (e.g. a bare number)", () => {
      expect(Result.isFailure(processPgAdminDiffOutput("[1]"))).toBe(true);
    });

    it("rejects an array whose element is neither an object nor null (e.g. a bare string)", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('["x"]'))).toBe(true);
    });

    it("rejects an array element that is itself an array", () => {
      expect(Result.isFailure(processPgAdminDiffOutput("[[]]"))).toBe(true);
    });

    it("accepts an unknown field and treats the entry as if absent, skipping it", () => {
      expect(processPgAdminDiffOutput('[{"unknown":1}]')).toEqual(Result.succeed(""));
    });

    it("treats a null status field as absent, not as Identical", () => {
      const result = processPgAdminDiffOutput(JSON.stringify([entry({ status: null })]));
      expect(result).toEqual(Result.succeed(headerPlus("ALTER TABLE test;")));
    });

    it("treats a null dependencies field as absent (no dependency filtering applied)", () => {
      const result = processPgAdminDiffOutput(JSON.stringify([entry({ dependencies: null })]));
      expect(result).toEqual(Result.succeed(headerPlus("ALTER TABLE test;")));
    });
    ...
    it("rejects a dependencies array whose element is itself an array", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('[{"dependencies":[[]]}]'))).toBe(true);
    });
    ...
    it("rejects a mistyped type field (number instead of string)", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('[{"type":123}]'))).toBe(true);
    });

    it("rejects a mistyped status field (number instead of string)", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('[{"status":123}]'))).toBe(true);
    });

    it("rejects a mistyped diff_ddl field (number instead of string)", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('[{"diff_ddl":123}]'))).toBe(true);
    });

    it("rejects a mistyped group_name field (number instead of string)", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('[{"group_name":123}]'))).toBe(true);
    });

    it("rejects a mistyped source_schema_name field (number instead of string)", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('[{"source_schema_name":123}]'))).toBe(true);
    });
    ...
    it("rejects a dependencies array whose element isn't an object or null (e.g. a number)", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('[{"dependencies":[1]}]'))).toBe(true);
    });

    it("rejects a dependency element with a mistyped type field", () => {
      expect(Result.isFailure(processPgAdminDiffOutput('[{"dependencies":[{"type":1}]}]'))).toBe(
        true,
      );
    });

    it("accepts trailing whitespace after the JSON array", () => {
      const result = processPgAdminDiffOutput(`${JSON.stringify([entry()])}\n  \n`);
      expect(result).toEqual(Result.succeed(headerPlus("ALTER TABLE test;")));
    });

    it("rejects two concatenated JSON arrays in a SINGLE buffer (this function's own single-buffer contract — diffSchemaPgAdmin no longer feeds it a multi-run concatenation; see parsePgAdminDiffEntries below for the per-run parse)", () => {
...
describe("parsePgAdminDiffEntries", () => {
  it("returns an empty array for an entirely empty buffer", () => {
    expect(parsePgAdminDiffEntries("")).toEqual(Result.succeed([]));
  });

  it("returns an empty array when the buffer is only the DESKTOP-mode NOTE prefix", () => {
    expect(parsePgAdminDiffEntries(PGADMIN_DESKTOP_NOTE_PREFIX)).toEqual(Result.succeed([]));
  });

  it("trims the DESKTOP-mode NOTE prefix from the front of the buffer before parsing", () => {
    const payload = PGADMIN_DESKTOP_NOTE_PREFIX + JSON.stringify([entry()]);
    expect(parsePgAdminDiffEntries(payload)).toEqual(Result.succeed(["ALTER TABLE test;"]));
  });
  ...
  it("returns the ordered, filtered, trimmed DDLs — not the rendered header/join", () => {
    ...
    expect(parsePgAdminDiffEntries(JSON.stringify(entries))).toEqual(
      Result.succeed(["DDL_1;", "DDL_2;"]),
    );
  });

  it("fails on two concatenated JSON arrays within one buffer, same as processPgAdminDiffOutput", () => {
    const payload = `${JSON.stringify([entry()])}${JSON.stringify([entry()])}`;
    expect(Result.isFailure(parsePgAdminDiffEntries(payload))).toBe(true);
  });
});

describe("renderPgAdminDiff", () => {
  it("returns an empty string for an empty DDL list", () => {
    expect(renderPgAdminDiff([])).toBe("");
  });

  it("renders the pgAdmin header followed by every DDL joined with a blank line", () => {
    expect(renderPgAdminDiff(["DDL_1;", "DDL_2;"])).toBe(headerPlus("DDL_1;\n\nDDL_2;"));
  });
});

describe("processPgAdminDiffProgress", () => {
  it.each([
    ...
    expect(processPgAdminDiffProgress(line)).toEqual(expected);
  });

  it("scans multiple lines and strips \\r\\n line endings before matching", () => {
    const input =
      "Starting schema diff...\r\nComparing Tables 45%\r\nnoise line\r\nDiffing 100%\r\n";
    expect(processPgAdminDiffProgress(input)).toEqual(["Comparing Tables ", "Diffing 1"]);
  });
