...
import { legacyShadowRunInputFromLocalContainerInputs } from "../../../command-internal/db-bootstrap/shadow-database.ts";
...
export const legacyRunDbPull = Effect.fn("legacy.db.pull.run")(function* (
  ...
  return yield* Effect.gen(function* () {
    ...
    const localInputs: Option.Option<LegacyLocalDbContainerInputs> = useExperimentalExport
      ? Option.none()
      : Option.some(
  ...
  if (output.format !== "text") {
    if (outcome.kind === "declarative") {
      ...
      if (schemaWritten === undefined) {
        ...
        // `legacyRunDbPull` confirmed at least one migration file was written
