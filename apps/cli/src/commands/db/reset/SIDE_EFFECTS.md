...
| `docker stop <storage\|auth\|realtime\|pooler container>`                                                                    | local path, both PG14 and PG15, before any database work                          | pre-teardown fence (#6445): a running satellite re-runs its own migrations on reconnect and races the init jobs; stopping also disarms `unless-stopped`. Concurrent, not-found tolerated per service; a genuine failure aborts before the database is touched, and the satellites that did stop are restored |
...
| `docker start <storage\|auth\|realtime\|pooler container>`                                                                   | local path, when the fence's stop, or the work inside it, fails or is interrupted | best-effort restore so a failed reset does not leave the stack down, followed by the same Kong reload the success path does (a restarted container can come back on a different address); a failure here prints a `WARNING:` line and the reset's own error is still the one reported                        |
...
**Both branches** stop the storage/auth/realtime/pooler containers before any database
work and then restart the same containers
...
| `1`   | local: container/volume remove, network/volume/container create, health-check timeout, PG14 SQL, satellite stop/restart, or Kong-reload failure                                                                |
| `130` | local: interrupted (Ctrl-C). The satellites stopped by the fence are restarted and Kong reloaded before the command exits, so an interrupt costs a few extra seconds; that restore is not itself interruptible |
