...
| GET    | `/containers/supabase_db_<project>/json` | —    | —            | local path — the running probe's Docker Engine API request over the active context's local socket/named pipe (Engine-identified 200 → running, 404 → absent; anything else → the spawned-CLI fallback) |
