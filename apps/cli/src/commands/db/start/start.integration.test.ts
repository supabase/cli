...
describe("LocalDockerEngine (direct Engine-API transport)", () => {
  ...
  it.live("gives no answer when the local socket cannot be dialed", () =>
    withDockerHost(
      `unix://${join(tempRoot.current, "never-created.sock")}`,
      engineContainerExists("supabase_db_engine-probe"),
    ).pipe(
      Effect.map((answer) => {
        expect(answer).toEqual(Option.none());
...
describe("isLocalDbRunning", () => {
  const probeWorkdir = (spawnerLayer: Layer.Layer<ChildProcessSpawner.ChildProcessSpawner>) =>
    Effect.gen(function* () {
  ...
  const runProbe = (
    ...
    const child = mockContainerCliSpawner(route);
    const effect = probeWorkdir(child.layer).pipe(
      Effect.provideService(LocalDockerEngine, engineAnswers),
    );
    return { child, effect };
  };
  ...
  it.live("falls back to the container CLI when the Engine gives no answer", () => {
    ...
    return effect.pipe(
      Effect.map((running) => {
        expect(running).toBe(false);
        expect(child.spawned.map((s) => s.args)).toEqual([
  ...
  it.live(
    ...
    () => {
      ...
      return withDockerHost(
        `unix://${join(tempRoot.current, "never-created.sock")}`,
        probeWorkdir(child.layer).pipe(Effect.provide(localDockerEngineLayer)),
      ).pipe(
        Effect.map((running) => {
          expect(running).toBe(false);
          expect(child.spawned.map((s) => s.args)).toEqual([
