...
| -------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------ | -------------------------------------------------------- |
| `DELETE` | `<feedback-env-url>/rest/v1/interfaces_feedback?delete_token=eq.<token>` | `apikey` = committed publishable key; `x-feedback-token: <token>`; `x-feedback-project-ref: <ref>` when resolved; `x-feedback-user-id: <uuid>` when logged in; `Prefer: count=exact` | —            | `Content-Range: */1` (deleted) vs `*/0` (no row matched) |
...
the matching `x-feedback-project-ref` / `x-feedback-user-id` header; extra
context against a context-free row is ignored server-side, so
...
consent opt-out. The CLI never reads a feedback row: no select request is
made and the row's text is never fetched or shown (CLI-2406 removes the
backend read path as well). The request times out after 10 s. `--debug` logs
the request line on stderr with the `delete_token` filter redacted
...
| `1`  | backend failure (PostgREST error, network failure, or 10 s timeout)                                        |
...
The confirmation prompt runs before the row's existence is known (the CLI
never reads the row), so a wrong token is answered with the not-found error
after the prompt. The delete runs behind a "Deleting feedback..." spinner.
`--yes` / `SUPABASE_YES` skips the confirmation prompt. `-o json` requires
`--yes` like the machine output formats do: it never prompts, even on a TTY,
so stdout stays payload-only, and it emits `{ "deleted": true }`.
...
rather than deleting without confirmation. The "Deleting feedback..." task is
reported on stderr as a `[task] start: …` line; stdout carries the payload
only.
...
{"type":"result","data":{"message":"Feedback deleted."},"timestamp":"…"}
```

Also requires `--yes`. The task start arrives as a `log` event before the
...
- The feedback text is never read or shown: the command is a token-scoped
  hard delete, and the confirmation prompt is the only guard against a
  mistyped or pasted token. Deleting is harmless to the user (the row exists
  for Supabase's benefit), so nothing is fetched first.
...
- Project-ref resolution order: `--project-ref` (an empty value counts as
  ...
  project can only be deleted with that same ref presented — rerun
...
- A row submitted while logged in (with telemetry consent) carries a `user_id`
  and can only be deleted while logged in as that same user — the
