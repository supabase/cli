...
| Path                                            | Format | When                                                  |
| ----------------------------------------------- | ------ | ----------------------------------------------------- |
| `<SUPABASE_HOME or ~/.supabase>/telemetry.json` | JSON   | after command completion, on both success and failure |
...
Prints one `result` NDJSON event whose `data` contains `gotrue_id`, `primary_email`, and
`username`.
