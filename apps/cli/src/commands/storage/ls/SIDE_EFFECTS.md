...
| local API TLS cert/key                        | PEM        | Legacy local TLS CA; stack-backed requests read configured CA files only when the observed Storage endpoint is HTTPS. HTTP stack endpoints skip CA resolution; when neither custom TLS path is configured, the embedded default is used.                                                                                                                                                                                                                                                                        |
| `<workdir>/supabase/.env*`, `<workdir>/.env*` | dotenv     | local path: read on both backends for `env(VAR)` interpolation while loading `config.toml`; on legacy runs to resolve `SUPABASE_API_*` for gateway URL/TLS, and on stack runs with an observed HTTPS Storage endpoint to resolve the configured CA (HTTP stack endpoints skip CA resolution); `SUPABASE_AUTH_{JWT_SECRET,SERVICE_ROLE_KEY}` overrides apply to legacy runs only                                                                                                                                 |
...
| `SUPABASE_API_PORT`, `SUPABASE_API_EXTERNAL_URL`, `SUPABASE_API_TLS_ENABLED`, `SUPABASE_API_TLS_CERT_PATH`, `SUPABASE_API_TLS_KEY_PATH`, `SUPABASE_API_ENABLED` | local: override matching `[api]` fields; legacy runs use them for gateway URL/TLS, while stack runs use TLS settings to trust an observed HTTPS Storage endpoint. HTTP stack endpoints skip CA resolution (shell wins); auth overrides are legacy-only | no                                   |
