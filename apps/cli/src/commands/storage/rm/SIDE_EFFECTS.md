...
| `<workdir>/supabase/.env*`, `<workdir>/.env*` | dotenv     | always, to resolve `SUPABASE_YES` (CLI-1878) and, on both backends, for `env(VAR)` interpolation while loading `config.toml`; on the local legacy path to resolve `SUPABASE_API_*` for gateway URL/TLS, and on stack runs with an observed HTTPS Storage endpoint to resolve the configured CA (HTTP stack endpoints skip CA resolution); `SUPABASE_AUTH_{JWT_SECRET,SERVICE_ROLE_KEY}` overrides apply to legacy runs only                                                                             |
