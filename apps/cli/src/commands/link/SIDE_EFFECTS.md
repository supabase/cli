...
| `GET`  | `/v1/projects/{ref}/api-keys`               | only when the `reveal=true` request is refused with 403: the token may list keys but not reveal secrets, and the redacted listing still satisfies the anon-key check       |
