...
import {
  buildPostgrestContainerSpec,
  buildPostgrestEnv,
  type PostgrestContainerSpecInput,
} from "./postgrest.service.ts";

describe("buildPostgrestEnv", () => {
  const base = {
  ...
  test("feeds the resolved JWKS string into PGRST_JWT_SECRET, not the raw jwt secret", () => {
    const env = buildPostgrestEnv({ ...base, jwks: '{"keys":["fake"]}' });
...
describe("buildPostgrestContainerSpec", () => {
  const input: PostgrestContainerSpecInput = {
    projectId: "proj",
  ...
  test("reuses (does not recompute) the resolved db password from a non-default dbUrl", () => {
    const spec = buildPostgrestContainerSpec({
      ...input,
