 ...
 * formats the `rw,z` bind. The shared SELinux relabel (`z`) lets Kong read
 * host files whose context is not `container_file_t` — e.g. checkouts on
 * mounts labelled `mnt_t` — where the container would otherwise answer 403
 * and GoTrue silently falls back to its built-in templates (supabase/auth#2783).
 * On hosts without SELinux the flag is a no-op.
 *
