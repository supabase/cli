 ...
 * SET but empty is used verbatim (unlike `local-config-values.ts`'s
 * `envOverride`, which treats an empty resolved value as unset). `projectEnvValues`
 * mirrors that module's own merged (dotenv + ambient shell, ambient-wins) map;
 * `??` only skips a `null`/`undefined` operand, never an empty string, so
 * this naturally reproduces "set, even if empty" semantics without a separate
 * presence check. No `SUPABASE_` prefix and no `env(VAR)` indirection — this
 * reads the raw env var directly, bypassing the decode-hook chain those only
 * apply to.
 *
