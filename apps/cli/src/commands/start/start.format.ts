 ...
 */
export function startAlreadyRunningMessage(): string {
  return `${aqua("supabase start")} is already running.\n`;
}
 ...
 */
export const START_STARTING_CONTAINERS_MESSAGE = "Starting containers...\n";
 ...
 */
export const START_WAITING_FOR_HEALTH_CHECKS_MESSAGE = "Waiting for health checks...\n";

/**
 * Printed on success (or an ignored unhealthy-timeout), immediately before
 * the status table renders to stdout.
 */
export function startCompletedMessage(): string {
  return `Started ${aqua("supabase")} local development setup.\n\n`;
}
 ...
 */
export function startSecurityNotice(): string {
  return (
    `${yellow("Local dev security notice")}\n` +
