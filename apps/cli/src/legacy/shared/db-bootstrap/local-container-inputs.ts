/**
 * The local-container-bring-up prelude designed to be shared by `db start`
 * (`commands/db/start/start.handler.ts`) and `db reset`
 * (`commands/db/reset/reset.handler.ts`) before calling their own composition
 * (`legacyStartDatabase`/`legacyRecreateLocalDatabase`): load the local project
 * context, resolve config values + the `LegacyDbBootstrapConfig` derivation, the
 * container's network id/opts/id, the Postgres container-spec fields common to
 * both callers, the lazy image-resolve `Effect`, and the `LegacyFreshDbSetupInput`
 * `setup` object `legacyRunFreshDbSetup` needs. Hoisted here (CLI-1955 review
 * follow-up) — the two callers used to each run an independently-typed ~130-line
 * copy of this exact sequence, with no test comparing them.
 *
 * **Only `db reset` (`reset-local-database.ts`) actually calls this today.**
 * `db start`'s own handler still carries its own ~130-line inline copy of the same
 * sequence, accreted with several `db start`-specific review fixes since the hoist
 * (the already-running-shortcut ordering, `experimental`/`realtime`/`storage`/`auth`
 * config overlays for its own fresh-volume setup, etc.) that were never reconciled
 * back into this shared shape (review CLI-1958). The two copies can silently drift
 * until that reconciliation happens — finishing it is tracked as a follow-up rather
 * than folded into an unrelated `db reset` change, given how much `db start`-only
 * nuance sits in that inline copy today.
 *
