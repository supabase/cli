 ...
 * stopped container.
 *
 * Gated with `describeLive` (the cli-e2e-ci signal, which is also the only environment with a real
 * Docker daemon). This is deliberately NOT a `runSupabaseLive` subprocess test: the contract under
 * test is the acquire pair itself, and driving it directly avoids standing up a full local stack
 * for `db diff` just to observe a container's cluster.
 *
...
/** A port well clear of the CLI's own defaults, so a live stack on this runner is untouched. */
const LIVE_SHADOW_PORT = 54399;
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const setup: LegacyShadowDbSetupInput<never> = {
          ...
          dbUrl: `postgresql://postgres:postgres@127.0.0.1:${LIVE_SHADOW_PORT}/postgres`,
        ...
        process.env[LEGACY_SHADOW_CACHE_ENV] = "1";
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            // Whatever a real run's migrations would do: a cluster-global role plus a database
            // object, neither of which may survive into the next run.
        ...
        yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            // Restored, not re-initialized: the baseline cluster's own roles are all still there.
