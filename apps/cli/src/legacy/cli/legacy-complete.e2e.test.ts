...
describe("supabase __complete (legacy)", () => {
  test(
    ...
    () =>
      runSupabase(["__complete", "migration", "li"], {
        entrypoint: "legacy",
  ...
  test(
    ...
    { timeout: E2E_TIMEOUT_MS },
    () =>
      runSupabase(["__completeNoDesc", "migration", "li"], {
        entrypoint: "legacy",
      }).then(({ exitCode, stdout }) => {
        expect(exitCode).toBe(0);
        const lines = stdout.trim().split("\n");
  ...
  test("root-level flag-name completion offers --debug", { timeout: E2E_TIMEOUT_MS }, () =>
    runSupabase(["__complete", "--d"], {
      entrypoint: "legacy",
    }).then(({ exitCode, stdout }) => {
      expect(exitCode).toBe(0);
