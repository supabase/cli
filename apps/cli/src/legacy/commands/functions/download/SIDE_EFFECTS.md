...
| `<workdir>/supabase/.temp/edge-runtime-version`     | plain text | Docker-unbundle path: overrides the default edge-runtime image tag when present                                                                                                        |
| `<workdir>/supabase/config.toml` (or `config.json`) | TOML/JSON  | Docker-unbundle path: resolves `edge_runtime.deno_version` and `project_id` (`loadProjectConfig`) — a new file-read surface versus the `--use-api` path, which reads no project config |
