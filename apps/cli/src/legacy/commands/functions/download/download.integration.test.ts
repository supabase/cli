...
import { containerArchiveBytes } from "../../../../shared/functions/functions-docker.ts";
...
/**
 * Spawner for the slim (uid 65532) docker-unbundle lifecycle: `create` →
 * `start --attach` → `cp <name>:… -` → `rm --force`. Every step exits 0
 * unless overridden; the copy-out step's stdout is the RAW tar bytes of
 * `copyOutArchive` — the shared `mockChildProcessSpawner` can't model that
 * (it emits newline-terminated text lines, which corrupts a tar stream).
 */
function mockSlimDockerLifecycle(
  opts: {
    /** Read at spawn time, so a test can fill it inside its own Effect. */
    readonly copyOut?: { archive?: Uint8Array };
    readonly startExitCode?: number;
    readonly startStderr?: ReadonlyArray<string>;
  } = {},
) {
  const spawned: Array<{ command: string; args: ReadonlyArray<string> }> = [];
  ...
  const spawner = ChildProcessSpawner.make((command) =>
    Effect.gen(function* () {
      ...
      const isCopyOut = args[0] === "cp" && args[2] === "-";
      const exitCode = isStart ? (opts.startExitCode ?? 0) : 0;
      const copyOutArchive = opts.copyOut?.archive;
      const stdout =
        isCopyOut && copyOutArchive !== undefined ? Stream.make(copyOutArchive) : Stream.empty;
      const stderr =
        isStart && opts.startStderr !== undefined
          ? Stream.fromIterable(opts.startStderr.map((line) => encoder.encode(`${line}\n`)))
          : Stream.empty;

      const exitDeferred = yield* Deferred.make<ChildProcessSpawner.ExitCode>();
      yield* Deferred.succeed(exitDeferred, ChildProcessSpawner.ExitCode(exitCode));
...
describe("legacy functions download — slim images (SUPABASE_USE_SLIM_IMAGES)", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
  });

  function buildSlimLayer(child: { layer: Layer.Layer<ChildProcessSpawner.ChildProcessSpawner> }) {
    const out = mockOutput({ format: "text" });
    const api = mockLegacyPlatformApi();
    const proxy = mockProxy();
    const layer = Layer.mergeAll(
  ...
  it.live(
    "stages the unbundle in the container and copies the files out host-side instead of binding the host functions directory",
    () => {
      vi.stubEnv("SUPABASE_USE_SLIM_IMAGES", "1");
      // uid 65532 cannot write into a host-owned `supabase/functions` bind on
      // native Linux, so the slim flow must never mount it — the extracted
      // files arrive via `docker cp` and are written by the CLI process.
      const copyOut: { archive?: Uint8Array } = {};
      const child = mockSlimDockerLifecycle({ copyOut });
      const { layer } = buildSlimLayer(child);

      return Effect.gen(function* () {
        copyOut.archive = yield* Effect.promise(() =>
          containerArchiveBytes({
            "hello-world/index.ts": "console.log('from slim unbundle')\n",
            "hello-world/nested/util.ts": "export const util = 1;\n",
          }),
        );
        yield* legacyFunctionsDownload({ ...baseFlags, useDocker: true });

        // The slim flow never uses `docker run` — the container must survive
        // its own exit for `docker cp` to read the extracted files.
        expect(child.spawned.some((spawned) => spawned.args[0] === "run")).toBe(false);

        // Slim runs get their OWN cache volume: a docker.io-seeded
        // `supabase_edge_runtime_<id>` volume is root-owned and unwritable by
        // uid 65532, so the two families never share a name.
        expect(child.spawned.find((spawned) => spawned.args[0] === "volume")).toEqual({
          command: "docker",
          args: [
            "volume",
            ...
            "--label",
            `com.docker.compose.project=${PROJECT_ID}`,
            `supabase_edge_runtime_slim_${PROJECT_ID}`,
          ],
        });

        const createCommand = child.spawned.find((spawned) => spawned.args[0] === "create");
        expect(createCommand).toBeDefined();
        const containerName = createCommand!.args[2]!;
        expect(containerName).toMatch(/^supabase_unbundle_hello-world_[0-9a-f]{8}$/);
        ...
        const functionsDir = resolve(tempRoot.current, "supabase", "functions");
        // Cache volume over the nonroot home; eszip bind under /tmp (readable
        // by uid 65532 on every slim tag); NO host functions bind.
        expect(createCommand!.args).toContain(
          `supabase_edge_runtime_slim_${PROJECT_ID}:/home/nonroot:rw`,
        );
        expect(createCommand!.args).toContain(
          `${hostEszipPath}:/tmp/eszips/output_hello-world.eszip:ro`,
        );
        expect(createCommand!.args).not.toContain(`${functionsDir}:/home/deno:rw`);
        expect(createCommand!.args.slice(-6)).toEqual([
          // Slim refs skip registry-candidate rewriting, so the resolved image
          // is the ghcr.io/supabase/cli reference itself.
          dockerfileServiceImage("edgeruntime"),
          "unbundle",
          "--eszip",
          "/tmp/eszips/output_hello-world.eszip",
          "--output",
          "/tmp/unbundle/hello-world",
        ]);

        expect(child.spawned).toContainEqual({
          command: "docker",
          args: ["start", "--attach", containerName],
        });
        expect(child.spawned).toContainEqual({
          command: "docker",
          args: ["cp", `${containerName}:/tmp/unbundle/hello-world`, "-"],
        });
        expect(child.spawned).toContainEqual({
          command: "docker",
        ...
        // The extracted files were written host-side, by the CLI process.
        expect(
          yield* Effect.tryPromise(() =>
            readFile(join(functionsDir, "hello-world", "index.ts"), "utf8"),
          ),
        ).toBe("console.log('from slim unbundle')\n");
        expect(
          yield* Effect.tryPromise(() =>
            readFile(join(functionsDir, "hello-world", "nested", "util.ts"), "utf8"),
          ),
        ).toBe("export const util = 1;\n");
      }).pipe(Effect.provide(layer));
    },
  );

  it.live(
    "still removes the staged container and carries the legacy-bundle suggestion when the unbundle fails",
    () => {
      vi.stubEnv("SUPABASE_USE_SLIM_IMAGES", "1");
      const child = mockSlimDockerLifecycle({ startExitCode: 1, startStderr: ["boom"] });
      const { layer } = buildSlimLayer(child);
      ...
      return Effect.gen(function* () {
        ...
        // The failure path must not leak a stopped container — the `rm` here
        // is the slim flow's replacement for `docker run --rm`.
        const createCommand = child.spawned.find((spawned) => spawned.args[0] === "create");
        const containerName = createCommand!.args[2]!;
        expect(child.spawned).toContainEqual({
          command: "docker",
        ...
        // No copy is attempted for a failed unbundle.
        expect(child.spawned.some((spawned) => spawned.args[0] === "cp")).toBe(false);
      }).pipe(Effect.provide(layer));
    },
  );

  it.live("refuses to extract an archive entry that escapes the functions directory", () => {
    vi.stubEnv("SUPABASE_USE_SLIM_IMAGES", "1");
    // Entry paths originate from an `unbundle` run over a remote eszip, so a
    // crafted path traversing above `supabase/functions` must be rejected
    ...
    return Effect.gen(function* () {
      ...
      expect(error).toBeInstanceOf(Error);
      expect((error as Error).message).toContain("refusing to extract Function file outside");
      expect(existsSync(join(tempRoot.current, "supabase", "evil.ts"))).toBe(false);
    }).pipe(Effect.provide(layer));
  });
});
