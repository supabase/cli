/**
 * Layer-exposure test for `legacyTestDbRuntimeLayer`.
 *
 ...
 * `legacyIdentityStitchLayer` is only provided to the child `dbConfig` layer
 * and NOT added to the top-level `Layer.mergeAll`, then
 * `serviceOption(LegacyIdentityStitch)` returns `None` and `test db --linked`'s
...
describe("legacyTestDbRuntimeLayer — LegacyIdentityStitch exposure", () => {
  it.live(
