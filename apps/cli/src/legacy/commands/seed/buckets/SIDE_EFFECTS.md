...
| `1`  | `auth.jwt_secret` (or `SUPABASE_AUTH_JWT_SECRET`) set but shorter than 16 characters — local only, enforced even when nothing is configured to seed; reported before an undecryptable `service_role_key`                                                                                                              |
...
- **Local env-var overrides.** For local runs, `SUPABASE_AUTH_JWT_SECRET` (if set
  ...
  rejection applies to the resolved secret (env or config value). The checks run
  in the status/stop order (jwt secret first, then service-role key) and, like
  the `[api]` checks, on the empty-config short-circuit too.
