...
resource has no pushable difference, when the resource's local gate is off,
or when the user declines the confirmation prompt.
...
| 1   | effective project config   | GET    | `/v2/projects/{ref}/config`                       | 200     | one read replaces every per-service `GET /v1/…` read the previous version made (the six endpoints listed in rows 2–7's Method/Path columns); no spinner; 401/403/404 get purpose-written messages, other statuses the generic `unexpected status N: body` shape; a partial response (missing blocks) is reported via a `Comparison scope:` line, not an error |
| 2   | api                         | PATCH  | `/v1/projects/{ref}/postgrest`                    | 200     | only when `api` has a pushable change and it's kept                                                                                                                                                        |
| 3   | db.settings                 | PUT    | `/v1/projects/{ref}/config/database/postgres`     | 200     | only when kept (this resource's own gate is "always processed", same as before)                                                                                                                            |
| 4   | db.network_restrictions     | POST   | `/v1/projects/{ref}/network-restrictions/apply`   | 201     | only when `db.network_restrictions.enabled` locally and kept; body always carries both CIDR arrays together                                                                                               |
| 5   | db.ssl_enforcement          | PUT    | `/v1/projects/{ref}/ssl-enforcement`              | 200     | only when `[db.ssl_enforcement]` is declared and kept                                                                                                                                                     |
| 6   | auth                        | PATCH  | `/v1/projects/{ref}/config/auth`                  | 2xx     | only when `auth.enabled` and kept; MFA phone/webauthn gated by addon cost prompt                                                                                                                           |
| 7   | storage                     | PATCH  | `/v1/projects/{ref}/config/storage`               | 2xx     | only when `storage.enabled` and kept                                                                                                                                                                        |
| 8   | experimental.webhooks       | POST   | `/v1/projects/{ref}/database/webhooks/enable`     | 2xx     | only if local `webhooks.enabled`; no GET/diff                                                                                                                                                              |

No `GET /v1/…` request remains anywhere in this table — row 1's single read
is every resource's source of remote state. `UpdateSigningKeys` is **not**
called by `config push`.
...
hook, each OAuth provider, and the currently-active SMS provider. A property
outside one of these groups that your file doesn't declare never appears in
any write, even when the group it belongs to ships for another reason.

Some declared properties have no v1 write path at all and are only reported,
never pushed: `db.major_version`, `db.pooler.{pool_mode,default_pool_size,max_client_conn}`,
`auth.oauth_server.{enabled,allow_dynamic_registration,authorization_url_path}` —
plus a handful of endpoint-specific cases an encoder can't structurally
express (e.g. `api.enabled = true` with no `api.schemas` declared,
`api.max_rows <= 0`, no SMS provider locally enabled). See the `Note:` lines
under Output.
...
| `1`  | effective-project-config read failure (network or unexpected status) — one failure mode now covers what used to be six independent per-service read failures |
...
`Pushing config to project: <ref>`; if the effective-configuration response omitted any
block, `Comparison scope: <present> (not returned: <missing>)` prints next. Then, per
resource, either `Remote <X> config is up to date.` or:
...
— one property block per changed property, in the same per-property format
`supabase config diff` prints (a changed secret renders as `<path> [secret]`
with no value, just a note that it differs from the remote digest; secret
values never appear in output). Experimental prints
`Enabling webhooks for project: <ref>`. Confirmations render `<title> [Y/n] `
(or `<title> [Y/n] y` when `--yes`).

After the resource loop, up to four `Note:` lines report anything the push
couldn't do:
...
Each line is omitted when its count is `0`; the leading count pluralizes the
noun that follows it. Every interpolated path/name goes through the same
control-character sanitizing `config diff` uses.
...
```jsonc
{
  ...
  "secrets": { "sent": [["auth", "captcha", "secret"]], "unchanged": [], "not_sent": [] },
  "remote_only": 12,
...
keys mirror `config.toml` paths. `project_ref`, `services[].service`, and
`services[].status` are the established contract; `services[].changes`,
`unsupported`, `unmanaged`, `secrets`, `remote_only`, and `scope` are
additive. Paths are segment arrays — a record key may itself contain a `.`.
...
- **Only declared, differing properties are written.** Each write body starts from the local file's projection (`fromConfigDocument` — the same convergence projection `config diff` compares with, ADR 0021), not the decoded config's schema defaults, so a value your file omits is never sent even when the endpoint could otherwise accept it.
