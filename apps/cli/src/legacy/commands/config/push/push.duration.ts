/**
 * Duration-string formatting and parsing for `config push`.
 *
 * `supabase/config.toml` duration fields (`auth.sessions.timebox`,
 * `auth.sms.max_frequency`, …) and the platform's own hosted duration
 * settings share one text format: an optional leading `-`, then one or more
...
/**
 * Formats a nanosecond count as a duration string.
 *
 * Rules:
 *   - 0 → "0s".
 *   - Only non-zero units are shown, but minutes/seconds always appear
 *     alongside hours when hours > 0: "1h0m0s".
 *   - Fractional seconds: "300ms", "1.5s".
 *   - The largest unit shown is hours.
 */
export function legacyDurationString(ns: number): string {
  if (ns === 0) return "0s";
...
/**
 * Converts seconds (an integer, as reported by the platform) to a duration
 * string. Used for `mfa.phone.max_frequency`, `sms.max_frequency`, and
 * `email.max_frequency`.
 */
export function legacySecondsToDurationString(secs: number): string {
  return legacyDurationString(secs * NS_PER_SECOND);
}
