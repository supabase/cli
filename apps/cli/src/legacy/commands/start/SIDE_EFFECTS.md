...
| `SUPABASE_USE_SLIM_IMAGES`                                                                                           | Ambient `process.env` only (`true`/`1` enable) — not project dotenv. Rewrites current Dockerfile pins to `ghcr.io/supabase/cli/<service>`. Kong, job images, PG14, OrioleDB, historical 15.x pins, and `deno_version = 1` stay on docker.io. Majors 13/15 use the published slim PG15 pin (`15.14.1.167`) when the flag is on; flag-off keeps `15.8.1.085`. `SUPABASE_INTERNAL_IMAGE_REGISTRY` does not apply to slim refs | no        |
