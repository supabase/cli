...
| `1`  | malformed CSV in a `--db-unban-ip` value, e.g. unterminated quote (`parse error on line 1, column 0: …`) — fails during flag parsing, before the `--experimental` gate, the handler, the `linked-project.json`/`telemetry.json` writes, and the `cli_command_executed` event |
