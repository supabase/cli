...
| `1`  | `LegacySecretsNoArgumentsError` — no positional pairs and no entries from `--env-file`                         |
| `1`  | `LegacyInvalidSecretPairError` — positional argument missing `=`                                               |
| `1`  | `LegacySecretsSetInputError` — an entry fails `V1BulkCreateSecretsInput` validation before any request is sent |
...
- Source order for merging entries: `--env-file` → CLI args (args override env-file for the same name). Explicit inputs only — `[edge_runtime.secrets]` from `config.toml` is never merged in (deliberate divergence from Go, supabase/supabase#45242).
