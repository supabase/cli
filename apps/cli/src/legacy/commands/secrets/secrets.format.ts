...
/**
 * Reproduces the byte output of `secrets list` pretty mode:
 *
 *   1. Build a markdown table source where each cell content is wrapped in
 *      backticks and any `|` in the name is markdown-escaped with `\|`.
 *   2. Pipe that through `glamour.RenderTable` (`AsciiStyle`).
 *
 ...
 * byte-identical on `secrets list` with empty, single, and pipe-containing
 * names.
 */
