...
describe("legacy workers new", () => {
  it.live("scaffolds the runtime's starter files and records the choice", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(out.stdoutText).toContain("source");
  ...
  it.live("generates a valid worker name when none is given", () => {
    ...
    return Effect.gen(function* () {
      yield* legacyWorkersNew(flags({ runtime: Option.some("deno"), size: Option.some("2gb") }));
      ...
      expect(out.stdoutText).toContain("Auto-assigned");
    }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
  });
  ...
  it.live("does not claim to reuse an entry that has nothing recorded", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(out.stdoutText).not.toContain("Reusing");
    }).pipe(Effect.provide(layer), Effect.ensuring(Effect.sync(repo.cleanup)));
  });
  ...
  it.live("rejects a config.toml that records an unknown runtime or size", () => {
    ...
    return Effect.gen(function* () {
      const runtimeError = yield* legacyWorkersNew(flags({ name: Option.some("api") })).pipe(
        Effect.flip,
      );
  ...
  it.live("rejects a config.toml that records an unknown size", () => {
    ...
    return Effect.gen(function* () {
      const sizeError = yield* legacyWorkersNew(flags({ name: Option.some("api") })).pipe(
        Effect.flip,
      );
