...
| `SUPABASE_USE_SLIM_IMAGES`                                                            | resolves the current-pin shadow Postgres and `pg_dump` images from the slim `ghcr.io/supabase/cli` builds (`true`/`1` enable); majors 13/15 use `15.14.1.167` when the flag is on; historical pins, PG14, OrioleDB, and flag-off `15.8.1.085` stay on docker.io | no        |
