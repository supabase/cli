...
import { Effect, Exit, FileSystem, Layer, Option, Path, PlatformError, Schema } from "effect";
...
const withWorkdir = <A, E, R>(
  run: (fs: FileSystem.FileSystem, path: Path.Path, workdir: string) => Effect.Effect<A, E, R>,
) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const workdir = yield* fs.makeTempDirectoryScoped({ prefix: "legacy-shadow-source-" });
...
describe("legacyLoadDeclaredSchemas", () => {
    ...
    () =>
      withWorkdir((fs, path, workdir) =>
        Effect.gen(function* () {
          ...
          yield* fs.writeFileString(
            path.join(workdir, "supabase", "custom", "x.sql"),
            "select 1;\n",
          );
          yield* fs.makeDirectory(path.join(workdir, "supabase", "database"), { recursive: true });
          yield* fs.writeFileString(
    ...
    () =>
      withWorkdir((fs, path, workdir) =>
        Effect.gen(function* () {
          ...
          yield* fs.makeDirectory(path.join(workdir, "supabase", "migrations"), {
            recursive: true,
          });
          yield* fs.writeFileString(
  ...
  it.effect(
    ...
    () =>
      withWorkdir((realFs, path, workdir) => {
          ...
          Effect.succeed({
            ...
            stat: (statPath: string) =>
              statPath === brokenAbs
                ...
                : realFs.stat(statPath),
          }),
        ).pipe(Layer.provideMerge(BunServices.layer));
        return Effect.gen(function* () {
          yield* realFs.makeDirectory(path.join(workdir, "supabase", "schemas"), {
            recursive: true,
          });
          yield* realFs.writeFileString(
            path.join(workdir, "supabase", "schemas", "a.sql"),
            "select 1;\n",
          );
      ...
      withWorkdir((fs, path, workdir) =>
        Effect.gen(function* () {
          ...
          if (Exit.isFailure(exit)) {
            const errorJson = stringifyJson(exit.cause);
    ...
    () =>
      withWorkdir((fs, path, workdir) =>
        Effect.gen(function* () {
          ...
          yield* fs.chmod(utf16First, 0o000);
          yield* fs.chmod(byteOrderFirst, 0o000);
          const exit = yield* legacyLoadDeclaredSchemas(
            fs,
            path,
            workdir,
          ...
          yield* fs.chmod(utf16First, 0o755);
          yield* fs.chmod(byteOrderFirst, 0o755);
        }).pipe(Effect.provide(BunServices.layer)),
      ),
  );
