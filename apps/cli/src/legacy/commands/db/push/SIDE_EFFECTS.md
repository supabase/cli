...
| `SUPABASE_USE_SLIM_IMAGES`         | resolves the pg-delta edge-runtime image from the slim `ghcr.io/supabase/cli/edge-runtime` build for the cache export; ambient shell only, unlike the neighboring project-`.env`-aware vars                                                                                       | no (ambient shell only)                                 |
