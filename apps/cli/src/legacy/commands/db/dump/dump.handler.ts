...
export const legacyDbDump = Effect.fn("legacy.db.dump")(function* (flags: LegacyDbDumpFlags) {
  ...
  yield* Effect.gen(function* () {
    ...
    // linked-project cache is refreshed unconditionally afterward — including
    // on a connection-resolution failure (IPv6 / pooler / login-role). The
    // resolver only returns the ref on success, so capture it up-front for the
    // linked path. `db dump` has no `--project-ref` flag, so the ref comes from
    // config.toml `project_id` then the `.temp/project-ref` file — the same
    // chain `resolveOptional`/smart generate use.
    if (connType === "linked") {
