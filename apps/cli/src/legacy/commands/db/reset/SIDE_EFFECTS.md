...
| `SUPABASE_AUTH_JWT_SECRET` / `SUPABASE_AUTH_SERVICE_ROLE_KEY`                                              | local-path bucket-seed step: override `auth.jwt_secret` / `auth.service_role_key` for the Storage service-role key, same as `seed buckets` (shell or project dotenv, `encrypted:` values decrypted)                                                                                                                                                                                                   | no                                                      |
