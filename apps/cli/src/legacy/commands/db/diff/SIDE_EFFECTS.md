...
| `~/.supabase/cache/shadow-baseline/shadow-baseline-<key>.tar`                                             | tar        | warm shadow-cache hit — the matching snapshot is streamed into the fresh shadow; every cache-eligible acquire (warm hit and successful cold export) also enumerates and `stat`s every `shadow-baseline-*.tar` for LRU keep-8 + 14-day mtime TTL and may delete other keys (`SUPABASE_HOME` overrides the `~/.supabase` root) |
