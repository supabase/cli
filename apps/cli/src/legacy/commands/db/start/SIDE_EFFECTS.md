...
| `docker image inspect` / `docker pull` / `docker tag` (registry-fallback resolve)                                                | when not already running — resolves the Postgres image; at its current Dockerfile pin, not a `.temp/postgres-version`, OrioleDB, PG13/14/15 fallback, slim, or `SUPABASE_INTERNAL_IMAGE_REGISTRY`-overridden image, it is pulled as `<candidate>@<digest>` and then `docker tag`ged as `<candidate>` |
