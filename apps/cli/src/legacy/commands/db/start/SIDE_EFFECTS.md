...
| `SUPABASE_USE_SLIM_IMAGES`                                                                                           | resolves every service image from the slim `ghcr.io/supabase/cli/<service>` builds instead of the docker.io Dockerfile pins (`true`/`1` enable); the pg 13/14/15 fallbacks and OrioleDB images stay on docker.io | no        |
