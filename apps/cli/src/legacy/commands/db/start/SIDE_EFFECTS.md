...
api.auto_expose_new_tables is deprecated and will be removed on 2026-10-30. Set it to false to
adopt the upcoming default of revoking Data API privileges on new entities in the public
schema.` on **stderr**, printed unconditionally during config load — even when Postgres
  is already running. Unset keeps the grants (the platform's current default) and stays silent.
