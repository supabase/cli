...
describe("legacy db schema declarative sync integration", () => {
  ...
  it.effect(
    "warns before writing when next sees legacy-coverage removals without a manifest",
    () => {
      seedDeclarative(tmp.current);
      const s = setup(tmp.current, {
        experimental: true,
      ...
      return Effect.gen(function* () {
        ...
        const warningAt = chunks.findIndex((chunk) =>
          chunk.includes("may have been generated by the legacy engine"),
        );
        const createdAt = chunks.findIndex((chunk) => chunk.includes("Created new migration at"));
        expect(warningAt).toBeGreaterThan(-1);
        expect(chunks[warningAt]).toContain("Extensions: pgcrypto, uuid-ossp");
        expect(chunks[warningAt]).toContain("pg_cron jobs: refresh download metrics");
  ...
  it.effect("suppresses the compatibility warning when a next export manifest is present", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(output).toContain("Found drop statements");
    }).pipe(Effect.provide(s.layer));
  });
