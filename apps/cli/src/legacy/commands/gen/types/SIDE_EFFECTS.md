...
| `SUPABASE_DB_PASSWORD`             | database password for the `--linked` workdir project                                        | no (**ignored** for `--local`; ad-hoc `--project-id` always mints a temporary login role instead of using it) |
