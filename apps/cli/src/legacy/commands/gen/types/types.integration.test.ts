...
const makeTempDirectory = (prefix: string) =>
  join("/tmp", `${prefix}${Math.abs(Effect.runSync(Random.nextInt))}`);

const writeConfig = (workdir: string, contents: string) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const supabaseDir = join(workdir, "supabase");
    yield* fs.makeDirectory(supabaseDir, { recursive: true });
    yield* fs.writeFileString(join(supabaseDir, "config.toml"), contents);
  }).pipe(Effect.provide(BunServices.layer));

const writeTempFile = (workdir: string, name: string, contents: string) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = join(workdir, "supabase", ".temp");
    yield* fs.makeDirectory(tempDir, { recursive: true });
    yield* fs.writeFileString(join(tempDir, name), contents);
  }).pipe(Effect.provide(BunServices.layer));

const removeDirectory = (workdir: string) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    yield* fs.remove(workdir, { recursive: true });
  }).pipe(Effect.provide(BunServices.layer));

const writeProjectFile = (workdir: string, name: string, contents: string) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const supabaseDir = join(workdir, "supabase");
    yield* fs.makeDirectory(supabaseDir, { recursive: true });
    yield* fs.writeFileString(join(supabaseDir, name), contents);
  }).pipe(Effect.provide(BunServices.layer));

const ensureDefaultConfig = (workdir: string) =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
...
function setup(
  opts: {
    ...
    }) => Effect.Effect<{ readonly types: string }, SupabaseApiError>;
    readonly getABranchConfig?: (input: {
      readonly branch_id_or_ref: string;
    }) => Effect.Effect<BranchConfig, SupabaseApiError>;
    readonly getPoolerConfig?: (input: {
      readonly ref: string;
    }) => Effect.Effect<PoolerConfig, SupabaseApiError>;
    readonly getProject?: (input: {
      readonly ref: string;
    }) => Effect.Effect<Project, SupabaseApiError>;
    readonly createLoginRole?: (input: {
      readonly ref: string;
      readonly read_only: boolean;
    }) => Effect.Effect<LoginRole, SupabaseApiError>;
  ...
  const workdir = opts.workdir ?? makeTempDirectory("supabase-gen-types-");
  ...
  const layer = Layer.mergeAll(
    opts.skipConfig ? Layer.empty : Layer.effectDiscard(ensureDefaultConfig(workdir)),
    runtime,
    BunServices.layer,
    ...
    ChildProcessSpawner.make((command) =>
      Effect.sync(() => {
        ...
        return ChildProcessSpawner.makeHandle({
          ...
          all: Stream.empty,
          exitCode: Effect.succeed(ChildProcessSpawner.ExitCode(step?.exitCode ?? 0)),
...
function withSslProbeServer<A, E extends Error>(
  run: (port: number) => Effect.Effect<A, E>,
  response: "N" | "S" = "N",
  options: { readonly host?: string; readonly port?: number } = {},
): Effect.Effect<A, E | SocketServer.SocketServerError | Socket.SocketError> {
  ...
  return Effect.scoped(
    Effect.gen(function* () {
        ...
        server.run((socket) =>
          Effect.scoped(
            Effect.gen(function* () {
              ...
              yield* socket.run(() =>
                writer(new TextEncoder().encode(response)).pipe(
                  Effect.andThen(writer(new Socket.CloseEvent())),
                ),
              );
            }),
          ),
      ...
      if (server.address._tag !== "TcpAddress") {
        return yield* Effect.die(new Error("failed to bind ssl probe server"));
      }
      return yield* run(server.address.port);
    }).pipe(Effect.provide(NodeSocketServer.layer({ host, port }))),
  );
}
...
describe("legacy gen types", () => {
  ...
  it.live("runs tokenless local generation through command wiring", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const workdir = makeTempDirectory("supabase-gen-types-command-local-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
  ...
  it.live(
    ...
    () => {
      const workdir = makeTempDirectory("supabase-gen-types-project-id-");
      return Effect.gen(function* () {
        yield* writeConfig(
          workdir,
  ...
  it.live(
    ...
    () => {
      const workdir = makeTempDirectory("supabase-gen-types-linked-");
      return Effect.gen(function* () {
        yield* writeConfig(
          workdir,
          ['project_id = "demo"', "", "[api]", 'schemas = ["auth", "storage"]'].join("\n"),
  ...
  it.live("allows --swift-access-control for local non-Swift generation", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const docker = captureDockerRun();
        const workdir = makeTempDirectory("supabase-gen-types-local-swift-flag-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
  ...
  it.live("allows --postgrest-v9-compat together with --db-url", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        ...
        expect(docker.env.has("PG_META_GENERATE_TYPES_DETECT_ONE_TO_ONE_RELATIONSHIPS=false")).toBe(
          true,
        );
      }),
    ),
      ...
      withSslProbeServer((port) =>
        Effect.gen(function* () {
          ...
          const { layer, out, child, api, linkedProjectCache, dbConfig } = setup({
            ...
            createLoginRole: ({ ref }) =>
              Effect.fail(
                statusApiError(500, `{"message":"unexpected login role creation for ${ref}"}`),
              ),
            onSpawn: docker.onSpawn,
          });
  ...
  it.live("resolves the linked workdir DB without ad-hoc project-ref semantics", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        ...
        yield* legacyGenTypes(defaultFlags({ linked: true, lang: "go" })).pipe(
          Effect.provide(layer),
        );
        ...
        sslProbeLayer: Layer.succeed(LegacyPgDeltaSslProbe, {
          ...
          requireSslForHost: () =>
            Effect.gen(function* () {
              probeCalls += 1;
              if (probeCalls === 1) {
                return yield* new LegacyPgDeltaSslProbeError({
  ...
  it.live("uses remote config schemas for explicit project-ref pg-meta typegen", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const workdir = makeTempDirectory("supabase-gen-types-remote-config-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "base"',
        ...
        try {
          ...
          yield* removeDirectory(workdir);
        }

        expect(docker.env.has("PG_META_GENERATE_TYPES_INCLUDED_SCHEMAS=public,private")).toBe(true);
  ...
  it.live("uses remote config schemas for linked pg-meta typegen", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const workdir = makeTempDirectory("supabase-gen-types-linked-config-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "base"',
        ...
        Effect.fail(
          new HttpClientError.HttpClientError({
            reason: new HttpClientError.TransportError({
              request: HttpClientRequest.get("https://api.supabase.test/v1/projects/ref/types"),
  ...
  it.live("spawns pg-meta for local generation and forwards child output", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const docker = captureDockerRun();
        const workdir = makeTempDirectory("supabase-gen-types-local-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
        ...
        expect(docker.env.has("PG_META_GENERATE_TYPES_INCLUDED_SCHEMAS=public,custom")).toBe(true);
  ...
  it.live("falls back to podman when the docker executable is missing for local generation", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const workdir = makeTempDirectory("supabase-gen-types-local-podman-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
  ...
  it.live("uses sanitized local docker ids and env-backed local db passwords", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const docker = captureDockerRun();
        const workdir = makeTempDirectory("supabase-gen-types-local-sanitized-");
        yield* writeConfig(
          workdir,
          [
        ...
        const { layer, child } = setup({
          workdir,
          env: { SUPABASE_DB_PASSWORD: "secret-password" },
          childStdout: ["generated"],
          onSpawn: docker.onSpawn,
        });
  ...
  it.live("forces v9 compat when rest-version reports v9 on a modern database", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const docker = captureDockerRun();
        const workdir = makeTempDirectory("supabase-gen-types-local-v9-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
  ...
  it.live("ignores rest-version v9 marker on databases older than 15", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const docker = captureDockerRun();
        const workdir = makeTempDirectory("supabase-gen-types-local-pg14-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
        ...
        expect(docker.env.has("PG_META_GENERATE_TYPES_DETECT_ONE_TO_ONE_RELATIONSHIPS=true")).toBe(
          true,
        );
      }),
    ),
  ...
  it.live("overrides the pg-meta image version from the pgmeta-version temp file", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const docker = captureDockerRun();
        const workdir = makeTempDirectory("supabase-gen-types-local-pgmeta-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
  ...
  it.live("prefers explicit --schema over config schemas for local generation", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const docker = captureDockerRun();
        const workdir = makeTempDirectory("supabase-gen-types-local-schema-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
  ...
  it.live("falls back to the workdir basename when config has no project_id", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const workdir = makeTempDirectory("supabase-gen-types-local-noid-");
        yield* writeConfig(
          workdir,
  ...
  it.live("generates from --project-id without a local project config", () => {
    const workdir = makeTempDirectory("supabase-gen-types-pid-no-config-");
  ...
  it.live("resolves the linked fallback without a local project config", () => {
    const workdir = makeTempDirectory("supabase-gen-types-fallback-no-config-");
    const { layer, api } = setup({
      workdir,
      skipConfig: true,
      projectId: Option.some(LEGACY_VALID_REF),
  ...
  it.live("fails with not-running parity when the local db container is missing", () => {
    const workdir = makeTempDirectory("supabase-gen-types-local-missing-");
    return Effect.gen(function* () {
      yield* writeConfig(
        workdir,
        [
  ...
  it.live("keeps not-running parity when podman reports the local db container is missing", () => {
    const workdir = makeTempDirectory("supabase-gen-types-local-podman-missing-");
    return Effect.gen(function* () {
      yield* writeConfig(
        workdir,
        [
  ...
  it.live(
    ...
    () => {
      const workdir = makeTempDirectory("supabase-gen-types-local-inspect-error-");
      return Effect.gen(function* () {
        yield* writeConfig(
          workdir,
          [
  ...
  it.live("generates locally with Go defaults when supabase/config.toml is missing", () => {
    const workdir = makeTempDirectory("supabase-gen-types-local-no-config-");
    const docker = captureDockerRun();
  ...
  it.live("honors local dotenv overrides when supabase/config.toml is missing", () => {
    const workdir = makeTempDirectory("supabase-gen-types-local-no-config-env-");
    const docker = captureDockerRun();
    const probes: Array<{ host: string; port: number }> = [];
    const { layer, out, child } = setup({
      workdir,
  ...
  it.live("reports a generic inspect failure when docker emits no stderr", () => {
    const workdir = makeTempDirectory("supabase-gen-types-local-empty-stderr-");
    return Effect.gen(function* () {
      yield* writeConfig(
        workdir,
        [
  ...
  it.live("defaults schemas to public for a db-url run without a project config", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const docker = captureDockerRun();
        const workdir = makeTempDirectory("supabase-gen-types-dburl-no-config-");
        const { layer } = setup({
          workdir,
          skipConfig: true,
          childStdout: ["generated"],
  ...
  it.live("surfaces pg-meta container failures after local db inspection succeeds", () => {
    return withSslProbeServer((port) =>
      Effect.gen(function* () {
        const workdir = makeTempDirectory("supabase-gen-types-local-run-error-");
        yield* writeConfig(
          workdir,
          [
            'project_id = "demo"',
  ...
  it.live("warns on stderr when SUPABASE_CA_SKIP_VERIFY is enabled", () =>
    withSslProbeServer((port) =>
      Effect.gen(function* () {
        const { layer, out } = setup({
          env: { SUPABASE_CA_SKIP_VERIFY: "true" },
          childStdout: ["generated"],
        });
  ...
  it.live("accepts legacy positional typescript without changing behavior", () => {
    ...
    return legacyGenTypes(defaultFlags()).pipe(Effect.provide(layer));
  });
