...
import { schemaPull } from "./pull.handler.ts";
...
export type SchemaPullFlags = CliCommand.Command.Config.Infer<typeof flags>;

export const schemaPullCommand = Command.make("pull", flags).pipe(
  ...
  Command.withHandler((commandFlags) =>
    schemaPull(commandFlags).pipe(
      withCommandInstrumentation({ flags: commandFlags }),
      withJsonErrorHandling,
    ),
  ),
  Command.provide(schemaRuntimeLayer(["schema", "pull"])),
);
