...
function buildLayer(opts: { cwd: string; path: Path.Path; env?: Record<string, string> }) {
  const runtimeInfoLayer = mockRuntimeInfo({
    cwd: opts.cwd,
    homeDir: opts.env?.SUPABASE_HOME
      ? opts.path.join(opts.env.SUPABASE_HOME, "..")
      : opts.path.join(opts.cwd, ".home"),
  });
  ...
  it.live("clears only cached link state and leaves project config unchanged", () => {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "supabase-unlink-command-" });
        ...
        yield* fs.writeFileString(path.join(projectRoot, "supabase", "config.toml"), initialConfig);
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        expect(out.messages).toContainEqual(
          expect.objectContaining({
            type: "success",
            message: "Local project is already unlinked.",
          }),
        );
      }),
    ).pipe(Effect.provide(BunServices.layer));
