...
function expectProfileLoadFailure(exit: Exit.Exit<unknown, unknown>, ...fragments: string[]) {
  ...
  if (Exit.isFailure(exit)) {
    ...
    const message = (error as ProfileLoadError).message;
...
beforeEach(() => {
  tempRoot = mkdtempSync(join(tmpdir(), "supabase-cli-settings-"));
});
...
describe("commandSettingsLayer", () => {
  ...
  it.effect("captures SUPABASE_WORKDIR verbatim, preserving an empty string", () =>
    Effect.gen(function* () {
      const config = yield* CommandSettings;
      expect(config.workdirEnvValue).toBe("");
    }).pipe(Effect.provide(makeLayer({ env: { SUPABASE_WORKDIR: "" }, cwd: tempRoot }))),
  );

  it.effect("captures an absent SUPABASE_WORKDIR as undefined", () =>
    Effect.gen(function* () {
      const config = yield* CommandSettings;
      expect(config.workdirEnvValue).toBeUndefined();
