...
const GO_PAYLOAD_SPEC_REGISTRY: ReadonlyArray<GoPayloadSpecEntry> = [
  {
    specName: "GO_BRANCH_RESPONSE",
    spec: GO_BRANCH_RESPONSE,
...
describe("go-payload specs vs types.gen.go (drift check)", () => {
  ...
  it.each(GO_PAYLOAD_SPEC_REGISTRY)(
    ...
    ({ spec, goTypeName }) => {
      ...
      expect(compareGoTypeToParsedGoType(spec, parsed)).toEqual([]);
    },
  );
...
`;
    ...
    const mismatches = compareGoTypeToParsedGoType(GO_SSL_ENFORCEMENT_RESPONSE, parsed);
...
`;
    ...
    const mismatches = compareGoTypeToParsedGoType(GO_BRANCH_RESPONSE, parsed);
    expect(mismatches).toContainEqual(
