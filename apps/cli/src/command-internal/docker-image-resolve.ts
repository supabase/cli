 ...
 * (`docker-run.layer.ts`) and `start`'s detached, long-running service
 * containers — the resolve/pull/retry algorithm is identical for both, only
 * the caller's process lifecycle differs.
 *
 * `projectEnvValues` is optional (see `docker-registry.ts`'s doc
 ...
 * it resolves an image; `docker-run.layer.ts` is a statically-composed
...
export function makeDockerImageResolver(
  ...
  const hasLocalImage = (image: string): Effect.Effect<boolean, DockerRunError> =>
    Effect.gen(function* () {
      ...
      // `docker-remove-all.ts`'s fuller explanation of this same
