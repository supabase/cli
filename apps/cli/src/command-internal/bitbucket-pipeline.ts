/**
 * `BITBUCKET_CLONE_DIR` — exported so `local-project-context.ts` can install it from a
 ...
 *
 * Hoisted here because it is needed by ≥2 call sites: `docker-run.layer.ts`
 * (`docker run`, e.g. `db dump`/`db test`) and `start`'s per-service container
 * creation (`command-internal/db-bootstrap/container-lifecycle.ts`).
 */
