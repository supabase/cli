 ...
 * directory (see `legacy-workdir-search.ts`), so a bare `supabase init` hint
 ...
 */
export function legacyMissingProjectConfigMessage(input: {
  ...
  return `failed to read supabase/config.toml or supabase/config.json in ${legacySanitizeInlineName(input.workdir)}: file not found. --workdir/SUPABASE_WORKDIR is used exactly as given and no ancestor directory is searched, so it must name the directory that contains your supabase/ folder.`;
}

/**
 * As {@link legacyMissingProjectConfigMessage}, but appends a
...
export const legacyMissingProjectConfigMessageEffect = Effect.fnUntraced(function* (cliSettings: {
  ...
  const base = legacyMissingProjectConfigMessage(cliSettings);
...
/**
 * Raised by {@link legacyRequireExplicitWorkdirProject} when an explicit
 ...
 */
export const legacyRequireExplicitWorkdirProject = Effect.fnUntraced(function* (cliSettings: {
  readonly workdir: string;
  readonly explicitWorkdir: boolean;
}) {
  if (!cliSettings.explicitWorkdir) {
