...
import { failsOn, makeLevelEnum } from "./fail-on.ts";

describe("makeLevelEnum (prefix matcher — db lint)", () => {
  const lint = makeLevelEnum(["warning", "error"], "prefix");
...
describe("makeLevelEnum (exact-ci matcher — db advisors)", () => {
  const advisors = makeLevelEnum(["info", "warn", "error"], "exact-ci");
...
describe("failsOn", () => {
  ...
  it("never triggers when the threshold is below 0 (fail-on none)", () => {
    expect(failsOn([{ level: "ERROR" }], (i) => i.level, -1, advisors)).toBe(false);
  });

  it("triggers when an item meets the threshold", () => {
    const items = [{ level: "WARN" }, { level: "ERROR" }];
    expect(failsOn(items, (i) => i.level, advisors.toEnum("error"), advisors)).toBe(true);
  });

  it("does not trigger when every item is below the threshold", () => {
    const items = [{ level: "WARN" }, { level: "INFO" }];
    expect(failsOn(items, (i) => i.level, advisors.toEnum("error"), advisors)).toBe(false);
  });

  it("treats an unknown item level as below every threshold", () => {
    const items = [{ level: "mystery" }];
    expect(failsOn(items, (i) => i.level, advisors.toEnum("info"), advisors)).toBe(false);
  });
});
