...
// that escape — see `pathMatch`'s escape handling below.
...
// - Walked-child construction (`walkSqlFiles`, below): `fs.WalkDir` builds
...
const globOne = (
  ...
  Effect.gen(function* () {
    ...
    // match; the follow-up `fs.Stat(fsys, fp)` in `sqlFilesGlob` below (which DOES
    ...
    // no-follow way `walkSqlFiles` below already does for a walked child: `readLink`
    ...
    for (const d of dirs) {
      ...
      for (const name of names) {
        if (pathMatch(file, name).matched) {
          ...
          // This function's own caller (`sqlFilesGlob`) already
  ...
  Effect.gen(function* () {
    ...
    const walk = (rel: string): Effect.Effect<void, string> =>
      Effect.gen(function* () {
        ...
        // matched-file stat failure below and `applySchemaFiles`'s read errors).
        const names = yield* fs
          .readDirectory(absDir)
          .pipe(
            Effect.mapError(
 ...
 * caller in this shared module needs them today (`applySchemaFiles`'s
 * `[db.migrations].schema_paths`, and the two `[db.seed].sql_paths` callers —
 * `getPendingSeeds` and `seed.ts`'s `resolveSeedFiles` — all pass none,
 ...
 * `[db.seed].sql_paths` (via `getPendingSeeds`, `seed-ops.ts`, and
 * `seed.ts`'s `resolveSeedFiles`) and `[db.migrations].schema_paths` (via
 * `applySchemaFiles`, `migration-apply.ts`) — all three call sites
 ...
 * config-read time (`resolveSeedSqlPath`), matching `config.resolve` step,
...
export const sqlFilesGlob = Effect.fnUntraced(function* (
  ...
  for (const rawPattern of patterns) {
    ...
    for (const match of [...matches].sort(utf8Compare)) {
      ...
      // `applySchemaFiles`'s read-error display path (`migration-apply.ts`).
