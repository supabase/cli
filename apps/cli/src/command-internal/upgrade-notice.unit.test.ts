...
describe("updateNotifierDisabled", () => {
  ...
  it.each([undefined, "", "0", "false", "FALSE", "f", "garbage", "yes", "on"])(
    ...
    (value) => {
      expect(updateNotifierDisabled(value)).toBe(false);
    },
  );
});
...
describe("isNewerCliVersion", () => {
  it.each([
    ...
    expect(isNewerCliVersion(latest, current)).toBe(expected);
  });
});
...
describe("runUpgradeNotice", () => {
  ...
  function setup(opts: {
    ...
    workdir = mkdtempSync(join(tmpdir(), "supabase-upgrade-notice-"));
  ...
  it("--debug=true surfaces fetch failures and --debug=false silences them, like pflag", async () => {
    ...
    await runUpgradeNotice(off.deps);
  ...
  it("a --debug operand after -- or consumed by a value flag is not the debug flag, like pflag", async () => {
    ...
    await runUpgradeNotice(afterTerminator.deps);
    ...
    await runUpgradeNotice(consumedValue.deps);
  ...
  it("project dotenv SUPABASE_DEBUG surfaces diagnostics, like godotenv before the Execute tail", async () => {
    ...
    mkdirSync(join(workdir, "elsewhere"), { recursive: true });
    symlinkSync(join(workdir, "elsewhere"), join(workdir, "supabase", ".temp"));
    await runUpgradeNotice(blocked.deps);
  ...
  it("resolves the cache under --workdir, then SUPABASE_WORKDIR, ahead of the cwd walk", async () => {
    ...
    await runUpgradeNotice(flagCtx);
    ...
    await runUpgradeNotice(lastWinsCtx);
    expect(readFileSync(join(flagDir, "supabase", ".temp", "cli-latest"), "utf8")).toBe("v2.114.0");

    const envCtx = { ...ctx.deps, env: { SUPABASE_WORKDIR: envDir } };
    await runUpgradeNotice(envCtx);
  ...
  it("uses the successful command's resolved working directory without re-resolving flags", async () => {
    ...
    await runUpgradeNotice({ ...ctx.deps, resolvedCwd: prompted });
  ...
  it("resolves --help and --version against the bare cwd, ignoring --workdir, like Go", async () => {
    ...
    for (const args of [
      ...
      await runUpgradeNotice({ ...ctx.deps, args });
    }
