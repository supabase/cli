...
/**
 * Walk a {@link GoType} spec and the parser's {@link GoParsedType} in
...
function kindToParsedKind(kind: GoType["kind"]): GoParsedKind | undefined {
  switch (kind) {
    case "string":
    case "bool":
    case "int":
...
function compareType(
  ...
  if (pointer !== parsed.pointer) {
    mismatches.push({
      path,
      message: `pointer-ness mismatch: spec says pointer=${pointer}, Go source says pointer=${parsed.pointer}`,
    });
  }
  ...
  const expectedKind = kindToParsedKind(inner.kind);
...
function compareStructFields(
  fields: ReadonlyArray<{ readonly json: string; readonly type: GoType }>,
  parsedFields: ReadonlyArray<GoParsedField>,
  path: string,
  mismatches: Array<GoStructDriftMismatch>,
): void {
  const keys = fields.map((field) => field.json);
  const parsedKeys = parsedFields.map((field) => field.json);
  if (keys.join(",") !== parsedKeys.join(",")) {
    mismatches.push({
      path,
      message: `field set/order mismatch: spec has [${keys.join(", ")}], Go source has [${parsedKeys.join(", ")}]`,
    });
  }

  const parsedByJson = new Map(parsedFields.map((field) => [field.json, field] as const));
  for (const field of fields) {
    const parsedField = parsedByJson.get(field.json);
    if (parsedField === undefined) {
      mismatches.push({ path, message: `field "${field.json}" removed from Go struct` });
      continue;
    }
    compareType(field.type, parsedField, `${path}.${field.json}`, mismatches);
  }

  const keySet = new Set(keys);
  for (const parsedField of parsedFields) {
    if (!keySet.has(parsedField.json)) {
