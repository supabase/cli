 ...
 * parent directory (reusing `spawn`/`legacyCollectText` exactly as
 * `legacy-container-cli.ts`'s `legacyContainerCliExitCodeAndStdout` does) —
 * the path argument is always the bare basename, listed after `--`, so a
 * name that happens to look like a flag is never misread as one. Git's own
 * pathspec matching already treats a directory pathspec as "anything under
 * that tree", so passing a directory here needs no special-casing: a
 * non-empty result means at least one file under it is modified or untracked.
 *
 ...
 */
export function legacyPathHasUncommittedChanges(
  path: string,
