...
describe("parseConnectionString (URL form)", () => {
  it("parses host/port/user/password/database and percent-decodes userinfo", () => {
    expect(parseConnectionString("postgres://alice:p%40ss@example.com:6543/appdb")).toEqual({
      host: "example.com",
      port: 6543,
      user: "alice",
      password: "p@ss",
  ...
  it("defaults the port to 5432 and the database to the user when both are absent", () => {
    expect(parseConnectionString("postgres://bob@example.com")).toEqual({
      host: "example.com",
      port: 5432,
      user: "bob",
      password: "",
  ...
  it("defaults user to the OS account and database to that user when both are omitted", () => {
    expect(parseConnectionString("postgresql://localhost")).toEqual({
      host: "localhost",
      port: 5432,
      user: osUser,
      password: "",
  ...
  it("omits sslmode/options keys when the query string does not set them", () => {
    const parsed = parseConnectionString("postgres://u:pw@h/db");
  ...
  it("omits runtimeParams when only structural/ssl keys are present", () => {
    const parsed = parseConnectionString("postgres://u:pw@h/db?sslmode=require");
  ...
  it("returns undefined for an unparseable URL", () => {
    expect(parseConnectionString("postgres://user:pw@ bad host/db")).toBeUndefined();
  });
  ...
  it("carries sslrootcert from the query or DSN (PGSSLROOTCERT-style CA pinning)", () => {
    expect(
      parseConnectionString("postgres://h/db?sslmode=require&sslrootcert=/ca.pem"),
    ).toMatchObject({ sslmode: "require", sslrootcert: "/ca.pem" });
    expect(parseConnectionString("host=h sslmode=verify-ca sslrootcert=/ca.pem")).toMatchObject({
      sslrootcert: "/ca.pem",
    });
  });
...
describe("parseConnectionString (libpq keyword/value DSN)", () => {
  it("parses a space-separated keyword/value DSN", () => {
    expect(parseConnectionString("host=pg.example.com port=6543 user=admin dbname=app")).toEqual({
      host: "pg.example.com",
      port: 6543,
      user: "admin",
      database: "app",
  ...
  it("honors single-quoted values with embedded spaces and backslash escapes", () => {
    const parsed = parseConnectionString("host=h dbname=db user=postgres password='se cr\\'et'");
  ...
  it("fills omitted DSN fields from PG* env vars (pgconn env defaults)", () => {
    ...
    process.env["PGDATABASE"] = "envdb";
    try {
      expect(parseConnectionString("user=admin")).toEqual({
        host: "pg.env.com",
        port: 6543,
        user: "admin",
        password: "env-secret",
  ...
  it("returns undefined when a keyword has no '=' value", () => {
    expect(parseConnectionString("host pg.example.com")).toBeUndefined();
  });

  it("returns undefined for a non-numeric port", () => {
    expect(parseConnectionString("host=h port=abc")).toBeUndefined();
  });
});
...
describe("empty-password precedence (pgconn parity)", () => {
  ...
  it("uses PGPASSWORD when the URL has no password component at all (user@host)", () => {
    expect(parseConnectionString("postgres://alice@db.example.com:6543/appdb")?.password).toBe(
      "env-secret",
    );
  });
  ...
  it("an explicit empty ?password= suppresses PGPASSWORD → .pgpass", () => {
    expect(
      parseConnectionString("postgres://alice@db.example.com:6543/appdb?password=")?.password,
    ).toBe("pgpass-secret");
  });

  it("an explicit empty DSN password= suppresses PGPASSWORD → .pgpass", () => {
    expect(
      parseConnectionString("host=db.example.com port=6543 dbname=appdb user=alice password=")
        ?.password,
    ).toBe("pgpass-secret");
  });
...
describe("multi-host failover (pgconn parity)", () => {
  ...
  it("defaults every host to the first port when no host carries one", () => {
    expect(parseConnectionString("postgres://u:pw@h1,h2,h3/db")).toMatchObject({
      host: "h1",
      port: 5432,
      fallbacks: [
        { host: "h2", port: 5432 },
  ...
  it("handles bracketed IPv6 literals in a multi-host URL", () => {
    expect(parseConnectionString("postgres://u:pw@[::1]:5432,[::2]:5433/db")).toMatchObject({
      host: "::1",
      port: 5432,
  ...
  it("keeps the query string (sslmode) intact for a multi-host URL", () => {
    expect(
      parseConnectionString("postgres://u:pw@h1:5432,h2:5433/db?sslmode=require"),
  ...
  it("parses a comma-separated multi-host DSN into primary + fallbacks", () => {
    expect(parseConnectionString("host=h1,h2 port=5432,5433 user=u dbname=db")).toMatchObject({
      host: "h1",
      port: 5432,
      fallbacks: [{ host: "h2", port: 5433 }],
    });
  ...
  it("omits fallbacks for the common single-host case", () => {
    expect(parseConnectionString("postgres://u:pw@h1:5432/db")).not.toHaveProperty("fallbacks");
  });
});
...
describe("passfile= DSN setting (pgconn parity)", () => {
  ...
  it("resolves the password from a ?passfile= URL setting over PGPASSFILE", () => {
    expect(
      parseConnectionString(`postgres://alice@db.example.com:6543/appdb?passfile=${customPath}`)
        ?.password,
  ...
  it("a present-but-empty passfile= suppresses PGPASSFILE (→ empty password)", () => {
    ...
    expect(
      parseConnectionString("host=db.example.com port=6543 dbname=appdb user=alice passfile=")
        ?.password,
    ).toBe("");
  });
});
...
describe("injected env lookup (project dotenv parity)", () => {
  ...
  it("fills omitted URL fields from the injected env (PGPASSWORD/PGSSLMODE/PGHOST)", () => {
    ...
    expect(parseConnectionString("postgresql://alice@db.example.com/appdb", env)).toEqual({
      host: "db.example.com",
      port: 5432,
      user: "alice",
      password: "dotenv-pw",
  ...
  it("uses the injected env for the keyword/value DSN form too", () => {
    ...
    const parsed = parseConnectionString("host=db.example.com user=alice", env);
...
describe("pgservice resolution (pgconn parity)", () => {
  ...
  it("resolves host/port/user/password/database/sslmode from the named service", () => {
    expect(parseConnectionString(`postgresql:///?service=prod&servicefile=${servicefile}`)).toEqual(
      {
        host: "db.example.com",
        port: 6543,
        user: "alice",
  ...
  it("resolves a service from the keyword/value DSN form too", () => {
    expect(parseConnectionString(`service=prod servicefile=${servicefile}`)).toEqual({
      host: "db.example.com",
      port: 6543,
      user: "alice",
      password: "svc-secret",
  ...
  it("resolves the service from the injected env (PGSERVICE/PGSERVICEFILE)", () => {
    ...
    expect(parseConnectionString("postgresql:///", env)?.host).toBe("db.example.com");
  });

  it("fails to parse (undefined) when the service is unknown", () => {
    expect(
      parseConnectionString(`postgresql:///?service=missing&servicefile=${servicefile}`),
    ).toBeUndefined();
  });

  it("fails to parse (undefined) when the service file does not exist", () => {
    expect(
      parseConnectionString(`postgresql:///?service=prod&servicefile=${join(tmp, "nope")}`),
    ).toBeUndefined();
  });
});
...
describe("keyword/value DSN backslash handling (pgconn parity)", () => {
  ...
  it("unescapes \\\\ and \\' inside a single-quoted value", () => {
    ...
    expect(parseConnectionString("host=h dbname=d user=u password='a\\\\b'")?.password).toBe(
      "a\\b",
    );
    expect(parseConnectionString("host=h dbname=d user=u password='it\\'s'")?.password).toBe(
      "it's",
    );
  });
  ...
  it("preserves a backslash before an ordinary char inside quotes", () => {
    expect(
      parseConnectionString("host=h dbname=d user=u sslrootcert='C:\\certs\\root.pem'")
        ?.sslrootcert,
    ).toBe("C:\\certs\\root.pem");
  });
...
describe("connect_timeout (pgconn parity)", () => {
  ...
  it("parses connect_timeout from a keyword/value DSN", () => {
    expect(parseConnectionString("host=h dbname=d user=u connect_timeout=7")).toMatchObject({
  ...
  it("falls back to PGCONNECT_TIMEOUT from the injected env", () => {
    ...
    expect(parseConnectionString("postgres://u:p@h/db", env)).toMatchObject({
  ...
  it("omits connectTimeoutSeconds when unset or zero (driver applies Go's default)", () => {
    ...
    expect(parseConnectionString("postgres://u:p@h/db?connect_timeout=0")).not.toHaveProperty(
      "connectTimeoutSeconds",
    );
  });

  it("rejects a non-numeric connect_timeout as a parse error", () => {
    expect(parseConnectionString("postgres://u:p@h/db?connect_timeout=abc")).toBeUndefined();
    expect(parseConnectionString("host=h user=u connect_timeout=abc")).toBeUndefined();
  });
});
...
describe("empty URL query overrides (pgconn parity)", () => {
  ...
  it("an empty ?user= overrides the userinfo with an empty user", () => {
    expect(parseConnectionString("postgres://alice@host/db?user=")?.user).toBe("");
  });

  it("an absent dbname/user query still falls back to path/userinfo", () => {
    expect(parseConnectionString("postgres://alice@host/realdb")).toMatchObject({
      user: "alice",
      database: "realdb",
    });
  });
...
describe("pgconn parse refinements", () => {
  ...
  it("rejects out-of-range ports (0, 65536, 70000) across query/structural/DSN/PGPORT", () => {
    expect(parseConnectionString("postgres://h/db?port=0")).toBeUndefined();
    expect(parseConnectionString("postgres://h:70000/db")).toBeUndefined();
    expect(parseConnectionString("host=h user=u port=65536")).toBeUndefined();
    const env = (name: string): string | undefined => (name === "PGPORT" ? "70000" : undefined);
    expect(parseConnectionString("host=pg.example.com user=u", env)).toBeUndefined();
  });
  ...
  it("uses the OS account (not $USER/$USERNAME) when PGUSER is empty/absent", () => {
    ...
    expect(parseConnectionString("postgres://host/db", env)?.user).not.toBe("not-the-account");
  });
...
describe("database= alias and empty service values (pgconn parity)", () => {
  ...
  it("honors a `database=` query key as an alias for dbname", () => {
    expect(parseConnectionString("postgres://host/postgres?database=prod")).toMatchObject({
      database: "prod",
    });
  });

  it("honors a `database=` keyword in the DSN form", () => {
    expect(parseConnectionString("host=pg user=u database=prod")).toMatchObject({
      database: "prod",
    });
  });
  ...
  it("uses last-wins for dbname/database aliases in a DSN (pgconn remaps at parse time)", () => {
    ...
    expect(parseConnectionString("host=h user=u database=appdb dbname=template1")).toMatchObject({
      database: "template1",
    });
  });
  ...
  it("an empty service connect_timeout= is a parse error", () => {
    const sf = join(tmp, "svc.conf");
    writeFileSync(sf, "[s]\nhost=h\nport=5432\nuser=u\nconnect_timeout=\n");
    expect(parseConnectionString(`postgres:///?service=s&servicefile=${sf}`)).toBeUndefined();
  });

  it("still uses a non-empty service value normally", () => {
    const sf = join(tmp, "svc.conf");
    writeFileSync(sf, "[s]\nhost=svc.example.com\nport=6543\nuser=alice\ndbname=appdb\n");
    expect(parseConnectionString(`postgres:///?service=s&servicefile=${sf}`)).toMatchObject({
      host: "svc.example.com",
      port: 6543,
      user: "alice",
      database: "appdb",
...
describe("more pgconn parse refinements", () => {
  ...
  it("accepts a port-only URL, falling the host back to PGHOST/default", () => {
    ...
    expect(parseConnectionString("postgres://:5433/postgres", env)).toMatchObject({
      host: "envhost",
      port: 5433,
      database: "postgres",
    });
  ...
  it("rejects a DSN with an empty key", () => {
    expect(parseConnectionString("=ignored host=prod.example.com")).toBeUndefined();
    expect(parseConnectionString("  =value host=h")).toBeUndefined();
  });
  ...
  it("treats a present-but-empty servicefile= as a parse error (overrides PGSERVICEFILE)", () => {
    ...
    expect(parseConnectionString("service=prod servicefile= host=h", env)).toBeUndefined();
    expect(
      parseConnectionString("postgres://host/db?service=prod&servicefile=", env),
    ).toBeUndefined();
  });
});
...
describe("poolerConfigFromConnectionString", () => {
