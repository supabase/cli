...
const connectFailure = (
  cfg: Partial<PgConnInput> & { readonly port: number },
  isLocal = true,
): Effect.Effect<DbConnectError> =>
  Effect.gen(function* () {
...
describe("dbConnectionSqlPgLayer extended batches", () => {
   ...
   */
  const asBatchExecError = (error: DbConnectError | DbExecError): DbExecError => {
    if (error._tag === "DbExecError") return error;
