 ...
 * {@link HealthCheckTimeoutError} (`./health-check.ts`), so the
 ...
 * and the pre-pull (`ensureImagesCached`) runs before bring-up, so a
 * `ImagePrepullError` (`./image-prepull.ts`) propagates straight out
 * and always fails the command with exit 1, with or without the flag.
 * Widening this match to accept `ImagePrepullError` would be a dead
 ...
 * (by project label) via {@link dockerRemoveAll}. A rollback failure is
 ...
 * value, this wrapper only forwards it to {@link dockerRemoveAll}.
 *
 * Also reclaims any {@link cleanupStartSecrets} staged-secret
 ...
 * containers come from {@link dockerRemoveAll}'s own
 ...
 * `CommandSettings.workdir`) is passed through only as
 * {@link cleanupStartSecrets}'s fallback — every container this same
 * `start` run just created carries its own matching `CLI_WORKDIR_LABEL`
...
export const rollbackStart = (
  ...
  Effect.gen(function* () {
    ...
    // line out of `dockerRemoveAll` and into each caller (`stop` prints
