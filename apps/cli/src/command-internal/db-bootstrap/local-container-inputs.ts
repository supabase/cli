 ...
 * project context, resolve config values + the `DbBootstrapConfig`
 ...
 * `FreshDbSetupInput` `setup` object `runFreshDbSetup` needs. Hoisted
 ...
 *    {@link LocalDbContainerInputs.postgresSpecBase}) and its `isFreshVolume`/`filterValue`
 ...
 *  - `db reset`'s resolved `version`/`seedFlags` (passed straight to `recreateLocalDatabase`,
 ...
 *    nested-env walk (`loadProjectEnv`/`resolveExperimentalWithProjectEnv`) — a
 ...
 *    {@link LocalDbContainerInputs.experimental}'s own doc comment). `db reset`'s caller
...
export interface LocalDbContainerInputs {
   ...
   * {@link LocalProjectContext.projectEnvValues} (the `@supabase/config`-backed reader) —
   ...
   * (`loadProjectEnv`), because it needs the gate before this prelude ever runs (to decide
   * Go-delegation for the remote target too) — its caller overrides {@link
   * FreshDbSetupInput.experimental} with that earlier value instead of using this field, to
...
/**
 * Builds {@link LocalDbContainerInputs} — see this module's header for the full call
 * order and for which parts are deliberately excluded (kept at each call site instead).
 *
 * Loads its own {@link LocalProjectContext} via {@link loadLocalProjectContext}
...
export const buildLocalDbContainerInputs = (
  ...
  // CLI-1956) — threaded straight through to `loadLocalProjectContext` so the shadow's
  ...
  // override, the same way `readDbToml(..., ref)` already does for those commands'
  // other config read. `db start`/`db reset` never pass this — see that function's own doc
  // comment.
  projectRef?: string,
  // The `remoteOverrideKeys` the caller's OWN, separate `readDbToml(..., ref)` read
  // already computed for the SAME matched `[remotes.<ref>]` block (`@supabase/config`'s
  // `loadCliConfig`, used by `loadLocalProjectContext` just above, merges the
  // remote block's VALUES but tracks none of which keys it set) — threaded into
  // `resolveDbBootstrapConfig`/`resolveDbSettingsEnvOverrides` AND
  // `resolveLocalConfigValues` below so a remote-set field (e.g. `db.major_version`,
  ...
  // {@link LocalProjectContext} loaded in scope BEFORE calling this function, since it
  ...
  // it AS-IS instead of calling `loadLocalProjectContext(workdir, mapError, projectRef)`
  ...
  // `loadLocalProjectContext` itself. That's only ever true today for a caller — `db
...
> =>
  Effect.gen(function* () {
    ...
    // inside `runFreshDbSetup`'s fresh-volume setup pipeline — see this field's own doc
    ...
    const setup: FreshDbSetupInput<DbConfigLoadError> = {
      ...
      // `Realtime.Enabled` (`internal/db/start/start.go:337-341`) — `runFreshDbSetup` only
