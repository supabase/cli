...
describe("pflagBoolValue", () => {
  ...
  it("accepts exactly Go's strconv.ParseBool literal set", () => {
    ...
    for (const raw of ["0", "f", "F", "FALSE", "false", "False"]) {
      expect(pflagBoolValue(occ([["f", [raw]]]), "f")).toEqual(Result.succeed(false));
    }
  });
...
describe("pflagEnumValue", () => {
  it("is none when the flag never occurs", () => {
    expect(pflagEnumValue(occ([]), "name-id-format", NAME_ID_FORMATS)).toEqual(
...
describe("pflagWorkdirValue", () => {
  ...
  it("falls back to the parsed flag when the anchored scan saw no occurrence (pre-path --workdir)", () => {
    expect(pflagWorkdirValue(scan([]), Option.some("/pre-path"), "/env")).toEqual(
  ...
  it("ignores the parsed flag when the --workdir token was consumed by another flag, falling to the env var", () => {
    ...
    expect(pflagWorkdirValue(scan([], ["workdir"]), Option.some("/x"), undefined)).toEqual(
      Option.none(),
    );
  });
  ...
  it("treats a changed-but-empty flag as the walk-up default, shadowing the env var (viper precedence)", () => {
    ...
    expect(pflagWorkdirValue(scan([]), Option.some(""), "/env")).toEqual(Option.none());
  });

  it("treats an empty env var as unset", () => {
    expect(pflagWorkdirValue(scan([]), Option.none(), "")).toEqual(Option.none());
  });
});

describe("pflagProfileValue", () => {
  const scan = (
    entries: ReadonlyArray<readonly [string, ReadonlyArray<string>]>,
    consumed: ReadonlyArray<string> = [],
    prePath: ReadonlyArray<readonly [string, ReadonlyArray<string>]> = [],
  ...
  it("falls back to the parsed flag when the anchored scan saw no occurrence (pre-path --profile)", () => {
    expect(pflagProfileValue(scan([]), Option.some("pre.yml"), "env.yml")).toEqual(
      Option.some("pre.yml"),
    );
  });
  ...
  it("ignores the parsed flag when the --profile token was consumed by another flag, falling to the env var", () => {
    ...
    expect(
      pflagProfileValue(scan([], ["profile"]), Option.some("alternate.yml"), undefined),
    ).toEqual(Option.none());
  });

  it("uses the env var when neither the scan nor the parser saw the flag", () => {
    expect(pflagProfileValue(scan([]), Option.none(), "env.yml")).toEqual(Option.some("env.yml"));
  });

  it("treats an empty env var as unset", () => {
    expect(pflagProfileValue(scan([]), Option.none(), "")).toEqual(Option.none());
  });
});

describe("resolvePflagProfile", () => {
  ...
  it.effect(
    ...
    () => {
      ...
      return withEnvProfile(
        profilePath,
        Effect.gen(function* () {
          const resolved = yield* resolvePflagProfile({
            occurrences: new Map(),
