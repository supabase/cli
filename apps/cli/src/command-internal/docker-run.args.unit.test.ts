...
describe("applyBitbucketDockerFilter", () => {
  const pgDelta: DockerRunOpts = {
    ...base,
...
describe("buildDockerArgs", () => {
  ...
  test("uses --network host for the host network", () => {
    const args = buildDockerArgs({ ...base, network: { _tag: "host" } });
  ...
  test("omits --entrypoint when none/absent (pg_dump / pg_prove keep their entrypoint)", () => {
    expect(buildDockerArgs(base)).not.toContain("--entrypoint");
    expect(buildDockerArgs({ ...base, entrypoint: Option.none() })).not.toContain("--entrypoint");
  });
