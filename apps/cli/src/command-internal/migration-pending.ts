...
/**
 * Local migration filenames are `<digits>_<name>.sql`; callers guarantee every path in
 * `localMigrations` matches, so the version capture group is always present.
 */
...
/**
 * The outcome of comparing local migration files against the remote `schema_migrations`
 * history.
 *
 * - `ok` — `pending` are the local migration paths to apply (those beyond the remote history,
 *   in order).
 * - `missing-local` — remote has versions with no local file. `versions` are the offending
 *   remote versions.
 * - `missing-remote` — local has files ordered before the remote head. `paths` are the
 *   out-of-order local migration paths.
 */
...
/**
 * Two-pointer reconciliation of local migration paths vs remote applied versions, comparing
 * versions as strings (`remote == local` / `remote < local`). Both sides must agree on ordering,
 * so `localMigrations` is re-sorted by version.
 */
export function findPendingMigrations(
  ...
  while (i < remoteMigrations.length && j < sortedLocal.length) {
    ...
    // Callers guarantee a match, so the capture group is present.
...
/**
 * Computes the `--include-all` pending set when reconciliation reports `missing-remote`: the
 * out-of-order paths first, then the local migrations beyond `remoteCount + diff.length`.
 */
