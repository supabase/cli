 ...
 * `seed.ts` (the first caller, `[db.seed] sql_paths`); hoisted here once
 * `migrate-and-seed.ts`'s declarative-schema-files branch (`[db.migrations]
...
// `foo\.sql`) is globbed via `pathMatch` (which handles the escape) instead of being
...
// level up from `globPattern`'s own slash-split below — which is still part of the SAME
 ...
 * via `pathMatch` (Go's `path.Match`). The caller validates the whole pattern up
 * front (`pathMatch(pattern, "").badPattern`), so a malformed class never reaches
 * here.
 */
export const globPattern = (
  ...
  Effect.gen(function* () {
    ...
    // backslashes — an absolute one is preserved verbatim by `resolveSeedSqlPath`, but
    // even a relative one can carry them — never matches the `/`-only split below and
    // `hasGlobMeta` misreads a plain backslash as glob syntax, so a literal path like
 ...
 * across `shadow-source.ts`/`migration-list.ts`) ports, so a directory with such
 ...
 * every directory match) AND `afero.Walk` (`commands/db/shared/shadow-source.ts`'s
 ...
 * {@link compareUtf8Bytes} too, not JS's default UTF-16-code-unit comparator (review:
 ...
 *
 * Hoisted here (from `migrate-and-seed.ts`, the first caller, for `db.migrations.
 * schema_paths`) once `seed.ts`'s `db.seed.sql_paths` resolution became a second
 * caller, and `shadow-source.ts`'s declared-schema walkers (a matched `schema_paths`
