 ...
 * (`get-api-keys.ts`), which ports `apiKeys.RunGetApiKeys` and maps a
 * non-200 to `unexpected get api keys status ...`.
 *
 * Parameterized on the caller's tagged-error classes so `link` and `seed` keep
 * their own `Link*` / `Seed*` error tags while sharing the message
 * shape and the truncation / classification policy in `mapHttpError`.
 */
