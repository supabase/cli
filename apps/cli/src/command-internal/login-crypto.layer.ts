...
export const loginCryptoLayer = Layer.sync(LoginCrypto, () =>
  LoginCrypto.of({
    ...
    decryptToken: (ecdh: ECDH, payload: LoginEncryptedPayload) =>
      Effect.try({
        try: () => {
          ...
          new LoginDecryptError({ message: `${DECRYPTION_ERROR_MSG}: ${String(cause)}` }),
      }),
  }),
);
