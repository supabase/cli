 ...
 *    `legacy-cli-settings.layer.ts`) only probes `supabase/config.toml` — a
 ...
 *    `legacy-local-project-context.ts`, `gen.signing-keys-config.ts`,
 ...
 *    must create the entry there), and `legacySeedBucketsRun`'s own load,
 ...
 *    at all.
 */
export function legacyShouldSearchAncestors(cliSettings: {
