...
/**
 * Per-family error mapping for {@link resolveBranchProjectRef}: each
 * caller keeps its own tagged error classes (built with `mapHttpError`)
 * so error identities, messages, and actionability stay family-owned.
 */
export interface BranchRefResolveMappers<EGet, EFind> {
