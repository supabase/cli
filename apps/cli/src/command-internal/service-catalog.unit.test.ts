...
import { serviceContainerIds, localDbContainerId } from "./docker-ids.ts";
import { SERVICE_CATALOG } from "./service-catalog.ts";

describe("SERVICE_CATALOG", () => {
  it("has exactly 13 excludable entries and 1 non-excludable entry (Postgres)", () => {
    const withExcludeKey = SERVICE_CATALOG.filter((entry) => entry.excludeKey !== undefined);
    const withoutExcludeKey = SERVICE_CATALOG.filter((entry) => entry.excludeKey === undefined);
  ...
  it("has no duplicate containerSuffix, excludeKey, or startOrder values", () => {
    const suffixes = SERVICE_CATALOG.map((entry) => entry.containerSuffix);
    const excludeKeys = SERVICE_CATALOG.filter((entry) => entry.excludeKey !== undefined).map(
      (entry) => entry.excludeKey,
    );
    const startOrders = SERVICE_CATALOG.map((entry) => entry.startOrder);
  ...
  it("uses startOrder values 1-14 with no gaps", () => {
    const startOrders = SERVICE_CATALOG.map((entry) => entry.startOrder).sort((a, b) => a - b);
  ...
  it("has no duplicate service keys", () => {
    const services = SERVICE_CATALOG.map((entry) => entry.service);
  ...
  it("every non-Postgres containerSuffix matches a serviceContainerIds suffix", () => {
    // serviceContainerIds bakes its suffixes into the returned container
  ...
  it("Postgres's containerSuffix matches localDbContainerId's suffix", () => {
    const projectId = "catalog-cross-check";
    const postgresEntry = SERVICE_CATALOG.find((entry) => entry.service === "postgres");
