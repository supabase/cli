...
export class MigrationDropError extends Data.TaggedError("MigrationDropError")<{
 ...
 */
export const dropUserSchemas = (session: DbSession) =>
  session
    .exec(dropObjectsSql)
    .pipe(Effect.mapError((cause) => new MigrationDropError({ message: cause.message })));
