...
export interface DockerRunOpts {
  ...
  /**
   * Skips this layer's own image resolution (`makeDockerImageResolver`) when the
   * caller already resolved `image` itself through a `projectEnvValues`-aware path (e.g.
   * `start`'s one-shot fresh-DB setup jobs, resolved via `ensureImagesCached` before
   * `startSetupLocalDatabase` ever calls this service). This layer's own resolver is
   * built once, statically, with no `projectEnvValues` in scope (see
   * `docker-run.layer.ts`'s header) — re-resolving an ALREADY-resolved image (e.g.
