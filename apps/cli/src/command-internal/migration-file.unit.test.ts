...
describe("parseMigrationContent", () => {
  it.each([
    ...
    expect(parseMigrationContent(content)).toEqual({
  ...
  it("defaults an ordinary migration to transactional execution", () => {
    expect(parseMigrationContent("CREATE TABLE example (id bigint);")).toEqual({
...
describe("formatMigrationTimestamp", () => {
  it("formats epoch millis as UTC YYYYMMDDHHMMSS", () => {
    ...
    expect(formatMigrationTimestamp(millis)).toBe("20260618090807");
  });

  it("zero-pads single-digit components", () => {
    const millis = Date.UTC(2001, 0, 2, 3, 4, 5);
    expect(formatMigrationTimestamp(millis)).toBe("20010102030405");
  });
});
...
describe("getMigrationPath", () => {
  it("builds <workdir>/supabase/migrations/<ts>_<name>.sql", () => {
    ...
    expect(getMigrationPath(posix, "/repo", "20260618090807", "remote_schema")).toBe(
