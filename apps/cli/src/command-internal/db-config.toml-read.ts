...
export interface DbTomlValues {
   ...
   * declarative branch (`applySchemaFiles`) — consumed by `migrateAndSeed`
   ...
   * resolution form. `prepareShadowSource`'s `schemaPaths` input (`db diff`/`db pull`'s
   * shadow-provisioning prelude) does that join itself (`resolveSeedSqlPath`), so it
   ...
   * {@link RemoteOverride.remoteOverrideKeys}'s own doc comment for the full
   * precedence rationale. Exposed here (in addition to being used internally, above) so a
   * caller resolving a SEPARATE config read for the same linked ref — `buildLocalDbContainerInputs`,
...
interface RemoteOverride {
   ...
   * every key in `ENV_OVERRIDABLE_KEYS` the matched block supplies, plus
 ...
 * instead — see `resolveValidatedRemoteProjectId`.
 */
...
const ENV_OVERRIDABLE_KEYS = [
  ...
  // `blockProvidesKey` check below correctly finds the key absent from the block, so
  ...
  // `resolveLocalConfigValues`'s `envOverrideBool("SUPABASE_API_ENABLED", ...)`
  ...
  // `resolveDbBootstrapConfig`/`resolveDbSettingsEnvOverrides`
  // (`command-internal/db-bootstrap/`) resolve for the shadow's own container spec on the
  ...
  // Not read by `resolveDbBootstrapConfig`/`resolveDbSettingsEnvOverrides` above —
  // these feed `resolveLocalConfigValues`'s OWN fields instead (`apiUrl`/`dbUrl`/
  ...
  "api.tls.enabled",
  // Not read by `resolveDbBootstrapConfig`/`resolveDbSettingsEnvOverrides` above,
  // same as `api.tls.enabled`/`api.port` — these feed `resolveLocalConfigValues`'s own
  ...
  // Not read by ANY of the resolvers above — these feed `resolveLocalJwks`'s/
  // `resolveAuthExternalUrl`'s/`resolveConfiguredSigningKeys`'s own fields
  ...
  // `envOverride`/comma-split string reads in `resolveLocalConfigValues` — same
  ...
  // fields the doc comment on `resolveLocalConfigValues`'s `remoteOverrideKeys` parameter
  // used to claim were safe to leave ungated. That claim rested on "their own `envOverride*`
  // calls cannot throw before a value the caller needs has already been resolved" — which doesn't
  // actually hold: `resolveLocalConfigValues` is a single synchronous function that either
  ...
  // function. Every dotted key below resolves through `envOverrideBool`/`envOverrideUint`/
  // `envOverrideAuthPasswordRequirements`, all of which throw on a malformed override — same
  ...
  // reads `resolveLocalConfigValues` resolves for its `passkey` validation input — same
  ...
  // `auth.mfa.phone.template`/`.max_frequency` are plain, non-throwing `envOverride` string
  // reads in `resolveAuthMfa` — same "non-throwing read is still a precedence bug"
  ...
  // `auth.captcha.provider` can't throw on its own (`envOverride` is a plain string read),
  // but `validateResolvedConfig`'s enum check (`config-validate.ts`) rejects any
  ...
  // `validateResolvedConfig` aborts the whole synchronous `resolveLocalConfigValues`
  ...
  // way `auth.email.smtp.pass` below is — `resolveAuthCaptcha`'s ungated `envOverride`
  ...
  // `SUPABASE_AUTH_EMAIL_SMTP_PASS` never reaches decryption at all. `resolveAuthEmailSmtp`
  // previously ran `envOverride` unconditionally before decrypting, so that same malformed
  // env value could win over a matched remote's valid `pass` and throw, aborting the whole
  // synchronous `resolveLocalConfigValues` call — same bug class as `.enabled`/`.port`
  ...
  // `envOverride` string reads in `resolveAuthEmailSmtp` — unlike `.enabled`/`.port`/
  ...
  // Not read by THIS reader's own resolved fields — tracked so `resolveAuthEmail`
  // (`local-config-values.ts`) also gates its own throw-capable
  // `envOverrideBool`/`envOverrideUint` calls for these `auth.email.*` scalars,
  ...
  // `auth.email.max_frequency` is a plain, non-throwing `envOverride` string read in
  // `resolveAuthEmail` — same "non-throwing read is still a precedence bug" reasoning as
  // `auth.email.smtp.host` above.
  "auth.email.max_frequency",
  // `auth.sms.*` (`resolveAuthSms`) has the identical "throws before a value the caller
  // needs is resolved" bug class as every other group above: `enable_signup`/`enable_confirmations`
  // and each provider's `enabled` run an UNGATED `envOverrideBool`, and each provider's
  // Secret-typed field (`auth_token`/`access_key`/`api_key`/`api_secret`) runs an UNGATED
  // `decryptAuthSecret` — either can throw on a malformed
  // ambient `SUPABASE_AUTH_SMS_*` override even when a matched remote block already set that field
  // at viper's OVERRIDE tier, aborting the whole `resolveLocalConfigValues` call (and the
  // shadow it feeds via `buildLocalDbContainerInputs`) — reachable via `validateAuthSmsProviders`,
  ...
  // The remaining `auth.sms.<provider>.*` fields (`resolveField` in `resolveAuthSms`) are
  // plain, non-throwing `envOverride` string reads — `vonage.api_key` sitting right next to
  ...
  // `decryptAuthSecret` — but were never added to this allowlist when `anon_key`/
  // `service_role_key` (their sibling API-key pair, right next to them in
  // `resolveLocalConfigValues`'s return block) were gated. Same bug class: an ungated
  ...
  // `studio.api_url` is validated with `goUrlParse` inside `validateResolvedConfig`
  // (gated on `studio.enabled`, matching `studio.port` above) — a plain, non-throwing
  // `envOverride` read here can still flip that downstream validate() outcome, same
 ...
 * `[auth.external.<name>]` table declares (`resolveAuthExternalProviders`'s own doc
 * comment). A fixed `ENV_OVERRIDABLE_KEYS` entry per provider can't cover every possible
 ...
 * arbitrarily-keyed map as `auth.external.<name>` above — a fixed `ENV_OVERRIDABLE_KEYS`
 ...
 * stale/missing ambient `_CONTENT_PATH` env var makes {@link resolveAuthEmail}'s caller-side
...
/**
 * Every literal member of {@link ENV_OVERRIDABLE_KEYS}, PLUS the dotted-key patterns for
 ...
 * {@link AUTH_EXTERNAL_PROVIDER_FIELDS}/{@link AUTH_EMAIL_TEMPLATE_FIELDS}/
 * {@link AUTH_EMAIL_NOTIFICATION_FIELDS}'s own doc comments). Every
 * `remoteWins(...)`/`remoteOverrideKeys.has(...)` call site across this module,
 * `local-config-values.ts`, and `db-bootstrap/bootstrap-config.ts` is typed against this
 * union (via {@link makeRemoteWins}), so a typo'd dotted key is a compile error instead of a
 ...
 * `local-config-values.ts`, once in `db-bootstrap/bootstrap-config.ts`) into a single
 * helper. The returned function's parameter is typed as {@link RemoteOverridableKey} —
...
function applyRemoteOverride(
  ...
  for (const name of Object.keys(remotes)) {
    ...
    if (resolveRemoteProjectId(name, block, lookup) === ref) {
      ...
      // `AUTH_EXTERNAL_PROVIDER_FIELDS`'s own doc comment), so flatten whichever provider
      // names/fields THIS matched block actually supplies instead of relying on a fixed list —
      // same per-leaf override-tier semantics as `ENV_OVERRIDABLE_KEYS` above, just
      ...
      // `AUTH_EMAIL_TEMPLATE_FIELDS`'s own doc comment.
 ...
 * `SUPABASE_INTERNAL_IMAGE_REGISTRY` (`getRegistryImageUrl`). Go's
 ...
 * Everything else is read from {@link loadProjectEnv}'s returned map
 * (`envOverride`, `resolveYesWithProjectEnv`, `resolveDbPassword`) or resolved
 ...
 * (`resolveYesWithProjectEnv`, `resolveDbPassword`) never mutate the global
 * environment. Commands that need an allowlisted key visible to a synchronous
 * `process.env` reader (`db dump` / `db pull` → `getRegistryImageUrl`) opt
 * into {@link applyProjectEnv} around the container work instead.
 *
...
/**
 * Apply the allowlisted project-`.env` keys (see {@link PROCESS_ENV_APPLY_KEYS})
 * to `process.env` **for the duration of the current scope**, then revert. This is
 * the opt-in counterpart to the pure {@link loadProjectEnv}: `bootstrap` /
 * `db push` / `db pull` / `db dump` run it around their pg_dump / migration
 * container work so a `SUPABASE_INTERNAL_IMAGE_REGISTRY` set
 * only in `supabase/.env` still reaches `getRegistryImageUrl` (which reads
 ...
 * accepted bool-spelling set (`parseGoBool`) and a malformed value
 ...
 * ({@link readDbToml}'s `vault` loop) with the same
 ...
 * `decryptSecret` call, same `failed to parse config: <cause>`
 * message). For `config push` — the other caller of
 * {@link assertDecryptableSecrets}, which has no such downstream vault
 ...
 * `Secret` field paths ({@link SECRET_PATHS}) are scanned — a non-secret string that
...
const readDbTomlCore = Effect.fnUntraced(function* (
  ...
  // regardless of a config file). The lenient `readDbToml({ validate: false })`
  ...
  // (`isLocalDbRunning`'s best-effort `projectId` probe,
  // `startSetupLocalDatabase`'s own accepted duplicate config-load pass —
  ...
  // in which case this stays `false` — see `ENV_OVERRIDABLE_KEYS`'s own doc
  ...
  // for `--local` connections, `db-config.layer.ts`). `DB_PASSWORD` is read only
  // by linked password resolution (`db-config.layer.ts`), so the local password
  ...
  // `validateResolvedConfig`'s `db.major_version` switch (called once, below) — an
  ...
  // than 1/2 → invalid) is `validateResolvedConfig`'s `edgeRuntimeDenoVersion`
  ...
  // `webhooksPresent`/`webhooksEnabled` feed `validateResolvedConfig`'s existing
  // `experimental.webhooks` check (`config-validate.ts:676-680`) — this D pipeline never
  ...
  // `resolveLocalConfigValues`, which already computes the identical pair from its own
  ...
  // (`expandEnv` is a no-op on a non-`env()` string).
  ...
  // Rejecting a non-empty, non-JSON `format_options` is `validateResolvedConfig`'s
  ...
  // default later). Validate-only: `seedBucketsRun`'s own call re-parses the real value.
  ...
  // `ENV_OVERRIDABLE_KEYS` entry gets below.
  ...
  // Local helpers mirroring the deleted `validateAuthConfig`'s closures — its Go-parity
  // CHECKS now live in `validateResolvedConfig`; `str`/`gate`/`fail` are still needed here
  // to build that call's `AuthInput`, and by the D-only sms/external checks below (never
  // part of the shared validator — see `config-validate.ts`'s module header).
  ...
  if (authEnabled) {
    ...
    if (passkeyRaw !== undefined && (yield* gate(passkeyRaw, "enabled", "auth.passkey.enabled"))) {
      ...
      // Matches `local-config-values.ts`'s own `resolveGotruePasskeyWebauthn`/
      // `strToArr` handling of this identical field.
  ...
  // `ENV_OVERRIDABLE_KEYS` field above.
  ...
  // Every PURE Config.Validate check this module/config-validate.ts jointly own (db.port
  ...
  // `config-validate.ts`'s module header for the accepted ordering tradeoff this
  ...
  if (authEnabled) {
    ...
    // `config-validate.ts`'s module header). Only the FIRST enabled provider is
    // validated.
  ...
  // `globArrayIssues`'s doc comment) — never called per-field, so a config
  ...
  // ELEMENT is (see `weakCoerceGlobEntry` above): a zero-length map
  ...
  // unconvertible and aborts the whole load (see `goUnconvertibleType` above).
  ...
  // as `globArrayIssues`: the caller combines issues across both `Glob` fields
   ...
   * `failOnGlobIssues`'s doc comment for why the two `Glob` fields must combine
  ...
  // The whole config decodes in ONE pass (see `globArrayIssues`'s doc comment) —
...
export const checkDbToml = (
  ...
  // caller known to run AFTER an earlier, same-invocation `checkDbToml`/
  // `readDbToml` call already printed the OrioleDB S3 `assertEnvLoaded` WARN
 ...
 * {@link checkDbToml}); pass `{ validate: false }` for a best-effort read that
