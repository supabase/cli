...
describe("EdgeRuntimeScriptError actionability", () => {
  it("classifies a docker-daemon failure as docker-not-running", () => {
    const result = classifyCliErrorActionability(
      new EdgeRuntimeScriptError({ message: "error diffing schema: ...", docker: "daemon" }),
    );
    ...
    expect(result.error_fingerprint).toBe("tag:EdgeRuntimeScriptError:docker_not_running");
  });

  it("classifies a registry-pull failure as an external network problem", () => {
    const result = classifyCliErrorActionability(
      new EdgeRuntimeScriptError({ message: "error diffing schema: ...", docker: "pull" }),
    );
    expect(result.error_kind).toBe("external_service");
    expect(result.error_category).toBe("network");
    expect(result.error_fingerprint).toBe("tag:EdgeRuntimeScriptError:registry_pull");
  });

  it("classifies an image-inspect failure as invalid config", () => {
    const result = classifyCliErrorActionability(
      new EdgeRuntimeScriptError({ message: "error diffing schema: ...", docker: "inspect" }),
    );
    expect(result.error_kind).toBe("user_actionable");
    expect(result.error_category).toBe("invalid_config");
    expect(result.error_fingerprint).toBe("tag:EdgeRuntimeScriptError:image_inspect");
  });

  it("classifies a non-docker script failure as a user db finding", () => {
    const result = classifyCliErrorActionability(
      new EdgeRuntimeScriptError({ message: "error diffing schema: exit 1:\n..." }),
    );
    expect(result.error_kind).toBe("user_actionable");
    expect(result.error_category).toBe("invalid_config");
    expect(result.has_suggestion).toBe(false);
    expect(result.error_fingerprint).toBe("tag:EdgeRuntimeScriptError");
  });
});
