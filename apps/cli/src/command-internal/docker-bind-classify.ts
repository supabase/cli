 ...
 * this classification — `docker-run.args.ts` (`docker run`) and
 * `command-internal/db-bootstrap/docker-create-args.ts` (`docker create`) — shares one
 ...
 * truncates that down to `"C"`, which {@link isBindMountSource} then misclassifies as a
