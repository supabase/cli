...
type RouteHandler = StubResponse | ReadonlyArray<StubResponse>;
...
function mockNotebooksHttp(routes: NotebooksHttpRoutes) {
...
function notebookMetadata(options: { readonly id: string; readonly name: string }) {
  return {
    type: "notebook",
    id: options.id,
    attributes: {
