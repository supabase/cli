...
export function mockContainerCliSpawner(route: (args: ReadonlyArray<string>) => RouteResult) {
...
export interface DefaultLocalResetRouteOpts {
...
/**
 * A happy-path Docker CLI route for `resetLocalDatabase`'s PG15+
 * container-recreate — everything succeeds (running, healthy, no restart
 * failures) unless overridden. `projectId` must match the `CommandSettings`
 ...
 */
export function defaultLocalResetRoute(projectId = "test", opts: DefaultLocalResetRouteOpts = {}) {
...
export const localResetCreateArgs = (
  spawned: ReadonlyArray<SpawnRecord>,
...
export const localResetRemovedContainers = (
  spawned: ReadonlyArray<SpawnRecord>,
 ...
 * `awaitStorageReady`'s static `HttpClient.HttpClient` requirement without
