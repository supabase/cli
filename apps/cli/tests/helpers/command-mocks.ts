...
import type {
  DbBatchStatement,
  DbSession,
} from "../../src/command-internal/db-connection.service.ts";
import {
...
import {
  LoginApi,
  type LoginApiSessionResponse,
} from "../../src/commands/login/login-api.service.ts";
import { LoginCrypto } from "../../src/commands/login/login-crypto.service.ts";
import {
  LoginCryptoError,
  LoginDecryptError,
...
// Default CommandCredentials mock. `mockCommandSettings` defaults to an env-set
...
/**
 * Tracked `CommandCredentials` mock for `unlink` / `login` / `logout` tests.
 *
 * - `deleteProjectCredential` (unlink): records refs in `deletedRefs`; `deleteFails`
 *   makes it raise `CredentialDeleteError`.
 * - `saveAccessToken` (login): records the saved token in `savedToken`; `saveFails`
 *   raises `InvalidAccessTokenError` (the token-path "cannot save" branch).
 * - `deleteAccessToken` (logout): `deleteOutcome` selects success (`"ok"`),
 *   `NotLoggedInError` (`"notLoggedIn"`), or `DeleteTokenError`
 *   (`"deleteError"`).
 * - `deleteAllProjectCredentials` (logout): flips `deletedAll`.
 */
export function mockCommandCredentialsTracked(
  opts: {
  ...
  readonly layer: Layer.Layer<CommandCredentials>;
  ...
  const deleteAccessToken =
    opts.deleteOutcome === "notLoggedIn"
      ? Effect.fail(new NotLoggedInError({ message: "You were not logged in, nothing to do." }))
...
): { readonly layer: Layer.Layer<LoginCrypto> } {
  const layer = Layer.succeed(LoginCrypto, {
    generateKeyPair: opts.keygenFails
      ? Effect.fail(new LoginCryptoError({ message: "cannot generate crypto keys: boom" }))
      : Effect.succeed({
...
export function mockLoginApi(
  opts: {
    readonly sessionResponse?: Partial<LoginApiSessionResponse>;
  ...
  readonly layer: Layer.Layer<LoginApi>;
  ...
  const layer = Layer.succeed(LoginApi, {
    fetchLoginSession: () => {
      loginCallCount += 1;
      if (loginCallCount <= failTimes) {
        return Effect.fail(new LoginVerificationError({ message: "Error status 404: not found" }));
      }
...
export function mockTelemetryStateTracked(): {
  ...
  const layer = Layer.succeed(TelemetryState, {
    get flush() {
      return Effect.sync(() => {
        flushed = true;
        flushCount += 1;
...
export function mockLinkedProjectCacheTracked(): {
  readonly layer: Layer.Layer<LinkedProjectCache>;
  readonly cached: boolean;
  /** Number of `cache` calls — see {@link mockTelemetryStateTracked}'s own `flushCount`. */
  ...
  const layer = Layer.succeed(LinkedProjectCache, {
    cache: (
      ref: string,
      _workdir?: string,
      apiUrl?: string,
...
export function mockCommandSettings(opts: {
  ...
  return Layer.succeed(CommandSettings, {
    ...
    accessToken: opts.accessToken ?? Option.some(Redacted.make(VALID_TOKEN)),
 ...
 */
export function statusCodeFailure(status: number): HttpClientError.HttpClientError {
...
export interface MockCommandPlatformApiOpts {
...
export interface MockCommandPlatformApiResult {
  ...
  // Wraps `layer` in a `CommandPlatformApiFactory` for commands that switched
  // from yielding `CommandPlatformApi` directly to the lazy factory shape.
  readonly factoryLayer: Layer.Layer<CommandPlatformApiFactory>;
}

export function mockCommandPlatformApi(
  opts: MockCommandPlatformApiOpts = {},
): MockCommandPlatformApiResult {
  ...
  const handler = (request: HttpClientRequest.HttpClientRequest) =>
    Effect.gen(function* () {
      ...
      const methodResponse = opts.byMethod?.[request.method as HttpMethod];
      if (methodResponse !== undefined) {
        return jsonResponse(request, methodResponse.status, methodResponse.body);
      }
      if (opts.response !== undefined) {
        return jsonResponse(request, opts.response.status, opts.response.body);
      }
  ...
  const httpClientLayer = makeHttpClientLayer(handler);

  const layer = Layer.effect(
    CommandPlatformApi,
    makeApiClient({
      baseUrl: opts.apiUrl ?? DEFAULT_API_URL,
      accessToken: opts.accessToken ?? VALID_TOKEN,
      userAgent: opts.userAgent ?? DEFAULT_USER_AGENT,
    }),
  ).pipe(Layer.provide(httpClientLayer));

  const factoryLayer = Layer.succeed(CommandPlatformApiFactory, {
    make: CommandPlatformApi.pipe(Effect.provide(layer)),
  });
...
export interface MockCommandPlatformApiServiceOpts {
  readonly v1?: V1Stubs;
  /**
   ...
   * `HttpClientResponse`, exactly like `jsonResponse` would build for
   ...
   */
  readonly raw?: Readonly<Record<string, ApiResponse | "fail">>;
}

export interface MockCommandPlatformApiServiceResult {
  readonly layer: Layer.Layer<CommandPlatformApi>;
  readonly requests: ReadonlyArray<{ readonly method: string; readonly input: unknown }>;
}

export function mockCommandPlatformApiService(
  opts: MockCommandPlatformApiServiceOpts = {},
): MockCommandPlatformApiServiceResult {
      ...
      return (input: unknown) =>
        Effect.gen(function* () {
          ...
          const stub = (stubs as Record<string, unknown>)[prop] as
            ...
            | undefined;
          if (stub === undefined) {
            return yield* Effect.die(`Unmocked CommandPlatformApi.v1.${prop}`);
          }
  ...
  const executeRaw: ApiClient["executeRaw"] = (definition, input) =>
    Effect.gen(function* () {
      ...
      if (stub === undefined) {
        return yield* Effect.die(`Unmocked CommandPlatformApi.executeRaw.${definition.id}`);
      }
      ...
      return jsonResponse(request, stub.status, stub.body);
    });

  const layer = Layer.succeed(CommandPlatformApi, { v1: v1Proxy, v2: v2Proxy, executeRaw });

  return { layer, requests };
}

/**
 * A `DbSession.execBatch` fake for mocks that only model statement-level
 ...
 */
export function sequentialExecBatch(
  session: Pick<DbSession, "exec" | "query">,
): (statements: ReadonlyArray<DbBatchStatement>) => Effect.Effect<void, DbExecError> {
  return (statements) =>
...
export function useTempWorkdir(prefix = "supabase-test-"): {
  ...
  return {
    get current() {
      if (root === undefined) {
        throw new Error(
          "useTempWorkdir().current accessed outside an active test — call it inside it.live(...) or it(...)",
        );
      }
      return root;
    },
 ...
 *
 * Point `homeDir` at a per-test temp dir (see {@link useTempWorkdir});
 ...
 */
export function isolatedHomeLayer(
  homeDir: string,
...
export function failWriteStringOnNthCallFsLayer(
  ...
  return failWriteStringFsLayer((_, calls) => calls === failOnCall);
}

/**
 * Same as {@link failWriteStringOnNthCallFsLayer}, but fails the first
 ...
 */
export function failWriteStringMatchingFsLayer(
  match: (path: string) => boolean,
): Layer.Layer<FileSystem.FileSystem> {
  return failWriteStringFsLayer((path) => match(path));
}

function failWriteStringFsLayer(
...
export function mockShadowContainerCliSpawner(
  ...
  if (opts.dbNotRunning === true && opts.dbInspectFailsWith !== undefined) {
    throw new Error(
      "mockShadowContainerCliSpawner: dbNotRunning and dbInspectFailsWith are mutually exclusive",
    );
  }
      ...
      Effect.gen(function* () {
        ...
        if (args[0] === "create") {
          if (failCreate) {
            ...
            stdoutLines = [FAKE_SHADOW_CONTAINER_ID];
          }
        } else if (args[0] === "container" && args[1] === "inspect") {
          stdoutLines = [neverHealthy ? SHADOW_STARTING_STATE : SHADOW_HEALTHY_STATE];
        } else if (args[0] === "rm") {
          if (failRemove) {
            exitCode = 1;
 ...
 */
const fakeTarBlock = (fields: ReadonlyArray<readonly [number, string]>): string => {
...
const fakeTarOctal = (value: number, width: number) =>
  `${value.toString(8).padStart(width, "0")}\0`;

/** One ustar member: a checksummed 512-byte header plus its NUL-padded content blocks. */
const fakeTarEntry = (name: string, content: string, typeFlag: "0" | "5"): string => {
  const header = fakeTarBlock([
    [0, name],
    [100, fakeTarOctal(typeFlag === "5" ? 0o755 : 0o600, 7)],
    [108, fakeTarOctal(0, 7)],
    [116, fakeTarOctal(0, 7)],
    [124, fakeTarOctal(content.length, 11)],
    [136, fakeTarOctal(0, 11)],
  ...
  return `${header.slice(0, 148)}${fakeTarOctal(checksum, 6)} ${header.slice(156)}${content}${padding}`;
};
 ...
 */
export const FAKE_UNSTAMPED_PGDATA_TAR = `${fakeTarEntry("data/", "", "5")}${fakeTarEntry("data/PG_VERSION", "17\n", "0")}${FAKE_TAR_END}`;
 ...
 */
export const fakePgDataTar = (markerContent: string): string =>
  `${fakeTarEntry("data/", "", "5")}${fakeTarEntry("data/PG_VERSION", "17\n", "0")}${fakeTarEntry("data/SUPABASE_BASELINE", markerContent, "0")}${FAKE_TAR_END}`;

/**
 * The `SUPABASE_BASELINE` member's content inside a `docker cp - <id>:<PGDATA>` stamp payload — a
 * plain 512-block walk over the archive `pgDataBaselineMarkerTar` built, so the fake export
 ...
 */
const fakeStampedMarkerContent = (stamp: string): string | undefined => {
  let offset = 0;
  while (offset + 512 <= stamp.length) {
    ...
    if (name === PGDATA_BASELINE_MARKER_NAME) return stamp.slice(offset, offset + size);
 ...
 */
export const FAKE_EMPTY_TAR = FAKE_TAR_END;
...
interface FakeContainer {
   ...
   * stamping publishes {@link FAKE_UNSTAMPED_PGDATA_TAR} and every warm-path test notices.
   */
 ...
 * this instead of {@link mockShadowContainerCliSpawner} whenever the shadow BASELINE CACHE
 ...
 */
export function mockDockerDaemonCliSpawner(
  opts: {
  ...
  const spawner = ChildProcessSpawner.make((command) =>
    Effect.gen(function* () {
      ...
      if (args[0] === "network" && args[1] === "inspect") {
        ...
        const isStamp = containerPath === PGDATA_PATH;
        const failed =
        ...
        if (opts.failCopyOut === true || container === undefined) {
          ...
          const marker =
            container.stamp === undefined ? undefined : fakeStampedMarkerContent(container.stamp);
          stdout = marker === undefined ? FAKE_UNSTAMPED_PGDATA_TAR : fakePgDataTar(marker);
        }
      } else if (args[0] === "container" && args[1] === "inspect") {
  ...
  const stepOf = (args: ReadonlyArray<string>): string => {
    ...
    if (args[0] === "cp") {
      if (args[1] === "-") {
        ...
        return containerPath === PGDATA_PATH ? "cp-stamp" : "cp-in";
      }
...
export interface BuildTestRuntimeOpts {
  ...
  // `Layer.Layer<CommandPlatformApi, SupabaseApiConfigError>` from
  // `mockCommandPlatformApi`; the error channel never fires in practice but
  // its presence here keeps callers from needing an `as` cast.
  readonly api: {
    readonly layer: Layer.Layer<CommandPlatformApi, SupabaseApiConfigError>;
    readonly httpClientLayer?: Layer.Layer<HttpClient.HttpClient>;
  };
  readonly cliSettings: Layer.Layer<CommandSettings>;
...
export function buildTestRuntime(opts: BuildTestRuntimeOpts) {
  ...
  const topLevelFactory = Layer.succeed(CommandPlatformApiFactory, {
    make: CommandPlatformApi.pipe(Effect.provide(opts.api.layer)),
  });
