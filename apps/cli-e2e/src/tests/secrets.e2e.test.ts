...
describe("secrets", () => {
  ...
  beforeEach(() => {
    tempDir = makeTempDir("cli-e2e-secrets-");
  });
  ...
  test("secrets list renders fixture data in output", async () => {
    const result = await exec(makeHarness(), ["secrets", "list", "--project-ref", PROJECT_REF]);
  ...
  test("secrets list exits non-zero on 401", async () => {
  ...
  test.skipIf(isRecording)("secrets list: ts-legacy stdout matches go", () =>
